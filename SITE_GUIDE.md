...
**Planning & strategy docs live in the repo — the `claude/` prefix is retired as a location convention (2026-08-08).** Every planning doc is tracked at repo root alongside `TECH_DEBT.md` and `PAGE_IDEAS_BACKLOG.md`, including the six migrated on 2026-08-08: `OPEN_ITEMS.md`, `X_STRATEGY_PLAYBOOK.md`, `REACH_GROWTH_PLAN.md`, `FUNDING_STRATEGY.md`, `CREATOR_CREDIBILITY_KIT.md`, `PARTNERSHIPS_REFERRALS_POLICY.md`. The `claude/` prefix used to denote assistant-facing, project-only docs kept *outside* the repo — but those copies drifted whenever a session couldn't read them (OPEN_ITEMS twice), so the split was dropped. **Do not recreate it:** no planning doc should live only in the project files, and a `claude/…` path in older cross-references is now just legacy citation text, not a real location. Still un-migrated at the time of writing (fold in when convenient): `claude/SEO_AUDIT.md`, `claude/OPENSOURCE_DECISION_BRIEF.md`, `claude/OWN_JOB_STRATEGY.md`, `claude/GRANTS_RESEARCH_KIT.md`.
