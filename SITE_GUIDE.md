...
## 13. Homepage carousel — completed set (35 slides — count drifts; markup is truth)
...
**The Bitcoin Hurdle Rate — SHIPPED (slide wired 2026-08-07; video pending silent strip).** Promoted from the pending concept. The shipped slide departs from the proposed stone-step-well idea — it is the v4 falling-light escarpment, the shadow edge sweeping the foreground (see the slide-added note + iteration record below) — and from "not Featured by default": it ships **Featured**, retiring The Bitcoin Migration (JM 2026-08-07). The **no-line-graph / channel-band** rule held (the chart identity stays on the page). Video referenced in `index.njk` but **not yet committed** — pending the external silent strip + marker check.
...
> ⚠️ **Pre-existing drift found during that renumber — NOT introduced by it, and deliberately left for JM rather than silently rewritten.** Re-deriving from markup (the stated source of truth) shows the table and `src/index.njk` agree only for **rows 1–9**; from row 10 they diverge. The cause is that several recent slides were **appended to the end of this table instead of inserted at their markup position**: Discount, or Premium? (markup 15) and The STRC Mechanism (markup 16, still tabled under its old name *STRC Below Par*) are the clearest cases, with The Bitcoin Hurdle Rate (markup 10) and What Daily Conviction Bought (markup 20) tabled at 38–39. Markup now carries **40** slides against the table's 39, so at least one is missing outright. Consequently a handful of iteration-record headers are off by one against the table — that mismatch predates this pass and the uniform +1 preserved it exactly rather than half-fixing it. Reconciling the table to markup is a standalone pass: it reorders ~30 rows of hand-written video-concept prose, and the row numbers are live citation anchors, so it wants doing deliberately and in one go, not as a side effect of a page ship.
...
| 3 | Bitcoin as Collateral | 0 | The pledge you can see. *Four thousand years of collateral – and the first pledge anyone can verify.* | Extreme macro in candlelit dark: a wax seal pressed onto parchment, and as the stamp lifts the impression glows from within, ember orange, still molten. First of three briefed takes (seal / boundary stone / vault-turned-glass), v1 accepted. Two deliberate calls that look like defects: the seal **never cools** — a cooling seal reads as one-time attestation, a molten one as continuous verification, which is the page’s actual claim; and v2 was rejected *despite* rendering the briefed lattice-spread, because it overshot into engulfing fire and lost the overlay dark zone. The spread therefore rides the headline, not the footage — the same division of labour as the Risks slide (row 4), whose antifragility beat also never rendered. |
...
| 6 | Bitcoin vs. Real Estate | — | Housing became the default store of value *by elimination, not merit* | Golden hour → twilight → dark silhouette |
...
| 9 | Bitcoin and The Power Law | 0 | For every 13% increase in Bitcoin's age, *the trend price doubles.* | Town at dusk, lawful illumination pattern |
| 10 | Bitcoin & Metcalfe's Law | **1** | The network is still there. *We just can't see it anymore.* | A single off-center bare tree on a still lakeshore at golden hour; the real tree and landscape stay crisp and sharp throughout while its mirror reflection in the water progressively blurs and breaks into unreadable ripples — the network persists while the on-chain reflection of it goes blind. (v2; see iteration record.) Silent — the PR-time audio track was stripped 2026-06-20 in `6c7b500` (resolved, not forgotten). _(Table drift note: the Doubling Ladder slide, shipped June 2026, is not separately tabled; `src/index.njk` is the source of truth for absolute order.)_ |
...
| 18 | The Bitcoin Horizon | — | What looks like turbulence over the course of a day is *trajectory over the course of a decade.* | Small ship at golden hour, crossing toward the horizon; near-shore chop, steady distant line; persistence-through-turbulence |
| 19 | Is Bitcoin a Bubble? | — | A thing isn't a bubble simply because *it has been called one.* | ~10 bubbles burst; one amber object remains impervious |
| 20 | The Half-Life | — | How long until your money *loses half its value?* | Candle burning down asymptotically |
| 21 | The Melting Ice Cube | — | Holding cash is not safety. It's an *active decision with an ongoing cost.* | Ice cube melts to fragment on weathered wood |
| 22 | The Bitcoin Synthesis | — | Six components. *One irreducible synthesis.* | Six antique keys assemble into clockwork mechanism |
| 23 | What Bitcoin Is | — | Most people consider just a few dimensions. Bitcoin is *all of them, simultaneously* | Raw mineral crystal, traveling amber light |
| 24 | The Bitcoin Migration | 0 | From what can't be fixed, *to what can't be broken.* | Heavy wooden door opens ~2/3, amber light. **Demoted from Featured 2026-08-07** (feat 1→0) to promote The Bitcoin Hurdle Rate — holds Featured at 13 and shifts balance to The Numbers; stays in the Arguments tab. |
| 25 | The Trilemma | — | Some problems are *navigated, not solved.* | Three geometric counterweights in coupled sway |
| 26 | The Money Trees | — | Two systems. *Different roots. One outcome.* | Two trees; left withers, right unchanged |
| 27 | The Bitcoin Retirement | **1** | A stack that lasts a lifetime — *or runs out before you do.* | Solitary mature tree in golden-hour meadow; sun moves from behind canopy to streaming rays through it; latent-state opening, persistence across the arc |
| 28 | Disciplined Rebalancing | — | A model you believe in is *a protocol you can run.* | Wheat field at golden hour, ears swaying in coupled bounded oscillation; fixed camera, stable disequilibrium |
| 29 | The Fixed Pie | 0 | Your share remains undiluted — *for eternity.* | Unmarked gold coin; surrounding coins dissolve |
| 30 | Bitcoin vs. The Stock Market | **1** | Three growth curves. *One decisive horizon.* | Three trees in a meadow at golden hour; the middle tree grows dramatically taller and fuller-canopied over 10 seconds while the flanking trees mature modestly; latent-state opening, divergent outcomes from identical conditions |
| 31 | The Bitcoin Heatmap | — | The pattern reveals itself *only when you zoom out.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 32 | Borrowing Against Your Stack | — | Every coin sold today is a coin that *compounds for someone else.* | Solitary ancient standing stone — weathered, lichen-marked, ~six feet tall — in a golden-hour meadow; tall grasses bend in gentle wind, long shadow extends from the stone across the foreground; the stone itself does not move; quiet persistence as the visual argument |
| 33 | Bitcoin-Backed Mortgages | — | The home opens. *The stack stays.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 34 | Living on Bitcoin | — | Save in bitcoin. Spend in fiat. *The float holds.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 35 | Bitcoin Defined | — | Define it. *Then debate it.* | Bed of glowing embers in deep darkness — same composition as Card 7 (Bounded by Energy) on the page. Camera fixed throughout. Four-layer subtle motion: heat shimmer above the bed, asymmetric ember pulse (no two embers in sync), a small natural wood-fire flame at center-left with irregular asymmetric tongues whose base merges with the surrounding embers, wisps of pale smoke rising and dissipating. Seamlessly looping 10-second silent video. |
...
| 37 | STRC Below Par | 0 | Built to hold a line — *trading beneath it.* | Engineered-luminous register (§6): a night masonry lock-chamber, one horizontal amber light-line, moon, rising water — stone arches stand above the waterline at the start and are half-swallowed by the end (the object-state-change device). **Numbers only, not Featured** (JM 2026-07-28 prominence decision). Silent, 720p, ~10s, 4.17 MB (audio + thumbnail streams stripped, +faststart). **Markup position:** inserted in `src/index.njk` after the Discount, or Premium? slide (mid-Numbers cluster), _not_ at the end — `src/index.njk` is authoritative for absolute order; this table row is appended here pending the next full re-derive (same drift class as Discount, or Premium?, which is also not yet integer-slotted). See iteration record below. |
| 38 | The Bitcoin Hurdle Rate | **1** | Every use of capital is measured against something. *Most bars are set far too low.* | Falling light on a dark hard-edged escarpment over low foreground hills — the two nearest hills sunlit at 0:00, fully in shadow by 0:09, the shadow edge sweeping across the foreground while the escarpment's upper strata stay lit (v4; the named-two-states device, §6). **Featured**, retiring The Bitcoin Migration (JM 2026-08-07). **Video NOT yet committed — pending the silent strip + marker check.** **Markup position:** after Bitcoin and Fixed Income; row appended pending the next full re-derive (STRC/Discount drift class). See iteration record below. |
| 39 | What Daily Conviction Bought | 0 | Nothing clever. Nothing timed. *Just every single day.* | Drystone wall at golden hour; a climbing vine from a lower-left woody stump spreads across the wall to the upper right by 0:08, stone visible behind, not smothered (first-take; continuous-growth phrasing worked — familiar nature-footage prior). **Not Featured** — personal-register retrospective, adjacent to the Bitcoin Exit essay. **Video NOT yet committed — pending the silent strip.** **Markup position:** after the deployment trilogy (Wait, or Deploy Now?); row appended pending the next full re-derive. See iteration record below. |

### Iteration record — slide 38, The Bitcoin Hurdle Rate (August 2026)
...
### Iteration record — slide 39, What Daily Conviction Bought (August 2026)
...
### Iteration record — slide 37, STRC Below Par (July 2026)
...
### Iteration record — slide 29, Bitcoin vs. The Stock Market (May 2026)
...
The trees direction has light visual overlap with two other tree-themed slides (slide 25 Money Trees / two trees, slide 26 Bitcoin Retirement / one tree). Adjacency is broken by slide 27 (wheat field) and slide 28 (gold coin) between slides 15 and 18; framings differ enough (centered single vs wide three) that the shared visual vocabulary reinforces site identity rather than producing monotony — the carousel's overall grammar is part of what makes the set feel like one artifact.

### Iteration record — slide 34, Bitcoin Defined (May 2026)
...
### Iteration record — slide 35, What Money Is For (May 2026)
