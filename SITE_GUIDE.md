...
- **Chips** (`.rt-cmp-chip`, `aria-pressed`): *Retired 2 years later · Retired 5 years later · Retired 2 years earlier · Retired 5 years earlier · Wanted $25K/yr less · Wanted $25K/yr more · Withdrew 4% (the traditional rule)*. Each is a single-lever rule (income OR retirement year — never both). Tapping a chip adds a variant column; **max 2 active variants** (+ base = 3 columns); a third tap evicts the **oldest** active variant; tapping an active chip removes it.
- **Availability, not clamping** (fix Aug 2026): each variant declares `available(base)`. When a **delta** rule (retire ±N, income ±$25K) would land outside the slider bounds it must be reproduced from (`retirementYear [2026,2070]`, `targetIncomeUSD [20000,1000000]`), the chip renders **disabled/dimmed** (`aria-disabled="true"`, still focusable) with a reason surfaced on hover (`title`), focus, and tap (the `#rtCmpReason` aria-live line, for touch); no column is added. The **4% chip is an absolute rule, deliberately NOT bound-gated** — if 4% of the projected stack is $12K, $12K is the truthful answer and the $20K income-slider floor is a UI constraint, not a modelling one; disabling it there would silence the traditional-rule comparison in exactly the early-retirement / small-stack cases where it is most sobering. Its only guard is the no-op (base already ~4%, `|0.04·stack − income| < 500`). If a base-slider drag pushes an active variant out of bounds, `cmpComputeAll` drops its column and its chip un-presses the same tick. Variants are **never silently clamped** — a "Retired 5 years earlier" column that was really 1 year earlier would be a false statement that understates the delta. This also removed the earlier `dca2` chip, whose `build` was byte-identical to `later2` (in this engine DCA runs until the retirement year, so "keep DCA going 2 more years" and "retire 2 years later" are the same lever) — a true "keep stacking past retirement" question needs a `dcaEndYear` field and its own later spec.
- **Columns**: leftmost is *Your current scenario* (the live base), then variants. Each shows *Years stack lasts* (or "∞ — escape velocity"), *Stack at retirement (today's $)*, and *Sustainable income*. **Everything is today's-dollars (real), trend-basis** — independent of the page's nominal/real and trend/current display toggles — because variants change the retirement year and only a common real basis compares cleanly across years (the note under the panel states this). The 4%-rule column is the one exception to the "carry the base income through" rule: its income is `0.04 × its own stack-at-retirement`, flagged with an *"at 4% withdrawal"* footnote.
- **Delta row** (per variant): the top-two signed differences vs base, priority **escape-velocity flip > years-of-stack > income > stack-value**, with a plain "retire YYYY instead of YYYY" annotation as lowest-priority filler. An EV flip is the headline in both directions ("reaches escape velocity — the base case depletes in ~N years" / "depletes in ~N years — the base case reaches escape velocity"). Delta numbers use the site-standard pos/neg colors only (`#7fc47f` / `#c0603a`); the flip headline is amber. **No column tinting, no winner treatment** — overlay/swatch colors are assigned by slot order, never by valence.
- **Chart overlay**: each active variant's drawdown curve overlays the projection chart as a thin (1.5px) lighter line — slot 0 warm gold `#f0c987` dash `[6,4]`, slot 1 cool sage `#8fb0a0` dash `[3,3]` — appended after the six base datasets (indices 6–7, outside the HTML legend's 0–5 toggle map) at `order:3` so the cream base drawdown line stays visually primary. Each column header carries a line-style-faithful SVG swatch in its overlay color (house font colors throughout), tying column to line. Chart-copy (§6.16) captures the canvas pixels, so the export cleanly includes overlays.
- **Engine reuse / perf**: `cmpComputeAll()` runs once per render tick, reusing `renderChart`'s already-computed base drawdown projection and computing each active variant exactly once (memoized, shared between overlay datasets and columns) — ≤ 2 added engine runs with 2 variants. Chip availability is cheap arithmetic on the base fields (the 4% chip's guard adds one `computeStackAtRetirement` loop).
