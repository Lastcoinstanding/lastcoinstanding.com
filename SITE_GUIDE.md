...
## 13. Homepage carousel — completed set (42 slides — re-derived from markup 2026-08-26; markup is truth)
...
**Compare Retirement Plans (§52) — PENDING FOOTAGE, brief written 2026-08-26.** The page shipped without a slide by design; the slide is a follow-up PR that lands only once footage clears the §13 acceptance gate chat-side. **Deliberately NOT an inventory-table row:** that table is one row per *live* slide and was re-derived to exactly 42 rows for 42 slides on 2026-08-26, so a row for a slide that is not in `src/index.njk` would reopen the drift that pass closed. It belongs here until the footage exists.

Copy (drafted with the page): label *"Compare Retirement Plans"*; headline *"Every plan is a choice against the plan you didn't pick."*; CTA *"Weigh two plans &rarr;"*. `data-cat="numbers"`. Featured status is JM's call at wiring time; the ship default is `data-feat="0"` as a hold, per the collateral/floor convention.

**Concept brief, written to the register canon above.** Two paths leaving one gate. A single wide golden-hour shot of a broad meadow where a worn track forks at a low drystone gatepost in the foreground; both branches run away from camera toward a bright horizon, one climbing gently along a ridge into fuller light, the other holding the flat ground through longer grass. Warm light dominates the frame throughout and the sun sits ahead of the traveller, not behind — the destination is the lit thing. Over the ten seconds the light *rises*: the far end of both tracks brightens, the ridge branch catching it first and the low branch a beat later, so the closing frame is the brightest in the clip and neither path ever falls into shadow. Nothing moves but light and grass.

**Why this brief and not another.** The argument is *two complete options from one starting point*, so the metaphor is a fork, not a race — and the two branches must read as **different, not better and worse**. That is the one thing to hold the line on in iteration: no branch may be styled as the wrong turn (no darkening, no thorns, no dead end), because the page refuses to crown a winner and the slide cannot contradict the page. Register: default warm/hopeful/constructive, natural setting, movement *with direction* — both tracks travel toward light. Not an engineered-luminous subject; there is no single quantity to make glow.

**Acceptance gate, to be measured chat-side per `NEW_PAGE_CHECKLIST §8` before any wiring PR:** mean-luminance floor at or above ~25%, no sustained falling brightness arc, closing frame at least as bright as the opening frame. This page's subject is a choice, not a risk, so the Stress Test exemption does not apply and all three items are hard.

**Known iteration risk (from the §6 record).** "A track forks and both branches stay good" is a *metaphor* brief, which the guide's prior-strength rule predicts will cost a revision — and the specific failure to expect is the model resolving the fork into a single dominant path, or shading one branch to signal the choice. If v1 does that, the fix the section already documents is to name **two object states** rather than a relationship: "the left track's far stones fully sunlit at 0:00 and the right track's far stones fully sunlit by 0:09" gives two nameable keyframes instead of an abstraction about balance.
...
| 34 | Disciplined Rebalancing | — | A model you believe in is *a protocol you can run.* | Wheat field at golden hour, ears swaying in coupled bounded oscillation; fixed camera, stable disequilibrium |
| 35 | The Fixed Pie | 0 | Your share remains undiluted — *for eternity.* | Unmarked gold coin; surrounding coins dissolve |
| 36 | Bitcoin vs. The Stock Market | **1** | Three growth curves. *One decisive horizon.* | Three trees in a meadow at golden hour; the middle tree grows dramatically taller and fuller-canopied over 10 seconds while the flanking trees mature modestly; latent-state opening, divergent outcomes from identical conditions |
| 37 | The Bitcoin Heatmap | — | The pattern reveals itself *only when you zoom out.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 38 | Borrowing Against Your Stack | — | Every coin sold today is a coin that *compounds for someone else.* | Solitary ancient standing stone — weathered, lichen-marked, ~six feet tall — in a golden-hour meadow; tall grasses bend in gentle wind, long shadow extends from the stone across the foreground; the stone itself does not move; quiet persistence as the visual argument |
| 39 | Bitcoin-Backed Mortgages | — | The home opens. *The stack stays.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 40 | Living on Bitcoin | — | Save in bitcoin. Spend in fiat. *The float holds.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 41 | Bitcoin Defined | — | Define it. *Then debate it.* | Bed of glowing embers in deep darkness — same composition as Card 7 (Bounded by Energy) on the page. Camera fixed throughout. Four-layer subtle motion: heat shimmer above the bed, asymmetric ember pulse (no two embers in sync), a small natural wood-fire flame at center-left with irregular asymmetric tongues whose base merges with the surrounding embers, wisps of pale smoke rising and dissipating. Seamlessly looping 10-second silent video. |
...
### Iteration record — slide 36, Bitcoin vs. The Stock Market (May 2026)
...
### Iteration record — slide 42, What Money Is For (May 2026)
