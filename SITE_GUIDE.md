...
### Structure
Hero (question-title) → **live status strip** (STRC price · coupon · effective yield) → **§A the lever board** (four levers — rate / supply / bid / a fuel gauge carrying the three live coverage ratios — each a status readout + append-only log with a "latest" badge) → **§B the below-par arithmetic + the interactive** ("the below-par lens": price input, horizon slider + never toggle, dividend-scenario toggle, opt-in bitcoin/Treasury overlays, demanded-yield ⇄ price converter) → **§C the sharp question** (a computed STRC-vs-bitcoin table: "if you're waiting for bitcoin to recover, why not simply own bitcoin?") → **§D the cost accounting** (each lever's cost, both ways; which-number-to-watch breakevens) → **§E the record strip** (a schematic of price-vs-par since issuance — shape, not tick data) → FAQ → related → disclosures.
