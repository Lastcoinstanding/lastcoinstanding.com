...
- **~~CSS foundation preamble missing.~~ CLOSED 2026-07-15.** The page shipped without the opening block every sibling carries, because the build re-derived widths from the STYLE_GUIDE §4.2 table (correctly) and skipped the donor's preamble along with them. `base.njk` has no site-wide palette, `body` theme, or link rule, so the omissions did not degrade — they fell back to the user agent: browser-white body with black text (fixed by JM's `f006b5d` / PR #36, which restored `:root` + `body` + `.hero`), then user-agent blue in-prose links, an unpadded `.content-area`, an unstyled `.section-intro`, and a `.help-tip` whose `.tip-content` printed inline as visible body copy. Remainder fixed here; the page now has zero missing foundation selectors against both donors. **The general rule is recorded in STYLE_GUIDE §4.2 ('Foundation preamble'), directly under the scaffolding warning that caused it** — that warning says what not to copy from a donor and was silent on what must be. Note the QA seam this exposed: the offline harness stubbed the palette tokens itself, which reproduced the exact assumption under test and hid every one of these; it now loads only base.njk's real blocks, so the page stylesheet must stand alone.
