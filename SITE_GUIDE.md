...
- **Verified** (branch `rt-compare-fixes`, local stitch-harness): exact-equality gate byte-exact including **at the bounds** (income at $20K and $1M, retirement at 2026 and 2070) and for the 4% chip from a `fixed`-basis base (confirming it no longer switches basis); availability disables the right chips at every bound with the right reasons; an active variant auto-drops (column + chip) when a base drag invalidates it; non-degeneracy (no variant column equals base, no two active columns identical); EV flip both directions; chip cap / oldest-eviction / toggle-off; 375px base-first vertical stack with 7 chips wrapping and zero horizontal overflow; disabled-chip style (opacity 0.4, dashed, `title` reason) and reason-on-tap confirmed; no console errors through rapid churn across bounds; FAQ visible text + FAQPage JSON-LD untouched.
