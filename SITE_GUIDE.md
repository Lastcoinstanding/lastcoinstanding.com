...
**Custody — the step the gate implies but does not itself perform.** A gate result certifies **a specific encode**, not a filename. So the wiring step verified the placed file's **SHA-1 against the gated encode** (`4cd5a4fdaa2a778a3323fb811ca5df4d834b0c5c`, 5,972,363 bytes) before anything was committed, and the file was deliberately **left untracked and unwired** on first discovery, because a file appearing on disk is not evidence that it passed anything. **Make this the pattern:** the gate says *this encode passed*; the wiring PR says *this is that encode*. Without the second half the first is a claim about a file nobody can identify later.
