...
**Params.** `share` (always written) · `tax` · `pos` (**v3.1: a channel position 0–1, 3dp; absent = today.** Was a marker-day in v3 — a stale day-encoded `?pos` is out of range and falls back to today) · `stack`. The ten v2 params (`exp`, `buf`, `shock`, `hz`, `yld`, `depth`, `btc`, `dep`, `cy`, `rec`) are **retired**: decoded and ignored, never rewritten. Verified.
