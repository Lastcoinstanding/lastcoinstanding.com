...
**Shipped 2026-08-10 (`feat/dashboard-v1`).** The site's **return-visit anchor destination** — the resolution of the three competing "come back here" surfaces (PAGE_IDEAS_BACKLOG dashboard ruling, JM 2026-08-05): the dashboard is the room, the site-wide **channel-position chip is its future doorway** (ships after, not built here), and the **Gallery-highlights strip is closed**, absorbed here as the jump-back-in row. Positioning is the site's own vocabulary — *"where are we in the channel, and what does that mean for your position"* — which Clark Moody's network-health dashboard (the explicit foil) does not own.
