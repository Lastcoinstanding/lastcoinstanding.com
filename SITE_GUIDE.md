...
**Chart geometry (a design conflict flagged, not silently resolved).** Design §2.4 defines the band's upper edge as spot→trend; §5.2 + the §2.2 fixture + the §7 overclaim flag point to trend→trend `trendCAGR` as the honest hurdle. With bitcoin on its floor today (~0.42× trend), spot→trend spikes to ~227%/yr at H=1 — which would breach the overclaim flag and wreck legibility. Resolution: primary declining curve = `trendCAGR(H)`; floor-path band beneath = spot→floor (the conservative/access-conditioned reading, ≈ the curve today because spot sits on the floor, widening if price rises above it); the spot→trend reversion upside is a **secondary stat** at the chosen horizon (~41% at 10yr), not a chart line. All three are computed in the engine, so §2.4's literal reading is a one-line flip if JM prefers it on preview.
