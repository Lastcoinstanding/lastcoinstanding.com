...
- **Bitcoin & Metcalfe's Law** (page shipped 2026-06-19; §29) — carousel slide pending a Grok Imagine video. Proposed tonal direction: a constellation/network of warm amber points joined by light-threads, dense and bright on one side and thinning into darkness on the other — the network effect made visible, with the "data going blind" read carried by the fade (lawful, not chaotic; in-family with the standing-stone and basin slides). Proposed copy: label "Bitcoin & Metcalfe's Law"; headline *"The network is real. The data is going blind."*; CTA *"Measure it yourself →"*. On ship, promote into Featured and retire one (Featured holds at ~10), per the curation rule.
...
- `bal` cells (holders) are **exact OLS fits** on Coin Metrics `AdrBalCnt` vs `PriceUSD`, log-log, by era. Left as-is from the approved draft (full 1.84/R²0.95; retail 1.48/0.82; inst 3.07/0.77; ETF 2.86/0.09).
- `act` cells (active addresses) were **recomputed at launch** (the draft's were analyst estimates). Live pull from **Blockchain.com Charts** (`n-unique-addresses`, `market-price`), OLS log-log, over the **same era boundaries** used for `bal`. Boundaries were calibrated by reproducing the exact `bal` values: retail = 2011-01-01→2016-12-31; **institutional onset = 2017-01-01→2020-03-31** (the COVID-crash boundary — reproduces `bal` inst 3.14/R²0.78 vs the page's 3.07/0.77, the closest match across candidate end-dates); ETF = 2024-01-01→present; full = 2011→present. Cross-checked against Coin Metrics `AdrActCnt` (same shape).
...
**Open items.**
- **Carousel slide pending** (needs a Grok Imagine video) — tracked in §13 "Pending additions."
- **OG image is a placeholder.** `og-bitcoin-and-metcalfes-law.jpg` currently duplicates the generic site card (`og-image.jpg`) so the social card isn't a 404 and the `.eleventy.js` passthrough has a real source; the real per-page branded OG still needs the §6.15 image pipeline (no local Node/Python at build time of this PR).
- **No local Eleventy build was run** for this PR (no Node on the authoring machine); templates were validated statically (JSON parse, include-path existence) and rely on the Cloudflare Pages preview build + the §10 post-deploy SEO curl checks.