...
- **Style Guide compliance fixes** (per `STYLE_GUIDE.md` §5, anti-patterns). Highest-impact fixes: Cormorant italic at small sizes on dark backgrounds (homepage subtitle, Migration cover subtitle, Trilemma subtitle, Half-Life Saylor quote — swap to Source Serif italic or Inter); WBI/Synthesis Inter Bold Orange title (swap to canonical Cormorant); Trilemma "Why This Is the Only Path" wide-tracked uppercase Cormorant (drop the wide tracking).
