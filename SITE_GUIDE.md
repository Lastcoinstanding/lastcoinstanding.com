...
- **STRC Below Par** (§42, shipped July 2026 without a slide). Carousel slide deferred by design, as with every new page. Tonal direction TBD — candidate register: a line hovering just under a level it was built to hold (a float that keeps dipping below a waterline; a needle resting just shy of a marked line). No triumphal or collapse imagery — the page is deliberately two-sided and verdict-free, and the slide must not tilt it. When it lands, promote into Featured and retire one (Featured stays ~10), and move this note into the inventory table below.

_Otherwise none currently._ The **deployment-trilogy** slides (Lump Sum or Ladder In?, Your Bitcoin Deployment Plan, Wait, or Deploy Now?) shipped June 2026 and are now in the inventory table below — the videos landed as a tonal arc rather than the literal channel-diagram directions first proposed (firelit two-glass pour → aerial golden river valley → sun emerging from cloud over calm sea). The Bitcoin & Metcalfe's Law slide shipped June 2026 as well (see the slide-added note above and the iteration record below).
