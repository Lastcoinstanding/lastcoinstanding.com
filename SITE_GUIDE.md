...
- **Senders write today's live position.** How Much Cash's underwater-manager handoff link carries `?pos=<livePos, 3dp>` (clamped into range, so it is always a value the receiver accepts).
