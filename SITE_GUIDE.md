...
**Vocabulary (2026-09-07): the visible noun is "episode", not "stretch".** The data model, the thinness rule and the sources lines already counted in episodes; the surface prose lagged. Both this page and Discount-or-Premium now say "episode" wherever they mean a run — with one deliberate exception on D-or-P, whose slider caption is attached to **sample**-basis figures and would be mislabelled by the change. The tooltip that defines the noun on first use is kept (§6.13).
