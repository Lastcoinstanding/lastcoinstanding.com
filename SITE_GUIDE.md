...
- **OG card:** product-forward (§6.15.2) &ndash; hero is the All-rows scorecard including the strips row, captured after a `click_before_capture` on the Scorecard tab. Ratings are editorial, not live data, so this card carries **no** MONTHLY_REFRESH regen dependency (unlike the chart-hero cards).
