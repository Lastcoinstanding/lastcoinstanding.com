...
- **Chips** (`.rt-cmp-chip`, `aria-pressed`): *Retired 2 years later · Retired 5 years later · Retired 2 years earlier · Wanted $25K/yr less · Wanted $25K/yr more · Withdrew 4% (the traditional rule) · Kept DCA going 2 more years*. The DCA chip renders only when Monthly DCA > 0, and is pruned from the active set if DCA returns to 0. Tapping a chip adds a variant column; **max 2 active variants** (+ base = 3 columns); a third tap evicts the **oldest** active variant; tapping an active chip removes it.
...
- **Engine reuse / perf**: `cmpComputeAll()` runs once per render tick, reusing `renderChart`'s already-computed base drawdown projection and computing each active variant exactly once (memoized, shared between overlay datasets and columns) — ≤ 2 added engine runs with 2 variants. **In this engine "keep DCA going 2 more years" and "retire 2 years later" are the same lever** (DCA runs until the retirement year), so `dca2` maps to `retirementYear + 2`; the DCA-framed chip exists for the DCA-slider user and is gated to DCA > 0.
- **Verified** (branch `rt-compare`, local stitch-harness): exact-equality gate byte-exact for all seven chips (the 4% chip reproduced at income = 4% × stack); EV flip both directions; chip cap / oldest-eviction / toggle-off; DCA gate; 375px base-first vertical stack with chips wrapping and zero horizontal overflow inside the panel; delta colors and swatch↔overlay tie confirmed by computed-style probe; no console errors through rapid chip/slider churn; FAQ visible text + FAQPage JSON-LD untouched.
