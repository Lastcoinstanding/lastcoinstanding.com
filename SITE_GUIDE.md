...
**Deliberately deferred, and tracked rather than silent.** (1) **The OG card** `og-compare-retirement-plans.jpg` — the meta tags reference it and the file does not exist yet; it needs the Python/Pillow pipeline, which is chat-sandbox work (`NEW_PAGE_CHECKLIST §8`'s tooling note). **Full generation spec — both cards, the conventions this one must honour, and the WDCB rider — is `OG_ASSET_HANDOFF.md`.** (2) **The carousel slide** — no slide in the ship PR by design; the concept brief is in §13 "Pending additions". (3) **Inbound `related:` links from the three existing family pages** — those are edits to other pages and were out of scope for this batch; the family strip (design §10, shipping separately) is what carries the linkage instead.
...
**Open.** No dedicated OG card — the hub points `og:image` at the flagship's `og-the-bitcoin-retirement.jpg`, which is on-family and presentable but previews as the flagship rather than as the hub. Lower priority than the Compare card (`§52`, `TECH_DEBT`), which is currently broken rather than merely generic; fold both into the next OG batch — specced as Priority 2 in `OG_ASSET_HANDOFF.md`.
