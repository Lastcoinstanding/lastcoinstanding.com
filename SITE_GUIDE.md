...
**The Bitcoin Hurdle Rate (proposed, August 2026).** Slide pending per house norm; brief separately. Register: the *decision* register, not oscillation or persistence — a single subject against which a **declining bar** is measured. Candidate concept (natural-element grammar, §5): a weathered stone step-well or a series of descending stone ledges in warm low light, water finding its level against each successively lower ledge — the bar a decision must clear, falling as the horizon lengthens. Proposed copy — label "The Bitcoin Hurdle Rate"; headline *"Does it beat bitcoin? The bar every capital decision has to clear — and it falls with the horizon."*; CTA *"Set the bar →"*. Not Featured by default (Positioning & Strategy is already well-represented in the Featured set); place in that cluster. Keep the headline a question adjacent to the conditions, per the page's own refrain discipline. **No line-graph / channel-band imagery** (§13 carousel rule — the chart identity lives on the page, not the slide).
...
| 23 | The Bitcoin Migration | **1** | From what can't be fixed, *to what can't be broken.* | Heavy wooden door opens ~2/3, amber light |
