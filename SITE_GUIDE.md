...
### The one automated exception (bot data-commits)

There is exactly one path that commits to `main` without a JM-merged PR: the
**STRC daily-close Action** (`.github/workflows/strc-daily-close.yml`, added
2026-08-10 — the site's first CI). It writes **only** `src/_data/strcClose.json`,
prefixed `data(strc): `, guarded by a >25% day-over-day fuse. This exception is
**scoped to that one file**; it does not generalize. Any other automation that
would commit to `main` needs its own explicit ruling. Full record: §42.
...
- **Name two object states, not a continuous quantity.** Grok interpolates reliably between two nameable keyframes ("arches above the waterline" → "half-swallowed"; "foreground hills fully sunlit" → "fully in shadow") but will not animate a continuous quantity described as a process ("the water rises", "the light falls across the landscape"). Given a process description it typically renders the cause while leaving the effect static — animating inflow without raising the level, deepening the sunset without moving the shadow boundary. If a brief describes something changing gradually, restate it as a start state and an end state in named objects. **Worked cases (third slide to hit this, hence the rule):** STRC Below Par (slide 36 — inflow rendered without the level rising) and the Bitcoin Hurdle Rate slide (a deepening sunset without the shadow boundary moving). **Boundary condition — this rule is about *weak* priors.** Continuous-process phrasing is only unsafe where the model has no strong prior for the motion and needs the two named keyframes to carry it (water levels, a travelling shadow edge). Where the process *is* a strong prior it renders cleanly from the process description alone and needs no two-state restatement: the What Daily Conviction Bought slide (38) briefed time-lapse vine growth as a continuous spread and landed first-take, because time-lapse plant growth is a familiar nature-footage pattern. Restate as two states when the motion is one Grok has to invent; leave it as a process when it's one Grok already knows.
...
**The engineered-luminous register (JM, July 2026) — a third tonal camp.** Alongside the site's two established video registers — **warm-dark / intimate** (fireplace, candle, tabletop; the human-scale emblems) and **golden-hour / natural** (meadows, trees, open sea; the living-element comparisons) — a third register earns its name with the July 2026 pair: **engineered-luminous.** Its grammar: **monumental dark architecture** (masonry, chambers, spans, vast constructed scale) at **night, in mist**, with **exactly one glowing element doing the semantic work** — a single amber light-line, a lit trend-wall, one luminous seam — against otherwise unlit stone. The mood is **ominous-calm: still, cold, weighty, never action or triumph.** It suits **The Numbers' *engineered* subjects** — the financialized, constructed instruments (a peg, a trend channel, a capital structure) that don't belong in the meadow register. The single glowing element is load-bearing: it is the one quantity the slide is *about*, and the darkness around it is what keeps the eye on it. **Founding entries:** Discount, or Premium? (the glowing trend-line wall the price rides above and below) and STRC Below Par (the night lock-chamber, one amber light-line, rising water). **Standing exclusions carry forward unchanged:** no orbs or glowing spheres, no neon grids, no Tron/synthwave aesthetic, no "digital" cyber-cliché, and no triumph cinematography (§5). One glowing element, doing real semantic work — never decoration, never a light show.
...
**Planning & strategy docs live in the repo — the `claude/` prefix is retired, migration COMPLETE (2026-08-08).** Every planning, strategy, design, and handoff doc is tracked at repo root alongside `TECH_DEBT.md` and `PAGE_IDEAS_BACKLOG.md`. Fifteen former project-only docs were migrated on 2026-08-08 (verbatim, CRLF preserved): `OPEN_ITEMS.md`, `X_STRATEGY_PLAYBOOK.md`, `REACH_GROWTH_PLAN.md`, `FUNDING_STRATEGY.md`, `CREATOR_CREDIBILITY_KIT.md`, `PARTNERSHIPS_REFERRALS_POLICY.md`, `SEO_AUDIT.md`, `OPENSOURCE_DECISION_BRIEF.md`, `OWN_JOB_STRATEGY.md` (since removed — see below), `GRANTS_RESEARCH_KIT.md`, `SEO_IMPLEMENTATION_PACK.md`, `FAQ_BLOCKS_AND_SCHEMA.md`, `THE_BITCOIN_EXIT_PUBLISHED.md`, `RETIREMENT_SCENARIO_COMPARISON_DESIGN.md`, and `RT_COMPARE_HANDOFF.md`. A sixteenth, `DISCOUNT_OR_PREMIUM_DESIGN.md`, already had a repo copy that was ahead — the project-only version was a stale mid-development duplicate and was discarded, not migrated. The `claude/` prefix used to denote assistant-facing, project-only docs kept *outside* the repo — those copies drifted whenever a session couldn't read them (OPEN_ITEMS twice), which is why the split was retired. **One exception, ruled 2026-08-10 and applied 2026-09-16: `OWN_JOB_STRATEGY.md` is project-side and was removed from the repo; repo docs cite it by name only.** **No other project-only planning docs remain, and no `claude/` doc references remain in the repo** (verified repo-wide 2026-08-08; the sole `claude/` string left is the `/home/claude/lcs-build` filesystem path in a build-history note, not a doc reference). **Do not recreate the split:** every planning/design/handoff doc lives at repo root and is cited by bare `<NAME>.md`.
...
- **Live doubling stat strip (v2, item c; polished 2026-08-05):** three kicker / value / caption blocks — "Bitcoin is · N days old · since the genesis block", "It takes · ~820 days · for the *trend* to double from here", "The constant · 12.76% · …2^(1/5.77)". Age and interval (`age × (2^(1/PL_B) − 1)`) computed live from `TODAY_DAYS` — never hardcoded. The doubling tile carries a tooltip clarifying it is the *trend's* doubling, not price's, and adapts to the above/below-trend state. §6.10a-declarative.
...
- Out-of-Sample Validation chart (**reader-driven since v2, item a; readout redesigned 2026-08-05**): the training-window cutoff is a drag handle + preset chips (end-2014/2016/2017/2020/2023). Default cutoff **end-2017 (slope 5.657)**; the regression, projection, and readout recompute in-browser on input. The readout is fitted a, b · implied trend today · **Δ vs the full-data self-fit** (the fourth field was changed from "actual vs. fit" — that number was contaminated by channel position, reading ~−57% even for a perfect fit; the trend-vs-trend Δ isolates what the extra years changed, and lands ~−1% at end-2017). A live drive-home sentence ("a fit that saw nothing after {cutoff} lands within {Δ}% … {N} years have barely moved the line") and a channel-position line ("today's price sits at {mult}× this fitted trend") sit under the readout; both adapt on drag / preset / cold-load. Every readout field, plus "out-of-sample" and "fit window", carries a `.help-tip`. Deep-linkable via **`?fit=YYYY-MM`** (restored before first render; the tab handler preserves the query string). The **end-2014 preset reproduces the documented bad fit (slope 6.787)**; the "we shipped the bad window" footer was cut and its one clause folded into the intro prose. (Original refit in commit `6604126`, 2026-05-07 — see DATA_AUDIT's Architectural change log.)
- **Exponent survey + explorer (v2, item b; restructured 2026-08-05):** copy is organized around **five reader questions** — what the exponent and coefficients are; why authors differ; whether fits change over time (the two Porkopolis snapshots answer this: drift, not revision); what the differences imply (fan-out with horizon); and why the site uses the one it does (the "reference fit" — longest-standing published, tightest floor, stable-by-design). A page-local explorer redraws the reference vs. a selected pair and tabulates implied price + deviation at 2026/2035/2045/2060 (ranks by implied price, not exponent). The five questions are mirrored into the page's **FAQ block** (`faq:` front-matter → `components/page-faq.njk` + FAQPage JSON-LD, §6.40). Reader-facing "canonical" was swept to "reference fit" per STYLE_GUIDE §5.
...
## 13. Homepage carousel — completed set (43 slides — re-derived from markup 2026-08-26; markup is truth)

All slides deployed with 16:9 widescreen silent videos, minimalist copy pattern (label + headline + CTA, no `.insight-desc`).

> **The end-to-end procedure lives in `NEW_PAGE_CHECKLIST §8`** — prompt → Grok → verify-the-take → strip streams → wire → promote, including *where the tooling runs* (the ffmpeg/verify steps are chat-sandbox work, not JM's machine). This section (§13) is the reference half of that workflow: the slide **inventory** (the distinctness check in §8 step 1 reads this table) and the per-slide **iteration records** (the evidence behind every §6 rule). Prompt-craft patterns are `§6`.
...
**Bitcoin Bull &amp; Bear Cycles slide added (July 2026).** Video landed — a weathered mooring post standing in dark water at dusk after a storm, successive swells each smaller than the last, an ember of light at the horizon; the post does not move. A warm-dark register, the sober sibling to the Stress Test slide. Silent, 720p, ~10s, 7.68 MB (raw Grok download stripped of its audio + mjpeg-thumbnail streams with `ffmpeg -c:v copy -an`; landed in-band, no re-encode). Wired the generic `video.carousel-video` way (`vid-bull-and-bear-cycles` / `/videos/bull-and-bear-cycles.mp4`), placed in the Models & Trends cluster immediately after the Doubling Ladder. Slide copy: label "Bitcoin Bull &amp; Bear Cycles"; headline *"Every storm so far has been smaller than the one before."*; CTA *"See the cycles →"*. **Shipped not-Featured (`data-feat="0"`)** — Bull & Bear is a Power-Law-family slide (Models & Trends, reads shared `power-law-data.js`), and the family already has its one Featured voice in Bitcoin & Metcalfe's Law; per the adjacency rule (only one family slide Featured at a time), featuring it would have meant demoting Metcalfe. JM's call (2026-07-23) was to keep Metcalfe Featured and ship Bull & Bear to the Numbers tab rather than churn the Featured rotation. Net Featured count unchanged at 13.

**The Bitcoin Hurdle Rate slide added (August 2026).** Inserted after **Bitcoin and Fixed Income** (the corporate-finance end of the Numbers cluster). Video (v4, falling light): a dark hard-edged escarpment over low rounded foreground hills — the two nearest hills fully sunlit at 0:00 and fully in shadow by 0:09 while the escarpment's upper strata stay lit, so the shadow edge visibly sweeps across the foreground. Slide copy: label "The Bitcoin Hurdle Rate"; headline *"Every use of capital is measured against something. Most bars are set far too low."*; CTA *"Clear the bar? →"*. Wired the generic `video.carousel-video` way (`vid-the-bitcoin-hurdle-rate`). **Shipped Featured (`data-feat="1"`) — The Bitcoin Migration demoted from Featured** (`data-feat="1"` → `data-feat="0"`; stays in the Arguments tab) to make room. JM's 2026-08-07 call: holds Featured at 13 rather than growing to 14, and shifts the balance toward The Numbers where the site's centre of gravity now sits; Migration makes the same argument in foundations register that the hurdle makes in allocator language. The one-Featured-per-family rule (which demoted the Doubling Ladder and Bull & Bear) does **not** bind here — the hurdle *uses* the Power Law but is a capital-decision tool, not a tight Power-Law-family slide, the same basis on which Fixed Income and Retirement are Featured. **Audio caveat:** the video still carries an audio track at wiring time; no `ffmpeg` in the authoring sandbox (the same condition that bit Metcalfe), so **the video is referenced but NOT committed** — pending an external silent strip + marker check before it lands. Net Featured count unchanged at 13.

**What Daily Conviction Bought slide added (August 2026).** Placed after the deployment trilogy (Wait, or Deploy Now?) — the same question answered *retrospectively* rather than prospectively. Video (first-take accept): an old drystone wall at low golden-hour sun, a climbing vine from a gnarled woody stump in the lower left occupying only the corner at 0:01 and spread across most of the wall with tendrils reaching the upper right by 0:08; stone stays visible behind the growth, not smothered. Continuous-process phrasing worked here where it failed on the hurdle slide — time-lapse plant growth is a familiar nature-footage prior, so no two-state restatement was needed (see §6). Slide copy: label "What Daily Conviction Bought"; headline *"Nothing clever. Nothing timed. Just every single day."*; CTA *"See what it bought →"*. Wired the generic `video.carousel-video` way (`vid-what-daily-conviction-bought`). **Shipped not-Featured (`data-feat="0"`)** — a personal-register retrospective adjacent to the Bitcoin Exit essay rather than the tools; the Featured rotation is carrying enough. **Audio caveat: same as the hurdle slide** — audio still attached, video referenced but NOT committed pending the external silent strip.
...
**Bitcoin as Collateral (§48) — SLIDE SHIPPED 2026-08-18, in the same branch as the page.** Not a follow-up PR after all: the video was generated and ffprobe-verified chat-side before hand-off, so the §8 content check was already satisfied and the slide landed with the page. Placed immediately **after Paper Bitcoin** — its named companion and its reading-order predecessor (the custody spectrum read from the holder's side, then the same spectrum read from the lender's). `data-cat="arguments"`, `data-feat="0"` — and that 0 is a **hold, not a demotion**: Featured status is JM's call and was undecided at ship, so the explicit 0 keeps it out of the Featured set until decided rather than letting an absent attribute imply the decision was made.

Copy: label "Bitcoin as Collateral"; headline *"The pledge you can see."* with *"Four thousand years of collateral – and the first pledge anyone can verify."* riding the `<em>`; CTA *"Read the exploration →"*. **The component has no sub slot** — `.insight-text` carries label / headline / CTA only — so the briefed sub line became the headline's em half, per the Risks-slide precedent where the em carries the payoff. Worth knowing before briefing any future slide with a three-part copy deck.
...
**The Bitcoin Floor (`/the-bitcoin-floor`, preview) — SLIDE SHIPPED 2026-08-22, in the same branch as the page.** ⚠️ The page itself is still **preview-only and carries a `noindex` guard**, and has no SITE_GUIDE page section yet — that, and the rest of the wiring, is the production pass. **This slide must not reach production before the page's noindex guard comes off**, or the homepage will link a Featured-eligible carousel slide at a page told not to be indexed. They ship together or not at all. Like Bitcoin as Collateral, not a follow-up PR: the video was generated and verified before hand-off, so the slide landed with the page rather than trailing it. Placed **immediately after Bitcoin and The Power Law** — its parent slide, and the reason for the placement is the reading: the floor is the lower edge of that same channel, so the two belong adjacent and in that order (the model, then the line it is judged against). `data-cat="numbers"`, `data-feat="0"` — the same **hold, not demotion** convention the collateral slide established: Featured status is JM's call, and an explicit `0` keeps the slide out of the Featured set until that call is made rather than letting an absent attribute imply it was.

Copy: label *"The Bitcoin Floor"*; headline *"The floor price has held for fifteen years — watched live, every day."* with the second clause riding the `<em>`, per the component's label / headline / CTA structure (there is still no sub slot); CTA *"See the record →"* rather than the default "Read more" — the page's payload is a record and a live status, not an argument to read.

Video (`/videos/the-bitcoin-floor.mp4`, 4,685,376 bytes, 1280×720, 10.04s, H.264): **one video stream, zero audio streams, `+faststart`** — verified from the container itself (`moov` at byte 32, ahead of `mdat` at 4811; the only handler types present are `vide` and the `mdir` metadata handler, with no `soun` track). Silent at source; the MJPEG stream was removed and the file re-encoded at CRF 23 chat-side before commit. Wired the generic `video.carousel-video` way, `id="vid-the-bitcoin-floor"`, no per-id JS registration — `index.js` selects `video.carousel-video` within the active slide, so there is no list to add to.

**Compare Retirement Plans (§52) — SHIPPED 2026-08-26; promoted out of this section to inventory row 34.** The page shipped without a slide by design and the brief sat here until footage existed, which is exactly the fence working: the entry could not become a table row while no slide was in `src/index.njk`, so the 42-for-42 re-derivation stayed true for the day it stood alone. The footage cleared the gate and the row now lives in the inventory table, which is re-derived to **43 rows for 43 slides**. The brief, the gate numbers and the custody check are recorded on the row and in its iteration record below.
...
Every slide video is briefed against one default register: **warm, hopeful, constructive.** Naming the vocabulary that produces it matters, because a brief should reach for these before it reaches for mood adjectives:

- **Movement with direction** — travel *toward* something, not motion for its own sake. A vine crossing a wall, a ship holding its line to the horizon, water finding a channel.
- **Structure and predictability made visible** — steps seen ahead, ordered fields, controlled flow. This site's subject is a channel; the footage should look like something that has a shape.
- **Light as destination and hope** — the frame moves toward light, or light arrives in it.
- **Natural settings, warmth dominating the frame.** Meadow, forest, shore, stone. Warmth is the majority of the picture, not a highlight inside a dark one.

**What "sci-fi" means here — and what it does not.** The register this site calls sci-fi is *one engineered-luminous element set against nature*. Discount, or Premium? (row 15) is the canonical case: a single lit trend-line wall, and nothing else in the frame glowing. §6 carries the full grammar. It is not neon, not cityscapes, not the Tron/synthwave palette, and not creepy — one glowing element doing real semantic work is the whole of it.

**Register follows the page's argument, not one site-wide mood.** Gravity and ominousness are permitted — and wanted — where the page's subject is risk. The Bitcoin Retirement Stress Test (row 22) is the standing precedent: its bridge in flood is foreboding by design, that foreboding is earned by the subject, and the slide is kept for exactly that reason. What the canon rules out is a dark register on a page whose argument is hopeful.

**Standing acceptance gate — every future slide.** Measured chat-side per `NEW_PAGE_CHECKLIST §8` (the sandbox that actually has `ffmpeg`/`ffprobe`), *before* the slide is wired:

1. **Brightness floor** — mean luminance stays at or above roughly **25%** for the whole clip.
...
- **The Bitcoin Retirement Stress Test (row 22) — offender → KEEP.** Reversed on review of the live slide: the foreboding is earned by the subject (navigating risk), and the bridge reads as safe passage rather than as threat. Replacement brief withdrawn. This is the ruling that produced the register-follows-the-argument rule above.
...
| 3 | Bitcoin as Collateral | **1** | The pledge you can see. *Four thousand years of collateral – and the first pledge anyone can verify.* | Extreme macro in candlelit dark: a wax seal pressed onto parchment, and as the stamp lifts the impression glows from within, ember orange, still molten. First of three briefed takes (seal / boundary stone / vault-turned-glass), v1 accepted. Two deliberate calls that look like defects: the seal **never cools** — a cooling seal reads as one-time attestation, a molten one as continuous verification, which is the page’s actual claim; and v2 was rejected *despite* rendering the briefed lattice-spread, because it overshot into engulfing fire and lost the overlay dark zone. The spread therefore rides the headline, not the footage — the same division of labour as the Risks slide (row 4), whose antifragility beat also never rendered. |
...
| 5 | What Money Has To Be | **1** | One money must do three things at once — *or it cannot do any of them.* | Three ornate brass lenses on oak workbench, candle behind; each shows same flame |
| 6 | Bitcoin vs. Real Estate | — | Housing became the default store of value *by elimination, not merit* | A house before dawn with a single lit window; the sunrise travels across the field toward it and the window stays warm the whole way — the closing frame is the brightest in the clip. **Replaced 2026-08-26 — gate CLOSED.** The retired take (golden hour → twilight → dark silhouette) opened at 41.9% mean luminance and fell to 14.8%, its one window light coming on mid-clip and going out again by the close. The replacement clears all three gate items: floor **39.5%**, rising arc, closing frame brightest. |
| 7 | Bitcoin vs. Rental Property | — | What if the yield were higher — *and the weather quieter.* | Wide cinematic shot of a sweeping field of tall grass at the moment a storm passes. First ~4 seconds: heavy rain falls diagonally, wind bends the grass in waves, slate-gray storm clouds churn low overhead, distant lightning hinted. The clouds then break in the upper third of the frame — golden god-rays pierce through, illuminating the wet field in patches, mist rising off the warm ground. Rain tapers; camera slowly pushes in. By the end, the storm has moved off to the right edge, warm dawn light bathes the field, droplets on grass catch the gold. Same field, different sky — storm reads as the landlord burden (operational load, gross-to-net leakage, policy risk), sun reads as bitcoin's quieter yield. v2 landed warmer/more natural than v1 on the close. |
| 8 | Bitcoin and Fixed Income | **1** | High yield, *without bitcoin's volatility*. | Autumn-forest stream cascade — a regular yield against an unchanging structure. |
| 9 | The Bitcoin Hurdle Rate | **1** | Every use of capital is measured against something. *Most bars are set far too low.* | Falling light on a dark hard-edged escarpment over low foreground hills — the two nearest hills sunlit at 0:00, fully in shadow by 0:09, the shadow edge sweeping across the foreground while the escarpment's upper strata stay lit (v4; the named-two-states device, §6). **Featured**, retiring The Bitcoin Migration (JM 2026-08-07). **Video committed and silent** — binary-marker check 2026-08-26 finds `vide` handlers only, no `soun`/`mp4a`. **Markup position:** after Bitcoin and Fixed Income, where this row now sits (the 2026-08-26 re-derivation). See iteration record below. |
| 10 | Bitcoin and The Power Law | 0 | For every 13% increase in Bitcoin's age, *the trend price doubles.* | A lit stone village on a canal at blue hour; the whole scene warms to gold across the clip and the lights never fade. **Replaced 2026-08-26 — gate CLOSED.** The metaphor is unchanged from the retired take — a town's lawful illumination pattern, a network compounding — and only the execution moved: the old footage never exceeded 18.8% mean luminance on a falling arc. The replacement clears all three gate items: floor **34.2%**, rising arc, closing frame brightest. |
| 11 | The Bitcoin Floor | 0 | The floor price has held for fifteen years — *watched live, every day.* | A dry-stone wall at golden hour, shot so the lowest course reads as a *course*: laid level and square, carrying rougher, more jagged stones above it. That asymmetry is the floor-vs-upper-band claim rendered without a chart. All the motion in the frame is light and grass; the stones never move — *time passes, the wall doesn’t*, which is the thesis. First-take accept; a v2 with a heavier sun arc was generated and declined for reading as time-lapse *effect* rather than as the thing it depicts. Placed immediately after Bitcoin and The Power Law — the floor is the lower edge of that same channel, so the model comes first and the line it is judged against second. `data-feat="0"` is a hold, not a demotion. ⚠️ The page is still preview-only behind a `noindex` guard; slide and page reach production together or not at all. See iteration record below. |
| 12 | Bitcoin & Metcalfe's Law | **1** | The network is still there. *We just can't see it anymore.* | A single off-center bare tree on a still lakeshore at golden hour; the real tree and landscape stay crisp and sharp throughout while its mirror reflection in the water progressively blurs and breaks into unreadable ripples — the network persists while the on-chain reflection of it goes blind. (v2; see iteration record.) Silent — the PR-time audio track was stripped 2026-06-20 in `6c7b500` (resolved, not forgotten). _(Table drift note: the Doubling Ladder slide, shipped June 2026, is not separately tabled; `src/index.njk` is the source of truth for absolute order.)_ |
...
| 14 | Bitcoin Bull &amp; Bear Cycles | 0 | Every storm so far has been smaller than *the one before.* | Weathered mooring post in dark water at dusk after a storm; successive swells each smaller; ember light at the horizon; the post stands — warm-dark register, a sober sibling to the Stress Test slide. Silent, 720p, ~10s, 7.68 MB (audio + mjpeg-thumbnail stripped; no re-encode). **Not Featured (feat=0):** the Power-Law family holds one Featured slide (Metcalfe); shipped feat=0 by JM's call (2026-07-23) rather than demoting Metcalfe. Placed in the Models & Trends cluster, after the Doubling Ladder. |
| 15 | Discount, or Premium? | 0 | Bitcoin runs above and below one enduring trend — this page tells you, live, *which is true right now.* | Engineered-luminous register (§6) — one of the two founding entries: a lit trend-line wall in monumental dark with the price riding above and below it, the dark sky/water band left clear to carry the overlay text. Structural-observation register, following the Bull &amp; Bear Cycles idiom. Silent looping `/videos/discount-or-premium.mp4`. **Numbers only, not Featured** — the Power-Law family holds one Featured slide (Metcalfe). Full slide record in `DISCOUNT_OR_PREMIUM_DESIGN.md`. |
| 16 | The STRC Mechanism | 0 | Built to hold a line — *trading beneath it.* | Engineered-luminous register (§6): a night masonry lock-chamber, one horizontal amber light-line, moon, rising water — stone arches stand above the waterline at the start and are half-swallowed by the end (the object-state-change device). **Numbers only, not Featured** (JM 2026-07-28 prominence decision). Silent, 720p, ~10s, 4.17 MB (audio + thumbnail streams stripped, +faststart). **Markup position:** immediately after the Discount, or Premium? slide, mid-Numbers cluster — and as of the 2026-08-26 re-derivation this row finally sits there. Tabled until then under its former name *STRC Below Par*. See iteration record below. |
| 17 | Lump Sum or Ladder In? | **1** | Deploy it all, or ladder in? *History sorts it out.* | Deployment trilogy P1. Firelit two-glass pour on a wooden table before a fireplace — a bold, full pour into one glass beside a thin, measured pour into the other; the lump-vs-ladder decision as two ways of filling the same vessel. Silent, 720p, ~10s. Grouped with P2/P3 as a tonal arc, in reading order, adjacent to the Doubling Ladder slide. |
| 18 | Your Bitcoin Deployment Plan | **1** | Your capital, your timeline — *modeled against the channel.* | Deployment trilogy P2. Aerial golden-hour flight over a river valley: a smaller tributary joining the main river as both wind toward the sun on the horizon — personal capital converging on a plan. Silent, 720p, ~10s (2-pass re-encode to land in the 3–10 MB band; the raw was ~11 MB). |
| 19 | Wait, or Deploy Now? | **1** | Wait for a lower entry, or deploy now? *The channel tells you when.* | Deployment trilogy P3. Sun emerging from behind a low cloud bank over a calm sea, rays breaking across the water — patience resolving into clarity; echoes the page's quiet-when-low, live-when-high spine. Silent, 720p, ~10s. |
| 20 | What Daily Conviction Bought | 0 | Nothing clever. Nothing timed. *Just every single day.* | Drystone wall at golden hour; a climbing vine from a lower-left woody stump spreads across the wall to the upper right by 0:08, stone visible behind, not smothered (first-take; continuous-growth phrasing worked — familiar nature-footage prior). **Not Featured** — personal-register retrospective, adjacent to the Bitcoin Exit essay. **Video committed and silent** — binary-marker check 2026-08-26 finds `vide` handlers only, no `soun`/`mp4a`. **Markup position:** after the deployment trilogy (Wait, or Deploy Now?), where this row now sits (the 2026-08-26 re-derivation). See iteration record below. |
| 21 | Bitcoin Portfolio Allocation | **1** | What a small position does to the whole — the growth, the drawdown, and the drift. *See it, cost included.* | Amber ink bloom suspended in dark water — a small position quietly transforming the whole (suspended particles, light filtering from the surface). Reads as invitation; first of the decision→consequence pair. Silent, 720p, ~10s, 2.24 MB. v2 landed after a quantified-size-budget rewrite (see iteration record). |
| 22 | The Bitcoin Retirement Stress Test | **1** | The same plan, with the bear market included. Early is brutal, late is nearly harmless — *find out which is yours.* | Stone bridge spanning a river in flood at dusk — the whole plan under load, ominous and foreboding by design (the sober-sibling register of The Bitcoin Retirement). Reads as examination; second of the pair. Silent, 720p, ~10s, 6.26 MB. First-take accept (architectural single-subject = weak prior; see iteration record). **KEEP — register audit 2026-08-26.** Provisionally rated an offender, then reversed on JM's review of the live slide: the foreboding is earned by the subject (navigating risk), and the bridge itself reads as safe passage. The replacement brief is withdrawn; this row is the precedent for the rule that gravity is permitted where the page argues about risk. |
| 23 | The Bitcoin Horizon | — | What looks like turbulence over the course of a day is *trajectory over the course of a decade.* | Small ship at golden hour, crossing toward the horizon; near-shore chop, steady distant line; persistence-through-turbulence |
| 24 | Is Bitcoin a Bubble? | — | A thing isn't a bubble simply because *it has been called one.* | ~10 bubbles burst; one amber object remains impervious |
| 25 | The Half-Life | — | How long until your money *loses half its value?* | Candle burning down asymptotically |
| 26 | The Melting Ice Cube | — | Holding cash is not safety. It's an *active decision with an ongoing cost.* | Ice cube melts to fragment on weathered wood |
| 27 | The Bitcoin Synthesis | — | Six components. *One irreducible synthesis.* | Six antique keys assemble into clockwork mechanism |
| 28 | What Bitcoin Is | — | Most people consider just a few dimensions. Bitcoin is *all of them, simultaneously* | Raw mineral crystal, traveling amber light |
| 29 | The Bitcoin Migration | 0 | From what can't be fixed, *to what can't be broken.* | Heavy wooden door opens ~2/3, amber light. **Demoted from Featured 2026-08-07** (feat 1→0) to promote The Bitcoin Hurdle Rate — holds Featured at 13 and shifts balance to The Numbers; stays in the Arguments tab. |
| 30 | The Trilemma | — | Some problems are *navigated, not solved.* | Three geometric counterweights in coupled sway |
| 31 | The Money Trees | — | Two systems. *Different roots. One outcome.* | Two trees; left withers, right unchanged |
| 32 | The Bitcoin Retirement | **1** | A stack that lasts a lifetime — *or runs out before you do.* | Solitary mature tree in golden-hour meadow; sun moves from behind canopy to streaming rays through it; latent-state opening, persistence across the arc |
| 33 | Bitcoin Escape Velocity | 0 | A retirement is supposed to run down. *There’s a line past which yours doesn’t.* | Four countable stepping stones in shallow water, the water lapping over them one after another — discrete countable objects standing in for a continuous level, which is the lock-chamber object-state-change lesson extended (and why this slide cost three briefs: carved band → two-tone stain → stepping stones). The water *breathes* over the stones rather than rising monotonically; JM accepted that as on-thesis, since the page's subject is a threshold's sensitivity — how little it takes to cross, and how easily you fall back. Placed immediately after The Bitcoin Retirement, `data-feat="0"` per the §13 adjacency rule. See iteration record below. |
| 34 | Compare Bitcoin Retirement Plans | 0 | Two complete plans, side by side — *and what the difference actually buys.* | A worn track forking at a stone cairn in a golden-hour meadow; both branches run away from camera toward a bright horizon, one climbing gently along a ridge, the other holding the flat ground through longer grass. **Both branches stay equally lit in every frame** — neither path is styled as the wrong turn, which is the one thing the brief held the line on, because the page refuses to crown a winner and the slide must not contradict it. Light *rises* across the ten seconds: the far end of both tracks brightens and the closing frame is the brightest in the clip. Nothing moves but light and grass; no text, no people. **Gate PASSED chat-side 2026-08-26, all three criteria hard:** floor **50.6%** mean luminance (double the ~25% requirement, and the brightest clip in the set), **strictly monotonic** rise, closing frame **55.0%** against a **50.6%** opening. **Custody:** the placed file's SHA-1 was verified against the gated encode before wiring (`4cd5a4f…`, 5,972,363 bytes) — the gate certifies a specific encode, so the wiring step checks it is that encode and not merely a file with the right name. **The chain is closed end to end:** gated → placed → committed → served on preview → **served from production** (all five byte-identical, SHA-1 `4cd5a4fdaa2a778a3323fb811ca5df4d834b0c5c`, 5,972,363 bytes, `Content-Type: video/mp4`) → **playing**, confirmed observationally by JM on the preview. Placed immediately after Bitcoin Escape Velocity, which is the funnel's own order: size one plan, then weigh two. See iteration record below. |
| 35 | Disciplined Rebalancing | — | A model you believe in is *a protocol you can run.* | Wheat field at golden hour, ears swaying in coupled bounded oscillation; fixed camera, stable disequilibrium |
| 36 | The Fixed Pie | 0 | Your share remains undiluted — *for eternity.* | Unmarked gold coin; surrounding coins dissolve |
| 37 | Bitcoin vs. The Stock Market | **1** | Three growth curves. *One decisive horizon.* | Three trees in a meadow at golden hour; the middle tree grows dramatically taller and fuller-canopied over 10 seconds while the flanking trees mature modestly; latent-state opening, divergent outcomes from identical conditions |
| 38 | The Bitcoin Heatmap | — | The pattern reveals itself *only when you zoom out.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 39 | Borrowing Against Your Stack | — | Every coin sold today is a coin that *compounds for someone else.* | Solitary ancient standing stone — weathered, lichen-marked, ~six feet tall — in a golden-hour meadow; tall grasses bend in gentle wind, long shadow extends from the stone across the foreground; the stone itself does not move; quiet persistence as the visual argument |
| 40 | Bitcoin-Backed Mortgages | — | The home opens. *The stack stays.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 41 | Living on Bitcoin | — | Save in bitcoin. Spend in fiat. *The float holds.* | (concept retrospectively documented — see live markup `src/index.njk` for the as-shipped video) |
| 42 | Bitcoin Defined | — | Define it. *Then debate it.* | Bed of glowing embers in deep darkness — same composition as Card 7 (Bounded by Energy) on the page. Camera fixed throughout. Four-layer subtle motion: heat shimmer above the bed, asymmetric ember pulse (no two embers in sync), a small natural wood-fire flame at center-left with irregular asymmetric tongues whose base merges with the surrounding embers, wisps of pale smoke rising and dissipating. Seamlessly looping 10-second silent video. |
...
### Iteration record — slide 34, Compare Bitcoin Retirement Plans (August 2026)

**The first slide briefed against the register canon and gated on measured luminance rather than on eyeball alone**, so it is worth recording as the worked example of that process end to end.

**The brief, and the one thing it held the line on.** The page's argument is *two complete options from one starting point*, so the metaphor is a **fork, not a race**: a worn track splitting at a stone cairn in a golden-hour meadow, both branches running toward a bright horizon. The constraint that mattered more than any compositional detail was that **neither branch may be styled as the wrong turn** — no darkening, no thorns, no dead end. The page refuses to crown a winner, and a slide that quietly picked one would contradict the thing it is advertising. Everything else in the brief was negotiable; that was not.

**Prediction and outcome.** The §6 prior-strength rule flags "a track forks and both branches stay good" as a *metaphor* brief and predicts a revision, with the specific failure being the model resolving the fork into one dominant path or shading one branch to signal the choice. The documented fallback was ready — name **two object states** ("the left track's far stones fully sunlit at 0:00, the right track's by 0:09") instead of an abstraction about balance. **It was not needed.** The take landed without the revision the rule predicted, which is a data point against the rule rather than for it: the useful reading is that a metaphor brief with **one hard, concretely-stated prohibition** behaves more like a weak-prior brief than the rule's category alone suggests.

**The gate, measured chat-side per `NEW_PAGE_CHECKLIST §8`.** All three criteria hard — the page's subject is a choice, not a risk, so the Stress Test exemption did not apply:

| Criterion | Requirement | Measured |
|---|---|---|
| Brightness floor | ≥ ~25% mean luminance | **50.6%** — double, and the brightest clip in the set |
| Arc | No sustained falling arc | **Strictly monotonic** rise |
| Close vs open | Closing frame ≥ opening | **55.0%** vs **50.6%** |

Visual confirmation alongside the numbers: both branches equally lit in every frame, the fork never resolving into a "right" path, no text and no people.

**Custody — the step the gate implies but does not itself perform.** A gate result certifies **a specific encode**, not a filename. So the wiring step verified the placed file's **SHA-1 against the gated encode** (`4cd5a4fdaa2a778a3323fb811ca5df4d834b0c5c`, 5,972,363 bytes) before anything was committed, and the file was deliberately **left untracked and unwired** on first discovery, because a file appearing on disk is not evidence that it passed anything. **Make this the pattern:** the gate says *this encode passed*; the wiring PR says *this is that encode*; the post-merge check says *this is what production serves*. Without those the gate result is a claim about a file nobody can identify later. The full chain for this slide — gated → placed → committed → served on preview → served from production → playing — is recorded on row 34, and the last link is the one no automated check here could close: the pane runs hidden, so playback was confirmed by JM on the preview rather than asserted from a `readyState`.


**Hashing an image from the drafting side — take the digest AFTER presenting it (added 2026-09-07).** The custody rule above assumes the hash identifies the bytes that arrive. For **images handed over from the drafting chat that is not automatically true: presentation re-encodes them**, so a digest taken *before* presenting names a file that no longer exists by the time it reaches Downloads, and the wiring step fails a check that was never going to pass. It cost a round trip on `og-the-rundown-v2.jpg`: the quoted SHA matched nothing on disk under MD5, SHA-1 **or** SHA-256, the card was correct in every other respect, and the bytes were held — correctly — until the drafting side re-hashed after presentation and the new digest matched. **Two ways to avoid it, both cheap:** take the hash **after** presenting, or **ship the file inside a zip**, which is passed through unmodified. **What does NOT change: a mismatch still stops the commit.** The failure mode this guards against — a file appearing on disk being taken as evidence it passed something — is exactly the same whether the cause is a swapped asset or a re-encode, and "it looks right" is not a custody check. Verify, then commit; if it fails, ask rather than judging by eye.

**Hashing production HTML — undo Cloudflare's email obfuscation first (added 2026-09-21).** A production page's hash varies per request: Cloudflare rewrites the footer `tips@` address as an `<a class="__cf_email__" data-cfemail="…">` with a per-request key and injects its `/cdn-cgi/scripts/…/email-decode.min.js` decode script. Every page, because the footer lives in the shared layout; `pages.dev` previews carry no obfuscation, so a preview hash and a production hash of the same build never match. The stable figure for a handback is the hash with those two insertions undone (restore `tips@lastcoinstanding.com`, drop the script tag), which then matches the local `_site` build once line endings are LF-normalised. Instance: `/` on 2026-09-21 — raw `f636bd43…` (varied on re-fetch), stable `6d9aa751…` = local build.
**Container check, as usual:** one video stream, zero audio streams (no `soun`/`mp4a`), no mjpeg thumbnail stream, `moov` at byte 36 ahead of `mdat` — silent and `+faststart` at hand-off, nothing to strip.

**Placement.** Immediately after Bitcoin Escape Velocity, which is the retirement funnel's own order — size one plan, then weigh two — and the same adjacency logic the family's other slides follow. `data-feat="0"`: The Bitcoin Retirement keeps the family's Featured voice.

### Iteration record — slide 9, The Bitcoin Hurdle Rate (August 2026)

Four takes; landed by the §6 "name two object states, not a continuous quantity" rule — which this slide, as the third case to hit the limit, prompted.
- **v1, receding ridgelines:** the brief asked for successive ridges each lower in frame than the last, so distance itself dropped the bar. Atmospheric perspective defeated it — the far ridges rendered pale and soft, which the eye reads as small, cancelling the scale contrast. Nothing moved but haze.
- **v2, haze reveal:** a vast ridge was to emerge from haze behind low hills. It was already fully visible at frame one, so the reveal never happened; 0:00 and 0:08 are near-identical.
- **v3, falling light:** composition landed — a dark, hard-edged layered escarpment dominating low rounded foreground hills — but Grok rendered the cause without the effect: the light quality changed (the face went redder, strata caught more edge light) while the shadow boundary stayed put. Same failure as STRC v2, where animating the inflow left the water level static.
- **v4, accepted:** fixed by naming two object states rather than a process — the two nearest foreground hills fully sunlit at 0:00 and fully in shadow by 0:09, while the escarpment's upper strata stay lit throughout; the shadow edge visibly sweeps across the foreground. Reads ominous, in-family with the Retirement Stress Test's sober-sibling register and appropriate to a page about a bar most capital fails to clear.
- **Editorial note:** the brief was rewritten between v1 and v2 after JM corrected the page's central claim. The declining hurdle is a precision refinement, not the finding — the hurdle is roughly double any realistic comparison at every horizon, so the decline never changes the answer. v1 was a competent render of the *subordinate* claim, which is why it read as inert. The lesson generalised into §6 (name two states).

### Iteration record — slide 20, What Daily Conviction Bought (August 2026)

First-take accept. An old drystone wall at low golden-hour sun with a climbing vine extending from a gnarled woody stump in the lower left; at 0:01 the vine occupies only the lower-left corner, by 0:08 it has spread across most of the wall with tendrils reaching the upper right; stone stays visible behind the growth throughout, not smothered. **Continuous-process phrasing worked here where it failed on the hurdle slide** — time-lapse plant growth is a familiar nature-footage pattern in the model's priors, so it needed no two-state restatement. (The §6 rule's boundary condition: process phrasing is safe precisely when the process is a strong prior.) **Unbriefed choices kept per §6:** russet and amber leaves rather than the briefed gold-green — reads as Virginia creeper in autumn and sits better in the site's palette than what was asked for; and a diagonally receding wall rather than one facing camera, giving the growth somewhere to travel. The old woody stump at the base does quiet argumentative work: it says the vine has been at this for years.

### Iteration record — slide 16, The STRC Mechanism (July 2026)

The STRC slide took three video briefs to land, and the lesson generalizes to any *engineered-luminous* subject (§6) where a quantity has to be seen changing in monumental dark architecture.

- **v1 — the static void.** A night lock-chamber described with adjective-heavy motion ("water slowly rising," "brooding," "tense") plus a quantified ruler (wall-course markers meant to imply a level). It rendered as a near-black still: dark-on-dark with no visible actor. Lesson: **a quantified ruler cannot carry motion if the architecture isn't visibly *doing* anything** — the viewer can't read a level against masonry they can barely see, and adjectives don't move water.
- **v2 — legible but frozen.** Adding visible inlet streams (water pouring in through side channels) and moonlit key-light fixed the identity and gave the scene life — you could finally read it as a filling lock chamber. But the water *level* still wouldn't move; Grok animated the streams and surface texture while leaving the fill height static. Lesson: rendering the *cause* (inflow) doesn't guarantee the *effect* (a rising level).
- **v3 — object-state-change landed it.** The fix was to tie the rise to a **discrete object state-change** rather than a continuous quantity: stone arches stand clearly *above* the waterline at the start and are *half-swallowed* by the end. That gives the model two nameable keyframes to interpolate between, and it moved the water. Naming the v1 failure explicitly in the **Avoid** block ("do not render a dark static scene; the water level must visibly change") also helped. And **Grok's unprompted moon was kept** — per the "ask what it's arguing" rule (§6), the moon is the external force that moves water (tides/the cycle), so it earns its place rather than being prompted away.

### Iteration record — slide 37, Bitcoin vs. The Stock Market (May 2026)
...
The trees direction has light visual overlap with two other tree-themed slides (slide 31 The Money Trees / two trees, slide 32 The Bitcoin Retirement / one tree). Adjacency is broken by the four slides that sit between those and BvSM at 37 — Bitcoin Escape Velocity (33, stepping stones), Compare Retirement Plans (34, forking track), Disciplined Rebalancing (35, wheat field) and The Fixed Pie (36, gold coin); framings differ enough (centered single vs wide three) that the shared visual vocabulary reinforces site identity rather than producing monotony — the carousel's overall grammar is part of what makes the set feel like one artifact.

### Iteration record — slide 42, Bitcoin Defined (May 2026)
...
### Iteration record — slide 43, What Money Is For (May 2026)
...
The first brief asked for amber ink blooming in water, but hit the **strong-visual-prior problem**: the ink-in-water archetype rendered as a dramatic **radial starburst / explosion in a void** — a shockwave, not the slow suspended bloom the slide wanted (a small position quietly transforming the whole). "Ink in water" carries an explosion prior the way "candle" carries a teardrop-flame prior (slide 34) and "storm" carries a chaotic-sea prior (Risks to Bitcoin) — the adjective "gradual" gets outvoted by the archetype.
...
### Iteration record — The Bitcoin Floor (August 2026)

**Two generations, take one accepted.** The brief was a **foundation course**: the lowest course of a dry-stone wall, laid level and square, carrying rougher stones above it. It rendered on the first take, which is worth recording precisely because it is rare in this section — most slides here took three.

**JM's accepted reading, and it is the reason the take works.** The **structured lower course against the jagged upper stones is the floor-vs-upper-band asymmetry**, rendered without a chart. That asymmetry is the page's own opening claim in prose — the floor is a line price has held with remarkable consistency, while the upper edge is volatile, with brief violent spikes — and the footage states it in stone before the reader has read a word of it. Nothing about that was briefed as an analogy; it was read out of the take afterwards, which is the good direction for this to run in.

**Where the motion lives is the second half of the reading.** All movement in the frame is carried by **light and grass**; the stones never move. *Time passes, the wall doesn't* — that is the thesis, and it is the whole argument for using a foundation course rather than, say, a rising level. Every earlier slide in this section that needed to show a **quantity** wrestled with animating change (see the Escape Velocity record above, and the lock chamber before it). This slide needed the opposite: to show **something not changing while time visibly does**, which the model renders easily and reliably. Brief that shape directly when the subject is persistence rather than transition.

**v2 — a stronger light arc — was generated and declined.** The second generation pushed the sun's travel harder to make the passage of time unmistakable. It read as more obviously animated and less like the thing it depicts: a heavier arc turns the frame into a time-lapse *effect*, where take one's subtler light lets the stillness of the stones stay the subject. Declined in favour of v1's subtlety. Recorded because "the more legible version was rejected" looks like an oversight in a diff and was a deliberate call — the same shape as the collateral slide's never-cooling seal, where the less literal option carried the claim better.
...
### Verify-the-math tables — display ruling (2026-09-20)

**Verify-the-math tables: every dollar column follows the nominal/real toggle — including BTC price (ruling reversed 2026-09-20).** The original rule ("BTC price is never deflated") left the audit tables' own identities (price × BTC left = stack value; income ÷ price = BTC sold) visibly false in real-dollars mode — caught by a reader (@TC_Blockchain, X, 2026-09-20) who did exactly what the table invites and found "the multiplication is off." Deflating price, value, and income by the same deflator preserves all three identities in both modes, so the price column now renders through `rtDollars()` like everything else, and the CSV price header is dynamic (nominal / today's $). Fixed on the flagship (`30199a0`) and Escape Velocity (`fd9a201`), where real is the default view. Any future page adopting the audit-table pattern inherits this rule: if a table teaches readers to check the math, every displayed column must multiply.
...
### Scenario comparison — "Compare — what if you…" (rt-compare)

Added July 2026. A side-by-side counterfactual panel (`.rt-compare`, `rt-`prefixed classes) sitting directly below the Sustainability readout on the Calculator tab. Reuses the page's existing projection engine with **zero new math** — each variant is a relative rule on the live base config (`{field, delta}`), recomputed through the same `projectStackOverTime` / `realStackAtRetirement` calls that draw the chart, on every slider input and chip toggle. In-memory UI state only: nothing is stored and, deliberately, nothing is written to the URL (the scenario-URL contract in §17.5 covers base sliders only).

- **Chips** (`.rt-cmp-chip`, `aria-pressed`): *Retired 2 years later · Retired 5 years later · Retired 2 years earlier · Retired 5 years earlier · Wanted 25% less ($…) · Wanted 25% more ($…) · Withdrew 4% (the traditional rule)*. Each is a single-lever rule (income OR retirement year — never both). The two income chips are **proportional (±25%)** as of rev2 (a flat ±$25K was a 6% nudge at a $410K target and more than the whole target at $20K); their labels resolve **live** to the current figure — `Wanted 25% more (${round5k(income×1.25)})` — updating as the base income slider moves. Tapping a chip adds a variant column; **max 2 active variants** (+ base = 3 columns); a third tap evicts the **oldest**; tapping an active chip removes it.
- **Availability — one principle, applied consistently** (`available(base)`): a rule is gated only when clamping it to a bound would **falsify its own label**. **Delta rules** (retire ±N years) state an absolute magnitude, so they stay bound-gated via `cmpRetireAvail` (`retirementYear [2026,2070]`); out of bounds ⇒ the chip renders **disabled/dimmed** (`aria-disabled="true"`, focusable) with a reason on hover (`title`), focus, and tap (the `#rtCmpReason` aria-live line, touch); no column, and an active variant a base drag invalidates is dropped (column + chip) that tick, never silently clamped. **Proportional and absolute rules** (income ±25%, the 4% rule) cannot falsify their label — "25% less" is exactly 25% less at any positive income — so they carry **no bound guards** (`cmpIncomeAvail` was deleted in rev2; multiplying a positive income can't reach zero, so no degeneracy guard either). The 4% chip keeps only its no-op guard (base already ~4%, `|0.04·stack − income| < 500`) and a zero-stack degeneracy guard.
- **Worked examples — keep these; they matter more than the principle.** Without the numbers a future session reads the asymmetry as an oversight and "harmonises" it. The rule: **disable when a rule produces a *nonsense* scenario; allow when it produces a *truthful* scenario that merely can't be dialed on a slider.** **Shown** (truthful, both land below the income slider's $20,000 floor and so are unreachable by dragging, yet each is a correct answer): 4% of a small stack resolves to **$592/yr**; 25% less of a $20,000 target resolves to **$15,000**. **Blocked** (nonsense): *negative income* — the old flat ±$25K rule at the floor gave $20,000 − $25,000 = **−$5,000**, a negative withdrawal that *grows* the stack and would falsely read escape velocity — and *zero stack*, where 4% × 0 = **$0** draws nothing and never depletes. The delta chips (±2/±5 years) stay bound-gated for the mirror reason: clamping "Retired 5 years earlier" to 1 year earlier would state a magnitude the column doesn't deliver.
- **Columns**: leftmost is *Your current scenario* (the live base), then variants. Each shows *Years stack lasts* (or "∞ — escape velocity"), **two stack-at-retirement rows** — *(today's $)* and *(YYYY dollars)* in the column's own retirement-year nominal, so the panel reconciles with the Sustainability card (which defaults to nominal) while the real row keeps cross-column comparison honest — and *Target income* (the today's-$ target the column draws; renamed from "Sustainable income" Aug 2026, which self-contradicted a depleting column whose *years stack lasts* is finite precisely because of that income). All compare figures are **trend-basis** (independent of the page's trend/current toggle). The 4%-rule column's income is `0.04 × its own real stack-at-retirement`, flagged *"at 4% withdrawal"*.
- **Delta row** (per variant): the top-two signed differences vs base, priority **escape-velocity flip > years-of-stack > income > stack-value**, with a plain "retire YYYY instead of YYYY" annotation as lowest-priority filler. The stack-value term reads from the **today's-$ row only — a cross-column delta is never computed from nominal figures.** An EV flip is the headline in both directions. Delta numbers use the site-standard pos/neg colors only (`#7fc47f` / `#c0603a`); the flip headline is amber. **No column tinting, no winner treatment** — the per-column dot colors are assigned by slot order, never by valence.
- **No chart overlay** (removed rev2 item 1): the panel sits well below the chart, so at tap time the payoff landed off-screen, and on a log axis nearby variants overplotted the base curve. The chart keeps its **six** datasets; chart-copy is unaffected. Columns/active chips carry a plain **color dot** (`.rt-cmp-dot`, slot color) purely to tell columns apart.
- **Engine reuse / perf**: `cmpComputeAll()` runs once per render tick, reusing `renderChart`'s already-computed base drawdown projection and computing each active variant exactly once — ≤ 2 added engine runs with 2 variants. Both stack rows come from one `computeStackAtRetirement` per column (real = nominal ÷ the shared `rtDeflator`; no new deflator). Chip availability is cheap arithmetic on the base fields (the 4% chip's guard adds one `computeStackAtRetirement`).
- **Non-degeneracy**: every rendered variant column differs from base in ≥ 1 of the three outputs, and no two active columns are identical — guaranteed by single-lever rules + the availability guard (an identical column would be a bug or a chip that should have been unavailable).
- **Verified** (branch `rt-compare-fixes`, local stitch-harness): exact-equality gate byte-exact including **at the bounds** (income at $20K and $1M, retirement at 2026 and 2070), for the 4% chip from a `fixed`-basis base (confirming it no longer switches basis), and for the 4% chip at a **sub-floor small-stack value** ($592/yr, reproduced with a temporary income step/min relaxation since the truthful 4% figure sits below the $20K UI floor); the 4% chip stays available in early-retirement / small-stack cases (`formatCurrencyShort` renders a few-hundred-$/yr income cleanly as e.g. `$592`); availability disables the right delta chips at every bound with the right reasons; an active variant auto-drops (column + chip) when a base drag invalidates it; non-degeneracy (no variant column equals base, no two active columns identical); EV flip both directions; chip cap / oldest-eviction / toggle-off; 375px base-first vertical stack with 7 chips wrapping and zero horizontal overflow; disabled-chip style (opacity 0.4, dashed, `title` reason) and reason-on-tap confirmed; no console errors through rapid churn across bounds; FAQ visible text + FAQPage JSON-LD untouched.
...
6c. **The Bitcoin Hurdle Rate** — what the trend *implies for a decision*, once the reader can place price against it. Converts bitcoin's trend growth into the opportunity-cost benchmark a CFO or allocator already uses, matched to the horizon of the decision (and declining with it). Reads after Discount/Premium (which supplies the trend-vs-price unit) and before the applied sizing/deployment pages that assume a decision has already been framed; the one page addressed directly to the corporate-finance reader, so it can also be entered cold from that audience.
...
9a. **STRC Below Par** (§42, shipped July 2026) — the episodic sub-page of Bitcoin Fixed Income. Reads immediately after it and never before it: it assumes the reader already knows what STRC is and how the preferred stack works, then examines what one instrument's below-par price is saying. The most timing-adjacent and most single-security page on the site, so it is placed deliberately last in the Numbers tier; its own two-sided, verdict-free construction is what keeps it from reading as a signal.

**The Bitcoin Dashboard** (§47, shipped 2026-08-10) sits **outside** this newcomer reading sequence by design: it is the **return-visit** surface, not a pedagogical step. A first-time reader is routed through the pages above; the dashboard is where a returning reader lands to re-check where bitcoin sits in the channel, and it links back *into* this sequence (its jump-back-in row deep-links the Power Law channel, Wait-or-Deploy, How Much Cash, Allocation, Retirement, and the Stress Test). Place it conceptually after a reader already grasps the channel — it assumes that vocabulary rather than teaching it.

**Bitcoin as Collateral** (§48, shipped 2026-08-18) reads **after the financialization cluster's existing pages and before The STRC Mechanism in specialization depth**. It is the general case those pages are instances of — Borrowing Against Your Stack is one borrower's loan, Bitcoin-Backed Mortgages is one loan type, STRC is one company's securities — so it earns them rather than the reverse, and a reader who arrives at it cold still gets a complete argument. It sits *before* STRC because STRC assumes a specific instrument and a specific quarter, where this page assumes only that the reader knows what a loan is. Its natural predecessor is Paper Bitcoin: the custody spectrum read from the holder's side, then the same spectrum read from the lender's.
...
- **Carousel slide (slide 37) — shipped May 2026.** Trees-in-meadow video landed in the carousel; see `§13` inventory and iteration record.
...
A grid where rows are holding-period horizons (6mo / 1y / 2y / 3y / 4y / 5y / 7y / 10y, longest at top) and columns are entry months from 2010 to today. Each cell's color encodes BTC's outperformance multiple over the comparator (S&P 500 TR or NASDAQ-100 TR) for that (entry, horizon) window. The eye reads the staircase: dense bright orange across the top-left (long horizons + early entries = always wins by huge multiples), gradient through golden yellow to pale yellow as horizons shorten and entries get more recent, scattered red where short-horizon entries near cyclical tops haven't recovered yet. The page's takeaway lives in the pattern strip above the heatmap: at 7y and 10y horizons, bitcoin outperformed in 100% of cases (116 of 116 and 80 of 80 windows on data through 2026-08); at 7y, median outperformance is 38× (the mean, 1,738×, is dominated by 2010–2012 entries and is not the stated figure); at 1y, bitcoin won in 74% of cases (pre-correction reading; re-read from the live strip) — *short-horizon entry timing matters; long-horizon entry timing doesn't*. Before 2026-09-16 this line read "100% at 4y+ (149 of 149)" and "164.5× average at 7y"; the first was false at five years on both the old and the real data (7 losses of 140 at 60 months, all 2021-top entries) and the second reproduced under no definition. The strip itself computes live from the arrays (`renderSidebar` in `bvsm.js`); these are the baseline for the OG card and meta tags, which are static and must be re-derived from the strip at each regeneration.
...
- **Homepage carousel slide** shipped May 2026. 10-second silent embers video animated from Card 7's still composition; caption "Define it. Then debate it." Slide is row 42 in the carousel inventory. Iteration record (candle-simile failure → wood-fire revision) in `SITE_GUIDE §13`.
...
### URL state — `?pos=` deep-link receiver (shipped 2026-08-09, `2d945a7`)
WODN reads its slider position from **`?pos=`** on load — the suite's shared channel-position param (§46). **2-tier precedence:** a valid `?pos=` wins over today's live default on first load (it sets `state.userMoved`, which also suppresses the async live-default re-assert in `renderLive`). **No `localStorage` by design** — WODN's whole relevance is *today's* live position, so a stored drag would contradict the page; the URL is therefore the only persistence layer. Write-back on drag (`replaceState`, debounced 250ms) is what makes a reload / deep link restore the slider; the reset-to-today link strips `?pos=` and returns to live. Out-of-range / invalid `?pos=` is ignored → live default. This completes the **How Much Cash → WODN underwater-manager handoff** (HMC writes today's `livePos()` into the link; §39). The `userMoved` flag has exactly one reader (the live-default gate), so overloading it for URL precedence is safe — audited before shipping.
...
**Underwater-manager handoff → WODN (shipped 2026-08-09, `2d945a7`).** The manager-note link (`#hcManagerWodnLink`) now carries **today's live channel position** into Wait-or-Deploy: `renderHandoff()` sets `href="/wait-or-deploy-now?pos=<livePos, 3dp>"` on each `renderAll`, so it upgrades seed→live. WODN is the receiver (its `?pos=` reader; §34). "Deciding when, from *here*" lands the reader on the deploy-or-wait question already at today's spot — the suite carry pattern (senders speak the receiver's vocabulary; §46). The position value is `clampPos`-ed before it goes in the link, so it is always one WODN accepts.
...
- **The no-arrival cash composition (the principle).** **The two-year rule is a valuation MARK, not a modeled trade** — the no-arrival branch marks terminal *cash* to bitcoin at the two-year price rather than assuming anyone executes a rebuy far above their sale. The display makes the mark explicit: when **ARRIVAL = 0%** the NEW STACK card renders the composition (`{stack} → {keptBTC} BTC + {cash} cash` / `≈ {marked total} BTC at the two-year price, after {tax}% tax — the cash held; bitcoin ran/fell while it waited`); the branch strip's no-arrival line adopts the same framing **in all cases** ("the cash was never redeployed at your target — held to the two-year mark it's worth {m₀}× in bitcoin terms: {kept} BTC + {cash} cash ≈ {total} BTC"; stack-empty keeps the multiple only). `noArriveA` already equals `keptBTC + soldBTC·m₀`, so the composition is a decomposition, not a re-derivation. `hcQA` asserts it reconciles: `keptBTC + cash / twoYrPrice ≈ marked total`, where `twoYrPrice = P·(1−t)/m₀` is the representative price implied by the median no-arrival multiple. Verified at JM's case (today's near-floor position, deep target, 50% share, tax 0, stack 25 → arrival 0%, `25 → 12.5 BTC + cash ≈ 13.55 BTC`). The explainer was reworded off "rebuying" to the valuation register for consistency. (Bug caught in build: the compact `usd()` helper didn't exist — only `usdFull()` — so the new composition threw mid-render and left stale DOM; defining `usd()` fixed it.)
...
### Integration
`explorations.json` (group *Models & Trends*, `interactive: true`, no tile); `sitemap.xml` @0.9; `llms.txt` (The Numbers); homepage concept card (trend-line-with-oscillating-price-and-gap-bracket SVG) in The Numbers + Latest (rolled *Wait, or Deploy Now?* out of Latest); `updates.json` (7/23/26); **bidirectional `related:`** with The Power Law, Bull & Bear Cycles, Wait-or-Deploy-Now, and The Doubling Ladder; tool-framing strip (decision-implying); `WebApplication` + `FAQPage` JSON-LD in the head file. **URL state:** `?y=<horizon-years>` only — the multiple is market state, not user state. *(The last clause is superseded by §41.1: `?mult=` was added 2026-09-05. The reasoning behind it was right and is preserved there — the multiple is still market state, which is exactly why the parameter is read-only.)*

### 41.1 The 2026-09 pass — `?mult=`, both bases, and the annualisation seam

Four changes, all shipped 2026-09-05. Three of them are the page's side of rules that now bind the whole site; the fourth is new surface area.

**`?mult=` — a shareable hypothetical position, read-only.** JM asked whether this page takes a URL parameter presetting the position the way Wait-or-Deploy's `?pos=` does. It could not have: WODN has a position *slider* to preset, and this page's position is whatever the market is doing. So the shareable form is `?mult=<multiple of trend>`, which works by **substituting the price the whole page reads from** — precisely what the live-price labelling canon (§26/§40) exists to prevent. It ships behind **three fences**, and they are the feature:
1. **Read-only.** Nothing writes `?mult=` back. The page never talks a reader into a hypothetical; it only honours a link they were handed. Dragging the horizon still writes `?y=` and carries `?mult=` through untouched, so a shared position stays shareable with the reader's own horizon on it.
2. **Never labelled live.** The pulse dot is suppressed *before the fetch is consulted*, the meta line reads "Hypothetical:" not "Live:", the price sub-line says the position was supplied rather than measured, and a banner states the substitution with a route back to the real reading.
3. **Exploration, not recommendation.** Verdict language is untouched and no copy branches on the fact that a position was supplied.

Clamped to **0.21–3.0** (half the floor to the ceiling) so a link cannot put a reader where the chart has no axis; out-of-range values fall back to the live reading rather than erroring. **Two claims had to be conditioned**, found by sweeping the rendered page rather than by reasoning about it: the stance line said "Bitcoin *is trading* at a 58% discount" and the floor note said "Bitcoin *is currently* at the bottom of its historical channel". Both assert the market and both would have been false under a supplied position; only the subject changed ("This position is…"), the arithmetic is identical. Syntax: `/discount-or-premium?mult=0.42`, combinable as `?mult=0.42&y=1.5`.

**The duration record publishes BOTH bases, labelled, with the episode row leading.** The page is where the reversion record is narrated, so it is where the samples-vs-episodes distinction is stated rather than silently resolved: *from the start of a stretch* (episodes) leads with the stronger ink because it is the read a visitor arrives for; *from any point inside one* (samples) follows. Each carries its own count. Both populations already came out of the scan, so this is rendering, not new computation. The episode row's N<3 branch names the stretches instead of publishing a median, matching the Dashboard and the Rundown. Background: `REVERSION_BASIS_MINIREPORT.md`.

**Sub-year windows report a total, not a rate (`STYLE_GUIDE §10.3.1`).** The slider still runs from six months — **the floor was deliberately not raised**, so every `?y=` link already shared keeps landing where it landed. What changed is what the page *says* there: below twelve months the readout reports the total return over the window and the card retitles itself ("Implied return if it reverts by Mar 2027"), because a card headed "Implied CAGR" carrying a total is mislabelled. **Three places needed it, not two** — the two cards were the obvious pair, and the UPLIFT/DRAG line between them is the third; leaving it annualised would have handed the reader two totals and a per-year gap that does not reconcile with them. At exactly twelve months the seam is clean: the annualised figure and the total are the same number.

**The rate chart is clipped at twelve months for the same reason.** §10.3.1 binds plotted points, not only sentences — this page was compliant in its cards and still drew a rate curve down to six months, putting a **~411%/yr** point on a public axis. The chart's *domain* is clipped and the caption says why; the control's floor is untouched. `shared/return-window.js` exports `MIN_MONTHS` so the clip and the copy read one constant.

## 42. The STRC Mechanism (`/the-strc-mechanism.html`, formerly STRC Below Par)

**OVERHAULED 2026-08-10 (`feat/strc-mechanism`) — renamed, reframed condition-neutral, and given the site's first CI automation.** Three changes:
- **Renamed** "STRC Below Par" → **"The STRC Mechanism"**, slug `/strc-below-par` → **`/the-strc-mechanism`**. The old slug **301s** via `_redirects` (new file at repo root, passthrough added to `.eleventy.js` beside `_headers`; CF Pages preserves query strings, so the `?h=/&dv=/&ov=/&p=` scenario params survive the redirect). Canonical/og:url/JSON-LD point to the new slug; the four page files were `git mv`-d to `the-strc-mechanism.*`. `STRC_BELOW_PAR_DESIGN.md` keeps its filename with a header note recording the rename (history stays findable).
- **Reframed condition-neutral.** The page no longer takes "below par" as its premise. It explains the mechanism — the monthly rate lever that targets $100 par — and reads price against par in **either** direction: below par, higher effective yield *and* a priced risk; above par, the market accepting a yield below the stated rate — the mechanism's credibility being paid for. Swept: hero (h1 = "The STRC Mechanism"), intro, the three-numbers line, §B/§A headings + leads, FAQ Q1, head meta/JSON-LD, and the JS header + above-par arithmetic branch. The disclaimer block and recommends-neither stance are unchanged; all live coverage/yield arithmetic is unchanged.
- **Daily official-close updater — the site's FIRST CI data automation.** A GitHub Action (`.github/workflows/strc-daily-close.yml`) fetches STRC's official daily close on market weekdays after the US close and commits `src/_data/strcClose.json`, which the page reads (injected as `window.STRC_CLOSE`; the JS uses it as the price seed). **Source: Yahoo Finance's keyless chart endpoint** — the originally-scoped Stooq CSV is no longer usable keyless (behind a JS anti-bot wall as of 2026-08-10, verified). Logic in `scripts/update-strc-close.mjs` (dependency-free Node, no secrets — default `GITHUB_TOKEN` + `contents:write`). **Guards:** skip-if-unchanged (weekends/holidays → no empty commit); **>25% day-over-day fuse** (aborts red, hand-verify); fetch/parse failure aborts red — the page keeps the prior close with its honest date, so staleness is visible, never masked. Full source/method in `DATA_AUDIT ("STRC daily close")`.
  - **THE BOT-COMMIT RULING (a standing exception to §0):** automated **data-only** commits to `main` by this Action are the **one sanctioned exception** to the "JM merges everything" rule — **scoped to `src/_data/strcClose.json` only**, guarded by the fuse, prefixed `data(strc): `. Every other change still goes through a PR JM merges. (Cross-referenced from §0.1.)
  - **Two freshness classes made visible** (now three, counting the live-from-spot coverage): the **price** is the official daily close (auto, daily); **effective yield + coverage ratios** recompute live from bitcoin spot each load; the **balance-sheet constants** (`STRC_DATA`, `MONTHLY_REFRESH §7.5`) stay filing-sourced and monthly, with their own "as of" badge. The status strip names all three.
  - **`workflow_dispatch` pre-merge is a GitHub limitation, not a gap:** an Action is not dispatchable until its workflow file is on the **default** branch, so the branch run could not be demonstrated pre-merge; the fetch+parse was verified locally (it produced the committed `$94.35` seed). After merge, run one manual `gh workflow run strc-daily-close.yml` to confirm the live Action, then the schedule maintains it.
  - **OG card — deferred handback** (same flow as the dashboard): the head still points `og:image` at the **old** `og-strc-below-par.jpg` (kept served, no phantom-200) until the drafting chat generates `og-the-strc-mechanism.jpg`; then register it in `.eleventy.js` `staticAssets` and repoint the meta. **After merge:** X-card re-scrape for the new slug (joins the re-scrape list).

**Added July 2026.** A **Living on Bitcoin** page in The Numbers (`interactive: true`, **no `calculator_tile`** — prominence control by design; not a personal-decision tool, same posture as Discount, or Premium?). Page-scoped classes use the `sb-` prefix; reads the shared `power-law-data.js` globals (for the live bitcoin fetch and the bitcoin-reversion overlay) and the mixed-content width tier (1100 board/chart/table, 880 prose). The sub-page of `/bitcoin-fixed-income` the parent's Risks tab always implied. Full design doc: `STRC_BELOW_PAR_DESIGN.md` (repo root); the build sits closest to the advice line of anything on the site and shipped **preview-first, counsel-gated** (SITE_GUIDE §0.1 exception invoked deliberately — a PR that is *not* merged until JM's counsel/register review).

**Thesis.** The sibling lens of Discount, or Premium?, one instrument over: a low price is *either* opportunity *or* information, shown with arithmetic, not adjectives. STRC — Strategy's variable-rate perpetual preferred, built to trade near $100 par — trades below it. The **discount reading** (the $12 coupon on a sub-$100 price is a higher effective yield, plus capped upside to par) and the **warning reading** (a peg that isn't pinning is telling you the market demands more than the mechanism delivers) are the same arithmetic; the page shows both and concludes neither.

**Two freshness classes, visually distinguished.** Live-computed from bitcoin spot: the three coverage ratios and their breakevens, and the bitcoin overlay. Dated constants: one `STRC_DATA` object at the top of `the-strc-mechanism.js`, every block carrying an "as of" badge, refreshed on `MONTHLY_REFRESH_CHECKLIST §7.5`. STRC's own price is a dated constant (last close), reader-adjustable in the lens — never mixed with the CoinGecko bitcoin spot, which drives coverage only.

### Structure (tool-first after the JM preview review, 2026-07-28)
Restructured for intuitiveness — the simple hook before the demanding artifact: Hero (question-title, one compressed intro paragraph) → **live status strip** (STRC price · coupon · effective yield) → **key-insights box** ("the page in three numbers": effective-yield-vs-coupon, the both-ways sentence, and full-waterfall coverage with its $17.4K breakeven — a reusable call-out piloted here, `sb-keyinsights`, candidate for later extraction) → **§B the below-par arithmetic + the interactive** ("the below-par lens": price input, horizon slider + never toggle, dividend-scenario toggle, opt-in bitcoin/Treasury overlays, and a collapsed demanded-yield ⇄ price converter) → **§A the lever board** (four levers — rate / supply / bid / a fuel gauge carrying the three live coverage ratios — each a card that keeps its title + 2–3 status readouts visible with **its full log & mechanics behind a native `<details>` accordion**, default closed, no stored state; the fuel gauge keeps the three ratios visible and collapses only its event log + constants) → **§C two questions, one from each shore** (a computed **three-column** table — STRC vs bitcoin vs the 10-year Treasury — answering "why not simply own bitcoin?" *and* "why not simply hold Treasuries?", with a mark-to-market-in-a-drawdown row and a de-tell "yield-bearing waiting room" note that links to Disciplined Rebalancing) → **§D the cost accounting** (each lever's cost, both ways; which-number-to-watch breakevens) → **§E the record strip** (a schematic of price-vs-par since issuance — shape, not tick data) → FAQ → related → disclosures. Progressive disclosure keeps the page a short scroll rather than a density wall; the demanding board arrives after the reader has the tool and the three numbers.

### Deliberately episodic (design §7 — a first for the site)
Unlike the evergreen pages, this examines a **live episode** and says so. The aging policy is a comment block at the top of the `.njk` and a decision step in `MONTHLY_REFRESH_CHECKLIST §7.5`: each month decide **ongoing** (live numbers carry it; nav entry stays) vs **resolved** (par regained or a dividend action taken → convert to a post-mortem, **retire the `explorations.json` entry** so the nav slot sunsets and reachability reverts to the parent + related links). The site never carries a stale dashboard in its nav.

### Guardrails (design §5 — this page is closest to the advice line)
- **No verdict, no buy/sell language, no price targets.** "Estimated:" prefixes every projection; the return-to-par figure never renders without its never-returns companion (same size, `.sb-pair`).
- **Both-sides, de-tell.** The supply lever's dual reading, the non-binding-framework honesty row, and the cumulative-arrears/cut-bar guardrails render as status fields, not footnotes. §C's is the page's one licensed rhetorical question.
- **Verify badges over silent shipping.** Anything not independently reconciled at build carries a visible `verify` badge: currently the reserve-implied *total* preferred bill (STRC's own bill *is* computed float × rate) and the company BTC-breakeven-ARR figure.
- **Single-security disclosure.** The sources block states education-not-advice and a no-position / no-compensation line, authored fresh (no boilerplate existed).

### Build reconciliations worth keeping (design §5 verify pass, all confirmed)
- **§B arithmetic** at $88.32: effective yield **13.59%**; discount **~11.7 months**; return-to-par **44.0 / 26.8 / 18.5 / 15.5 %/yr** at 6mo/1y/2y/3y; a 12-month par holder **+0.3%**. All matched the design doc.
- **Coverage** at bitcoin ~$64.5K: **5.20× / 4.79× / 3.15×** (gross / standalone / waterfall). Breakevens: gross = 1.0× at bitcoin ≈ **$12.4K**; standalone and waterfall both = 1.0× at ≈ **$17.4K** — they collapse to one price by construction (each ⇔ pool = senior + STRF + STRC notional).
- **Notional** derived from `shares × par` as the single source of truth (10.460B); the filed `strcNotionalB` (10.464B) differs by ~$4M / 0.04% (rounding) — cross-checked, not inherited.
- **Dividend bill** computed float × rate: STRC = **$1.256B/yr**; the broader total (~$1.8B reserve-implied) is **not** reconstructible bottom-up (STRK/STRD floats absent) and carries a `verify` badge rather than any inherited "$1.2B".

### Open items
- **OG card — SHIPPED.** `og-strc-below-par.jpg` (1280×720, ~51 KB, product-forward §6.15.2) generated externally from the branch preview and committed at the repo root; the `.eleventy.js` passthrough is now **active** (uncommented). **Hero = the below-par lens chart** (the single amber return-to-par curve), NOT the lever board — JM's call after reviewing both cards (the board is too dense at card scale; the curve reads at a glance). The `scripts/build-og-images.py` CARDS entry uses `hero_selector: ".sb-chart-wrapper"` to keep monthly regenerations consistent with the shipped card. No longer pending — the §10 `curl -I` `image/jpeg` check runs post-merge as normal.
...
- **Carousel slide — SHIPPED July 2026** under The Numbers only, `data-feat="0"` (not Featured, per JM's 2026-07-28 prominence decision). See the §13 inventory row + three-brief iteration record and the engineered-luminous register (§6).
...
### v1.1–v1.2 (shipped together 2026-08-06) — one forward look, a peak stat, an IRR, a hover
- **"If the habit continued" — a banded forward scenario block. This narrows the R1 "retrospective only" ruling:** a forward projection is now *allowed on this page*; income / sustainability analysis remains the retirement cluster's. The block simulates ten more years of the same daily buy from today (`amount / (plPrice(d) × band-mult)`), stacked on the accumulated coins, valued at the band price ten years out. **Two bands — floor → trend, presented as the planning range** (v1.2). **Upper-band demotion (v1.2), mirroring `RETIREMENT_CALCULATOR_DESIGN`'s floor-vs-upper asymmetry finding:** upper excursions have been brief spikes, not places price stays, and each cycle's overshoot has been smaller — so upper is dropped from the cards (no `$` figure computed or shown) and demoted to one caption clause with a tooltip (verbatim spike language, linking `/bull-and-bear-cycles`). Floor/Trend tooltips reuse the canonical channel vocabulary (structural floor at 0.42× trend; the central Power Law trend); "fair value" was cut as a non-canonical valuation judgment. A **humility clause** links `/the-power-law#caveats` ("a model whose own authors warn it weakens with distance"). **Handoff** (carry-the-scenario, POSITIONING §3.5): a state-carrying link into `/the-bitcoin-retirement` via its `?stack=` (2dp) + `?dca=` (monthly = daily × 30.4, int) receivers — both already exist, so no receiver gap.
- **Stat grid → 8 cards in four related pairs (v1.2):** BTC accumulated | Total invested · Current value | Peak value · Multiple | Annualized return · Longest underwater | Deepest drawdown. "Value" renamed **"Current value."**
- **"Annualized return" card (v1.2) = money-weighted IRR**, NOT `(value/invested)^(1/years)` — that naive CAGR treats every dollar as invested on day one and understates a DCA stream (legend ~22%). The IRR solves `amt·Σ(1+r)^tᵢ = current value` on the daily flows (closed-form geometric sum; legend ≈ **38%/yr**). Live tooltip contrasts the two and states the naive CAGR-equivalent, so it never goes stale. Caption: "money-weighted (IRR)."
- **Peak-value stat card** — highest stack value + date (legend ≈ $1.37M, 2025-10-10), "visible only in the rearview mirror" (anti-timing).
- **Chart hover/tap tooltip** — date · contributions · stack value · BTC held · multiple, index-mode + `intersect:false` (touch-friendly).
- **BTC-bought copy (v1.2)** reframed as the second-decade insight (the first decade's ~11.3 BTC vs ~0.24 on the trend path next; the floor path gathers more than twice the trend's coins).
- **Consistency:** the FAQ "is this a prediction?" answer and the closing prose read "retrospective at its core, with one forward band-scenario element — a range, not a forecast" (v1.2 prose sweep: "a range, never a number" / "two paths and a history of brief spikes above them"). Parity gate P1 figures unchanged (the forward block and IRR do not touch the retrospective sim). No new external data → DATA_AUDIT unchanged.
...
**What it is.** A quiet, single-row house block on every slugged page channeling readers to the newsletter. **Substack-first** (JM approved the shape 2026-08-05): the list lives on Substack — exportable, so still *owned* — and the site's only job is to send readers there. It is the site's answer to the structural weakness that explorations are read once: a way to make the audience returnable without renting it from an algorithm.
...
Per `FUNDING_STRATEGY`'s no-funnel commitment: promises little and means it — update-only, no drip sequence, no upsell.
...
## 45. The Bitcoin Hurdle Rate (`/the-bitcoin-hurdle-rate.html`)

**Added August 2026. Ship record:** built `1ab4ea4` (2026-08-06), **merged to production via PR #40 as `d6b9609` (2026-08-07)**; **v2 channel-position view merged `52fc022` (2026-08-08)**, recorded at `587a020`. JM re-confirmed the page live on 2026-08-28 — a confirmation, not a second ship date. A **Positioning & Strategy** page in The Numbers (`interactive: true`, **with** a `calculator_tile` — it takes company/personal inputs, unlike the market-state explorers). Page-scoped classes use the `hr-` prefix; reads the shared `power-law-data.js` globals and the mixed-content width tier (1100 page / 880 essay). Full design at `HURDLE_RATE_DESIGN.md` + build order `HURDLE_RATE_BUILD_PROMPT.md` (both committed to the branch). Promoted from the backlog's "Bitcoin's CAGR as the new hurdle rate" (surfaced 2026-08-02), flagged there as the highest-value idea for the asset-management ambition specifically.

**Thesis.** Every capital decision is a comparison against the next-best use of the money — the hurdle rate a CFO already uses. For anyone who holds bitcoin or could, bitcoin's trend growth is the candidate hurdle: a use of capital returning less than bitcoin's trend has, over that horizon, been the lower-returning choice — if the trend holds and the capital can wait. It is opportunity cost, not a monetary thesis, which is why it reaches the allocator audience the site is built for.

**The finding that makes it a page.** The hurdle is not a number — it is a declining curve, matched to the horizon of the decision, because the Power Law's growth rate decays with age. A three-year purchase is measured against ~35%; a thirty-year asset against ~21%. Comparing every decision to one headline CAGR is wrong in a direction that matters.

**Two lenses, one engine (§6.17).** Company (does this project clear the bar?) and Personal (does this use of money clear the bar?). The arithmetic is identical; the inputs, examples, and frictions differ. Both ship because the divergence is itself content — the "Where the two lenses diverge" essay section is load-bearing, not a footer: the company's mandate/accounting constraint, the individual's concentration constraint, and the unifying insight that "can the capital wait?" and "is the business cashflow positive?" are the same condition in different vocabulary. The second-engine passage carries its limit inline — the hurdle applies to *surplus* capital at the margin, never to the capital that generates the cashflow.

**The access condition (the honesty mechanism).** The arithmetic hurdle overstates the binding one (the tool shows the smooth trend; the realised rate over any window lands above or below it; the hurdle that actually binds a holder is lower) &mdash; and the reason is the access condition: bitcoin's trend return is realised only by capital that can hold through a ~−73% drawdown without being forced to sell. Two verdict-changing gates encode this — a not-cashflow-positive company and personal capital with a deadline both flip the verdict to "the hurdle does not apply" and blank the stat strip, rather than decorating an unchanged answer (design §3.5). The overclaim limit ("at the margin", "capital that can wait", "for a holder who can survive the drawdown") lives *inside* the verdict sentence, with both conditions — not in a caveat block, so a screenshot of the tool carries its own limits.

**Zero new model.** `PL_A`/`PL_B`/`PL_FLOOR`/`plPrice`/`TODAY_DAYS`/`fetchTodayPrice` all from the shared module; core primitive `trendCAGR(t,H) = ((t+365.25·H)/t)^(PL_B/H)−1`, recomputed live from `TODAY_DAYS`. Fixture reproduces 37.6 / 35.4 / 33.5 / 29.7 / 24.5 / 21.1 at t = 6,424 (2026-08-06). No new coefficients; the only DATA_AUDIT rows are **HR-1 / HR-2** for the two short-rate presets (money-market ~4.3%, savings ~4%) — everything else reads the shared module.

**§2.3 CAGR-window reconciliation (the build's blocking gate).** The build prompt premised that `/the-bitcoin-retirement` "computes a ~28% trend CAGR." Implementation reality: the retirement page renders **no bare CAGR** — it projects forward *prices* along the full trend and explicitly declines a single CAGR (its year-over-year % is captioned "not a fixed bitcoin-price CAGR", `the-bitcoin-retirement.njk:185`). The bare "~28%" a reader actually saw lived on `/bitcoin-fixed-income` (one spectrum card + two prose lines). Verified it is hardcoded static text, and **not a PL-1 exponent conflict** (the page's live calculator uses shared `PL_B` via `plPrice`; the `0.28` seed is overwritten live). Reconciliation: the "28% today → 14% 2045 → 10% 2065" trajectory is a **10-year-forward window** (29.7 / 15.0 / 9.8 recomputed). Resolution shipped in the same PR: windowed the three fixed-income figures to a "10–15-yr forward" basis (brackets this page's live 10yr 29.7% / 15yr 26.8%) + DATA_AUDIT row **BFI-1**. Every CAGR on the hurdle page names its window — hero, verdict, stat strip, FAQ, meta description.

**Chart geometry (a design conflict flagged, not silently resolved).** Design §4.2 defines the band's upper edge as spot→trend; §5.2 + the §4.1 fixture + the §7 overclaim flag point to trend→trend `trendCAGR` as the honest hurdle. With bitcoin on its floor today (~0.42× trend), spot→trend spikes to ~227%/yr at H=1 — which would breach the overclaim flag and wreck legibility. Resolution: primary declining curve = `trendCAGR(H)`; floor-path band beneath = spot→floor (the conservative/access-conditioned reading, ≈ the curve today because spot sits on the floor, widening if price rises above it); the spot→trend reversion upside is a **secondary stat** at the chosen horizon (~41% at 10yr), not a chart line. All three are computed in the engine, so §4.2's literal reading is a one-line flip if JM prefers it on preview. **v2 UPDATE (2026-08-08):** that flip shipped as an **opt-in view**, not a replacement — see the v2 bullet below.

**v2 — the channel-position view (branch `feat/hurdle-position-view`, 2026-08-08).** A **view toggle** (the §6.22 `chart-range-toggle` segmented control — deliberately not a second §6.17 yin-yang) sits **above the verdict** and **governs the whole reading**: the verdict, all five stat cards, and the chart all compute on the selected view's basis (design §4.0 corrected — the position view is the *complete* answer when selected, not a chart overlay; the first pass built it chart-only and its verdict contradicted its own chart at `?view=position&r=50` — see `HURDLE_RATE_DESIGN.md §13.1`). **From the trend** (default, = v1): bar `trendCAGR`. **From the channel** (opt-in): bar `posCAGR` = spot→trend, read from bitcoin's Power-Law-channel position today. Both views' floor band is `spot→floor`, legend **"Floor case (0.42× trend)"**. **Shared y-axis** across both views (max across both curves at current k, not hardcoded) so toggling shows the jump — this **reverses build §2's per-view axis** and means the default view is **no longer pixel-identical to production** (build §8), both by design (§13.1). The position view plots from **H=3** — a fence **load-bearing for §4.2** (clears the §7-flag-1 overclaim the axis alone doesn't; dropping it reopens the §4.2 rejection); below 3yr the whole answer block fences. `posCAGR` is non-monotonic above trend, so the verdict handles double-crossings honestly. The v1 "optimistic edge" card is reworked into the **position-adjustment / bridge card** (both views, position-neutral, sign-carrying headline = the pts the channel moves the bar off trend; no sixth card). A bold **computed lede** sits atop each chart caption. Gating: not-live → figures explicitly labelled " (latest monthly data)", never silently live. URL gains `?view=trend|position`; joins localStorage stickiness. Full record: `HURDLE_RATE_DESIGN.md §13` + **§13.1** (post-review rebuild). **Debug param `?k=` (review-only):** forces channel position (e.g. `?k=1.5`) so label-neutrality can be checked at a simulated above-trend position without waiting for the market. Overrides spot to k×trend(t), presents as live for the review, **not linked from the page**, **stripped from the URL on any interaction**. Used for the mandatory pre-ship neutrality sweep (design §5).

**MERGED TO PRODUCTION 2026-08-08 (merge `52fc022`).** Final state after two JM preview reviews (`HURDLE_RATE_DESIGN §13.1` rebuild + `§13.2` polish): the view buttons carry the live channel position — **"From trend (1.0×)" / "From today (0.43×)"** (the second number live from `k`; the trend view *is* the k=1.0 case, so it's one calculation at two positions; edge `k`→1.00× shows "From today (at trend)"). The toggle is **duplicated above the chart** (one state, both instances synced by `applyViewUI`). Verdict callout height is equalised with a **dynamic min-height** (`equalizeVerdict` measures both views per render, `border-box`) so the chart stays put on switch without padding the short cases. **Default stays the trend view** — comprehension order (the channel figure is a deviation that reads as a pitch without the trend baseline first); design §4.0. Meta/OG/JSON-LD/FAQ 5 rewritten so the **level leads** and the decline is mechanism, matching the OG card image. The below-trend-language pre-production gate (`OPEN_ITEMS`) was read as written and **closed** — no advocacy language remains. **Open:** trend-view card 2 mixes trend→trend (the 1.0× bar) with spot→floor (real k) — a labeling question flagged in §13.2, not blocking. **OG card `og-the-bitcoin-hurdle-rate.jpg` predates the card-4 rework and needs regenerating** against production (it still shows the old "optimistic edge" card).

**The refrain.** "Does it beat bitcoin?" — a question, never a slogan — appears exactly three times (hero subtitle, above the verdict, essay close), each adjacent to the two conditions (the trend holds; the capital can wait). Three, not more; a refrain repeated too often becomes a jingle.

### Not
Not advice (facts-not-signals; no "you should"), not a single-security/MSTR analysis (the general principle only — the MSTR examination is a separate backlog entry), not a three-way IRR/WACC/hurdle readout (deferred; new backlog entry), not a Sharpe-adjusted readout (dropped — it needs a project volatility the reader doesn't have).

### Open items
- **OG card** — `og-the-bitcoin-hurdle-rate.jpg` **not yet generated** (no working Python on the authoring machine). Meta wired to the final URL; `.eleventy.js` passthrough entry **added**. Hero selector for the generator = the hurdle-curve chart, not the input board (STRC precedent: dense boards don't read at card scale). Register in MONTHLY_REFRESH §6 regen list when it lands.
- **Carousel slide** — proposed in §13 Pending additions; brief separately.
- **Substack essay** — deferred to after preview per JM (inverts the Bitcoin Exit order because the claim is analytical). Log the reciprocal essay→tool link at ship.
- **Homepage concept card** — not added unilaterally (curated editorial placement); flagged for JM in the PR.
- **Money-market / savings presets** — the two short-rate-sensitive presets (~4.3% / ~4%) are approximate starting points the user overrides; **tracked via DATA_AUDIT HR-1 / HR-2** (as-of 2026-08-06, review 2026-11-06), **not** a MONTHLY_REFRESH line — the audit row's review-date column carries the tracking, keeping the monthly refresh tail at zero.

### Integration
`explorations.json` (group *Positioning & Strategy*, `interactive: true`, SVG `calculator_tile` @ position 20 → anchor `#explore`); `sitemap.xml` @0.9; `updates.json` (8/6/26, "New:"); `llms.txt` (The Numbers); `.eleventy.js` OG passthrough; calc-tile icon at `components/calc-tile-icons/the-bitcoin-hurdle-rate.njk`; **`related:`** carries Bitcoin vs. The Stock Market, the Doubling Ladder, Bitcoin vs. Real Estate, Borrowing Against Your Stack (each with a decision-relevant `desc`); **`/the-power-law` lives inline in the hero, not in the strip** (§6.10 one-placement — the hero link is load-bearing for the central number); tool-framing strip (decision-implying); `WebApplication` + `FAQPage` JSON-LD; FAQ ships inline (5 Q&As). **URL state:** `?r=` (candidate return), `?h=` (horizon), `?lens=` (company|personal); per-page localStorage `lcs.bhr.calc.v1`.

**v1.1 (JM review, 2026-08-06).** Lens toggle moved above the inputs (it gates which inputs render); section rhythm tightened toward input+chart+verdict co-visibility (chart not shrunk); chart legend enlarged to axis-label scale; stat numbers use `--text` with a one-sentence help-tip per card, grid re-flowed 5-across (no 4+1 orphan); funding-source control given an honest framing tooltip (it relabels the fifth stat card as cash yield / debt cost / equity cost and changes no figure — the control is presentational in v1); copy revisions including the "ceiling"→smooth-trend rewrite and a new idle-treasury passage (links `/the-melting-ice-cube`, no company named). Full record in `HURDLE_RATE_DESIGN §11`; two v2 items (channel-position-aware hurdle HIGH, funding-drives-arithmetic MEDIUM) in `PAGE_IDEAS_BACKLOG`. Cashflow control deliberately left un-colour-coded (facts-not-signals).

**History.** Built Substack-first per the 2026-08-05 build prompt (branch `feat/get-updates-substack`); QA'd at 375px + desktop against a static harness (no Node locally), confirmed single-row → stacked with no horizontal overflow and zero third-party requests. Backlog "Owned audience" entry marked shipped in the same PR; `REACH_GROWTH_PLAN`'s "email capture" line is now live (that doc is now repo-tracked).

## 46. URL state conventions (shared vocabulary)

**Added 2026-08-09** with the How Much Cash → Wait-or-Deploy handoff (§39, §34). As pages hand state to each other, a **shared parameter vocabulary** means any page's link to any other reads the same way; divergent names would make every handoff need its own translation. So parameter names are chosen **for the suite, not just for one link**.

**The rule.** When a page needs to encode a quantity another page also uses, **adopt the existing param name and encoding** — don't mint a synonym. And **only adopt the names that fit the quantity**: take the half that matches, refuse the half that doesn't. Wait-or-Deploy is a single-position tool, so it took How Much Cash's **`pos`** and deliberately **not** its `rebuy` — forcing `rebuy` onto a page with no rebuy leg would make the vocabulary consistent and meaningless.

**The channel-position family — `pos`.** The canonical encoding for "where in the Power Law channel": a **normalized position** (0 = floor, 1 = upper band, slightly negative = sub-floor), a **float at 3 decimal places**, range-checked to `[POS_MIN ≈ -0.08, POS_MAX = 1.0]` and `clampPos`-ed. **Never** a ×-trend multiple and **never** a day-index — the day-encoding lesson (§39, HMC v3) was that a non-normalized encoding forces exactly the lookalike conversions the reuse rule forbids. Because the mirror twins share `channel-entries.js` / `power-law-data.js`, `pos=0.62` denotes the identical channel position on both — portable with zero translation.
...
## 47. The Bitcoin Dashboard (`/dashboard.html`)

**Shipped 2026-08-10 (`feat/dashboard-v1`) — build `fd00ee2`, OG `897fa46`, merged to production via PR #43 as `8d184bc`.** JM re-confirmed the page live on 2026-08-28 — a confirmation, not a second ship date. The site's **return-visit anchor destination** — the resolution of the three competing "come back here" surfaces (PAGE_IDEAS_BACKLOG dashboard ruling, JM 2026-08-05): the dashboard is the room, the site-wide **channel-position chip is its future doorway** (ships after, not built here), and the **Gallery-highlights strip is closed**, absorbed here as the jump-back-in row. Positioning is the site's own vocabulary — *"where are we in the channel, and what does that mean for your position"* — which Clark Moody's network-health dashboard (the explicit foil) does not own.

**v2 (2026-08-10, `feat/dashboard-v2`) — ribbon repoint + four tiles + freshness.** Three shifts:
- **The chip is rescoped to the ribbon — no separate chip element is built.** The site-wide **channel ribbon** (§40) now points its whole strip at `/dashboard` (was `/the-power-law`) and closes with a **"see where we are →"** CTA in orange, matching the `/calculators` tile and homepage card. The ribbon *is* the "site-wide entry point" the v1 ruling called the chip; building a second chip element would duplicate it. The ribbon's data content (mult · zone · price) is unchanged; the dashboard itself keeps `channel_ribbon: false` (no self-link). The Power Law channel stays reachable via the dashboard's first jump-back-in tile (verified, unchanged). This **closes OPEN_ITEMS Dashboard follow-up #1**.
- **Four new tiles (nine total), same live-compute fence.** (a) **Price is where trend was in ___** — inverts the trend, `d* = (spot/PL_A)^(1/PL_B)`, mapped to a calendar date; both the below-trend (date in the past, "N.N years ago") and above-trend (date in the future, "where trend is expected to reach it") branches are coded, only the below-trend one renders while price is under trend. (b) **Below-trend streak + the record** — the current continuous stretch below trend, plus the median and longest historical stretches, **computed live**; deep-links Wait-or-Deploy via the existing `?pos=` schema. **Measured as elapsed time (day-spans → months), NOT sample counts:** `PL_DATA` is a ~12-day grid (481 samples, 476 of 480 gaps exactly 12 days), **not** monthly — so a 72-sample run is ~28 months, not 72; a sample-count reading would print a "72-month" record that contradicts `/discount-or-premium`'s ~21-month duration record. This surfaced the module's long-standing wrong "monthly samples" comment, **corrected in this pass** (module header + inline comment), the finding **recorded in `DATA_AUDIT.md` ("PL-2 sample cadence")**, and the repo swept for the same claim (tile 4 "monthly samples"→"samples"; `/discount-or-premium` episode caption; `/disciplined-rebalancing` tooltip; bvsm + calculators-minis comments). The two records do not conflict: `/discount-or-premium` measures a *depth-matched wait-to-trend*; tile b measures *any* continuous below-trend stretch (a clarifying clause on the tile says so). (c) **10-year trend hurdle** — the structural bar `(trend(t+10y)/trend(t))^(1/10)−1`, the Hurdle Rate's `trendCAGR(10)` primitive (so both pages quote the same ~29.7%); links the Hurdle Rate for the from-today lens. (d) **Below the all-time high** — `(ATH−today)/ATH` from `max(PL_DATA, live spot)`, with the ATH month. **Register guard held:** descriptive framing only ("where price sits relative to its highest recorded level"), no recovery language, no implied signal.
- **Freshness treatments.** The channel-position **marker** gains the ribbon dot's live pulse/glow (the canonical two-ring `lcsRibbonPulseRing`, copied into the page CSS per foundation-preamble discipline; runs only when `source === 'live'`). The price badge becomes **"Today (live) · <Month D, YYYY>"** — the client date always shows; "(live)" only when the fetch resolves (labelling canon).
- **Reciprocal back-links added** (the v1 deferral, now that the ribbon is the entry point): `/dashboard` is added to the `related:` front matter of the two `?pos=` receivers (Wait-or-Deploy, How Much Cash). `related.njk` gained a slug-aware hub label so the card reads "Dashboard", not "Visual index".
- **New monthly-refresh line:** the dashboard OG card bakes in the position multiple (0.42×), so `MONTHLY_REFRESH_CHECKLIST §6` now carries a manual check to regenerate it if the multiple moves materially or the zone changes.

**v3 (2026-08-10, `feat/dashboard-v3`) — copy precision, edge branches, one new tile, reorder. Ten tiles.** Dashboard page files only (no shared-module change: `positionLabel` already had a below-floor branch, confirmed not modified).
- **Copy precision.** Subtitle → *"Key stats on bitcoin's position in its Power Law channel right now — live-computed in your browser."* Hero position track now runs the **full content width** of the tile, with a plain caption (*"Today's price divided by the trend price. Below 1× is below trend…"*) and a `?` that explains the channel for newcomers. Price tile gained a **dynamic derivation line** (live: *"Fetched live on page load."*; fallback: *"Live fetch unavailable — showing the latest sample (⟨date⟩)."*) — badge canon unchanged. Below-trend share defines the samples inline (*"…price samples — one every ~12 days since 2010…"*). Hurdle retitled *"10-Year Trend Hurdle Rate"* with an explicit trend-to-trend subtitle. Days-to-double → *"Implied trend doubling"* with the dollar pair (⟨trend⟩ → ⟨2×trend⟩) and a layman tooltip for the 12.76% constant. *"Price is where trend was"* → **time-first reframe** *"Behind the trend's schedule"* (big value = the number of years; the date moves to the sub-line; above-trend flips to *"Ahead of schedule"*).
- **Edge branches (all four unit-checked offline; none renders at today's ~0.43×).** Below-floor (marker clamps to the track start, `positionLabel` returns a below-floor word, the multiple still shows exactly); above-trend reframe; **at-ATH** (spot = series max → *"0% — at the all-time high"*); **new-ATH** (spot > series max → *"a new all-time high — today"*, naming the prior high). The ATH tile also states the figure + basis (*"…the highest close in this ~12-day series; brief intraday spikes may sit slightly higher"*).
...
**V1 tile set — exactly five, live-compute-only (the backlog fence).** ZERO new data sources, zero new MONTHLY_REFRESH_CHECKLIST lines; every number derives from `shared/power-law-data.js` (`PL_DATA` + spot) at runtime, nothing hardcoded:
1. **Channel position** (hero) — `TODAY_PRICE ÷ plPrice(TODAY_DAYS)` as ×-trend, named by the **shared `positionLabel()`**. This page is that vocabulary's **6th consumer**; it introduces **no** zone-family wording (TECH_DEBT position-label divergence entry). A pure-CSS position bar places the marker on the floor→trend→upper scale (trend tick computed from `PL_FLOOR`/`PL_CEIL`, not hardcoded).
2. **Bitcoin price** — `fetchTodayPrice()`, provenance strictly through `todayPriceLabel()`/`todayPriceIsLive()` (live-price labelling canon, §26/§40: "live" only on a real fetch; seed and fallback read "latest monthly data"). The live-gated dot pulse obeys the same canon — animates only on `source === 'live'`.
3. **Trend price today** — `plPrice(TODAY_DAYS)`, with floor (`PL_FLOOR×`) and upper (`PL_CEIL×`) as context.
4. **Below-trend share of history** — counted live by iterating `PL_DATA` against `plPrice()` at each sample (~58%, **not hardcoded**), with the register note "below trend is bitcoin's ordinary condition".
5. **Days-to-double + the constant** — trend doubling time (`TODAY_DAYS × (2^(1/PL_B) − 1)`) and the ~12.76% age-growth constant, both derived from `PL_B`.
...
- **OG card deferred** (handback): head meta references `/og-dashboard.jpg`; the image + its `.eleventy.js` `staticAssets` registration land in a follow-up once the drafting chat generates it (NEW_PAGE_CHECKLIST §7/§8). Until then the OG URL phantom-200s — tracked, not dropped.
- **No FAQ / thin SEO by design** — bookmark magnet, not a search target (sitemap priority 0.7, weekly). WebApplication JSON-LD only.

**v3.1 (2026-09-05, `fix-reversion-basis` + `fix-streak-precision`) — the reversion tile moves to the episode basis, and the local port is retired.** Two published figures on the site's return-visit anchor changed, so it went to a branch, a preview and JM's merge rather than direct to main. Background and the before/after table: `REVERSION_BASIS_MINIREPORT.md`; the row is in `DATA_AUDIT.md`.
- **The count was the point, and it ends.** Median `~145% over ~9.1 months` → `~141% over ~8.5 months`; quickest `~2.0 months` → `~4.3 months`; slowest unchanged; **count `65 completed samples` → `6 completed episodes`.** A long stretch contributes dozens of samples and exactly one episode, so "65 completed" read as a record about ten times deeper than it is. Six is the number of independent things that have happened.
- **The local port is retired — this is the part that makes agreement structural.** The v3 bullet above records the tile as *porting* `/discount-or-premium`'s `scanDurations` verbatim. **That is no longer how it works.** The tile reads `shared/reversion-durations.js`, the same scan Discount-or-Premium and the Rundown read, so the three pages cannot drift apart by being tuned separately. The port is exactly what let a samples-vs-episodes divergence sit unnoticed (Rundown 6 episodes / 65 samples against the Dashboard's 7 / 70 — the open episode was the entire difference).
- **N<3 now reachable, and it fires.** On the sample basis the count was always in the dozens and the branch was dead code; on the episode basis it fires at 0.40× and below. Verified live at that depth: the tile drops the median and the spread sentence and names the stretches instead. Compact by design, because this is a tile — count, durations, and the implied read for the slowest so it still answers *"and what would that be worth"*. §10.3.1 still governs that read.
- **The streak prints whole months.** Precision follows the series: the streak is read off the same ~12-day grid as every other duration on the site, so a tenth of a month is precision the source does not have. Display rounds; computation stays exact. The rule lives in `shared/reversion-durations.js` (`fmtMonths` / `fmtMonthsShort`), not in three stylesheets' worth of call sites, because it is a property of the data rather than a per-page style choice.

**Open enhancements.** Optional macro tiles (idle-$17T, fiscal-canary, MMF flows — explicitly OUT of scope; each carries its own refresh tail). *Closed:* the channel-position chip (rescoped to the ribbon repoint, v2); companion reciprocal back-links (added v2); the OG card + staticAssets registration (landed 2026-08-10, `897fa46`).

---

## 48. Bitcoin as Collateral (`/bitcoin-as-collateral.html`)

Shipped 2026-08-18. **The Arguments / "Holding & Spending"**, `interactive: true`, no `calculator_tile` (see below). Built from the reviewed v2 preview, which is the copy-of-record: all prose, tables, cell reasonings and tooltips ported verbatim.

**Thesis.** Collateral is the pledge behind a loan, and for four thousand years every lending system has been machinery for answering one question the borrower's word cannot: is the pledge real? Bitcoin is the first asset where the pledge itself is verifiable by anyone, without permission. And lenders currently offer some of the harshest terms in secured credit against it. The page takes both facts seriously and refuses to resolve them: it names what would have to change, then instruments it.

**Four tabs plus a coda.**

1. **The History** &ndash; the four questions every lender asks, the Roman trio (fiducia / pignus / hypotheca), verification-by-paper and its failures (goldsmiths, the 1963 salad-oil scandal), the custody paradox, and gold's forced trade-off between bearer and practical.
2. **The Scorecard** &ndash; the interactive grid (below).
3. **The Gap** &ndash; what $100 of each asset borrows, why houses don't get margin calls, the five forces decomposed into three structural and two market, the serious objection (performance in the seizure state), and the tripwire watchlist.
4. **The Practice** &ndash; five pledge structures compared, the Celsius clause, the handoff to Borrowing Against Your Stack, and the record.

The **coda sits below the tabs and is seen by every reader** regardless of which tab they end on. That placement is load-bearing: the page's one forward-looking claim (collateral value is a use of bitcoin the market barely prices yet) belongs where nobody can miss it, and it is stated as an option, not a prediction.

### 48.1 Scorecard design decisions

- **Ten properties &times; seven ways of holding five asset classes.** Vertical separators mark the five classes.
- **Two assets get two columns each &ndash; gold and bitcoin.** This is the argument of the page, not a formatting choice: gold's two modes (physical/allocated vs. paper/unallocated) are a *forced trade-off*, and bitcoin's (self-custody/multisig vs. custodial) are a *choice*. Read the two bitcoin columns together and the distance between them is the distance between what bitcoin makes possible and how most institutional bitcoin is actually held. The page claims the capability, never the practice.
- **Three lens views, filtered and redrawn, never dimmed.** All rows / the lender's eye (six properties, ranked) / the borrower's eye (four, ranked). Dimming was rejected: a dimmed row still reads as a rating the reader has to actively discount.
- **The strips row ("All ten, compressed") always summarises all ten**, whatever the lens shows &ndash; it is a *column* summary, not a summary of the visible rows. Bitcoin self-custody's barbell shape (brilliant at one end, unproven at the other, almost nothing between) is the fastest read on the page and the reason the row exists.
- **&#9650; markers only on cells the thesis expects to move** &ndash; six cells, all bitcoin, across liquidity / legal certainty / stress. Each carries plain-language "what would move it" copy. Nothing else on the board is marked, which is itself the claim: gold's assay costs and real estate's foreclosure timelines are finished.
- **No column is designed to win.** Treasuries take the rows they deserve, and bitcoin scores Poor on income and Weak on stress.

### 48.2 The record (three instruments, quarterly)

The page maintains its own evidence rather than asserting a trajectory: (1) the **rate & LTV series**, logged quarterly *with lender composition noted*, because the series only compares like quotes; (2) the **collateral panel**, which tracks named publishing sources rather than a fake market total, read both in coins and in dollars because those answer different questions, and which lists what cannot be observed as exactly that; (3) the **event ledger** of at-scale liquidations. First entries ship at launch with a Q4 placeholder row. A page arguing the asset is maturing as collateral should be the same page that lists every episode.

### 48.3 Build notes

- **Width:** mixed-content tier (1100 container / 880 prose, STYLE_GUIDE §4.2), re-derived from the table &ndash; *not* inherited from the preview's 1060/780.
- **Sticky tab bar + end-of-tab continue links** &ndash; the house multi-tab pattern this page establishes (backlog entry, Site & platform). Tabs give random access but hide sequence; the continue links restore the intended order without taking the random access away. The bar parks beneath the sticky `.site-nav`, whose height the page JS measures into `--bac-nav-h` rather than hardcoding (the two existing sticky sub-bars on the site disagree, 65px vs 44px &ndash; measurement avoids joining that drift).
- **One tooltip mechanism, not two.** Every `?` help-tip and every &#9650; marker renders through a single `position: fixed` card painted by the page JS. The §6.13 absolutely-positioned sibling card cannot work here: the scorecard sits in `.bac-scrollwrap` and four `?` triggers sit in `.bac-tablewrap`, both `overflow-x: auto` &ndash; and a box with `overflow-x: auto` and `overflow-y: visible` computes overflow-y to auto too, so the card gets clipped vertically as well as horizontally. Shipping the sibling card for prose tips and the fixed card for table tips would have left a trap where a term that later moves into a table silently loses its tooltip.
- **Cycle-drawdown figures are lifted, not re-derived.** The Gap's maturation sentence cites Bull & Bear Cycles' canonical `CYCLES` record (completed cycles at &minus;85 / &minus;84 / &minus;77, with 2011's &minus;93 discounted as a pre-infrastructure artifact) rather than the preview's corpus-sourced "77% to 94%". The current-cycle figure (&asymp;51%) is computed from the shared ~12-day `PL_DATA` series against the same documented 2025 peak, and the basis difference is named in a note beneath the paragraph per STYLE_GUIDE §10.3 rule 5.
- **OG card:** product-forward (§6.15.2) &ndash; hero is the All-rows scorecard including the strips row, captured after a `click_before_capture` on the Scorecard tab. Ratings are editorial, not live data, so this card carries **no** MONTHLY_REFRESH regen dependency (unlike the chart-hero cards). Generated chat-side against the branch preview (Playwright capture + Pillow chrome), since the build sandbox has neither.
  - **Deliberate deviation from the §6.15.2 40&ndash;70 KB target: this card ships at 95,129 bytes (92.9 KB) at q82.** The reason is the hero's nature, not a mis-set quality knob. Every other product-forward card captures a *chart* &ndash; large flat regions and smooth gradients, which JPEG compresses cheaply. This one captures a **text-dense grid**: ten rows &times; seven columns of small pip labels, whose high-frequency edges are exactly what JPEG spends bits on. The quality ladder was walked before accepting: even **q74 only reaches 82 KB**, still outside the band, and it starts visibly softening the cell text that is the entire point of showing the grid. Crisp cell text won over band compliance. **Suggested §6.15.2 amendment at the next docs pass:** *"text-dense grid heroes run 80&ndash;95 KB at q82; expected, not a failure."*
...
**Open follow-ups.** None outstanding on this page. *Closed:* the carousel slide (shipped in-branch, §13); the OG image (`og-bitcoin-as-collateral.jpg` landed 2026-08-18, verified serving `image/jpeg` on the branch preview &ndash; see the size-deviation note in §48.3).

---

## 49. Bitcoin Escape Velocity (`/bitcoin-escape-velocity.html`)

Third page of the retirement family (**The Bitcoin Retirement** flagship · **Retirement Stress Test** · this). Shipped 2026-08-20 after a preview build and three review rounds. Mixed-content width tier (1100px page / 880px prose). No nav entry: the family earns a hub before nav grows (cluster-thesis canon), so `/calculators` — fed by `explorations.json` — is the discovery surface.

**What it is.** The flagship asks whether a whole plan works and holds the audit surface. This page isolates one question — where the threshold sits between a stack that depletes and one that outgrows its spending — and makes it draggable on all three inputs at once. Page order: hero + tool-framing → Set the plan (steppers) → The verdict → The Threshold → Year by year → Assumptions → Verify the math → both-sides callout → related strip.

**The escape-year definition, and why the page's payload is a threshold rather than a year.** Escape velocity is reached at the first modeled year Y such that the *real* residual (real annual stack growth minus real annual withdrawal) is non-negative for **every** year from Y through the horizon — not merely at Y. "Permanently" is horizon-scoped, and the page says so, because the Power Law's implied growth rate decays with time.

That condition turns out to be **monotone under this engine**: once withdrawals outrun growth the stack shrinks and never recovers; once growth leads, the margin only widens. So the escape *year* is the first withdrawal year or never, and reporting it would restate the input. Verified by sweep at build time across stack, withdrawal and retirement-year axes; the one exception is a razor-thin band under `incbasis: fixed`, where the real draw decays. The quantity that actually varies is the **threshold on each input**, and that is what the page reports, plots and lets you jump to.

**Three first-class verdicts**, equal typographic weight, no alarm styling on failure:

- **escape** — "the stack's growth permanently exceeds spending from YYYY";
- **deplete** — "the stack depletes in YYYY";
- **shrink** — survives the window but growth stops covering withdrawals from a named turn year, with "on pace to deplete around YYYY, beyond the window" (rounded to five years; it is an extrapolation past the horizon the reader chose). The shrink branch is not in the original spec and was added at build time: it is reachable at ordinary inputs, and calling it "depletes in YYYY" would be false. A shrinking plan usually *grows for years first* — the default plan ends at 139% of its starting real value — so the copy names the turn rather than claiming it "loses ground every year".

**The threshold solver.** `lineFor(axis, scenario, basis)` is the single implementation of "where is the line on this axis". It places the slider ticks, writes the result sentences, derives the out-of-range rescue tips and powers click-to-jump. There is deliberately no second implementation: a tick that disagreed with its own caption would be worse than no tick. Stack and withdrawal bisect; **retirement year is scanned, not bisected** — the stack a plan needs falls as the year moves out and then rises again once the horizon reaches the low-real-growth end of the Power Law, so escape is not monotone in that input. `basis` is a parameter rather than a read of the `PRICE_BASIS` global; when it was a global read, the QA pass solved under trend while checking under gap-persists and produced 22 false failures.

**Engine sharing and the parity assertion.** Projection math is copied from `the-bitcoin-retirement.js` per the stress-test precedent (`daysSince` / `plPriceAtDate` / `dateForYear` / `projPriceForGrowth`, the `SCENARIO` shape, the sell-to-cover-income loop), reading shared `ModelingAssumptions` and `power-law-data`. The only deliberate deviation is the absence of pre-retirement DCA — this page has three inputs and no contribution field — and the audit table keeps the flagship's reproduce-any-row identity with `dcaAdded` pinned at 0.

`window.evParityQA()` is the guard, and it must stay green: 20 scenario vectors × both price bases, asserting (1) this page's `deplete` state matches the flagship's `achieved === false`; (2) depletion years are identical; (3) a claimed escape year really is the first permanent crossing; (4) the spectrum marker's DOM offset and `.escape` class match the computed verdict — verifying data arrays is not verifying what rendered; (5) the shrink-loop extension leaves every boundary classification unchanged and never places an extended depletion inside the horizon the verdict already cleared; (6) each reported threshold itself escapes while one increment on the failing side does not; (7) click-to-jump lands the exact solver value; (8) live-assumption recompute equals a caches-bypassed recompute across both growth models and all three inflation presets.

**Session-stickiness policy.** The three plan inputs persist in **`sessionStorage` only** — never `localStorage`. The stack is position data: how much bitcoin a reader holds is the most sensitive number they will type on this site, and it must not outlive the tab. Load precedence is **URL params > sessionStorage > defaults**. The only `localStorage` this page touches is shared `ModelingAssumptions`, which holds no position data. (The flagship's `localStorage` stack-stickiness is a separate site-wide policy question — see TECH_DEBT.)

**URL params** (§46 conventions): flagship names `stack` / `retire` / `income` / `years` / `incbasis` so a scenario carries across the whole retirement family, plus this page's own `basis` (price basis), `dollars` (display basis) and `btcstep` / `incstep` (gradation). Defaults are omitted, so a clean URL is the default plan. Values are **not** snapped to the gradation grid on read — snapping a carried-in 0.37 BTC to 0.25 would silently answer a different question than the link asked.

**Relationship to the flagship.** Retirement is the audit surface: every number inspectable, the full year-by-year table, the price chart, the comparison columns. Escape Velocity is the exploration surface. Its collapsed "Verify the math" disclosure is a courtesy check-the-work affordance reusing the flagship's audit renderer, and it cross-links back with the live scenario in URL params. The two pages must never disagree about whether a scenario escapes; `evParityQA()` is what enforces that.

**Defaults: 1.00 BTC / $100,000 / 2035** — the wholecoiner plan, 30-year horizon (→ 2065). Chosen so first load lands on the failure side under both price bases by a clean margin rather than a rounding error, one to two stepper clicks from the line on every axis. Under trend it is a *shrink* (turns over 2055, on pace to deplete ~2080); under gap-persists it depletes in 2043. Defaults are starting positions, never recommendations.

## 50. The Bitcoin Floor (`/the-bitcoin-floor.html`)

Fourth page of the Power Law family (**Bitcoin and The Power Law** · **The Bitcoin Dashboard** · **Discount, or Premium?** · this). Shipped 2026-08-22 after a preview build, four review rounds and JM's walk. Mixed-content width tier (1100px page / 880px prose). Grouped under *The Numbers → Models & Trends*, beside the page that owns the model.

**What it is.** The Power Law page owns the model — the coefficients, the channel, the limits. This page takes the channel's **lower edge** on its own: 0.42× the trend price, at the same exponent, so the two run parallel rather than converging. It answers four things: where price sits against that line right now, every close approach the record contains and what followed each, what entering near the floor has actually paid, and what would falsify the model. Page order: hero + live distance readout → what the floor is (+ channel chart) → the closest approaches → how entering has paid off → the entries (scatter) → the tripwire → the other side → verify → FAQ.

**The reliability reframe (round 2, JM ruling — the load-bearing editorial decision).** The build's first draft dramatised **rarity**: "touched four times, broken once", event-counting in the hero, a section titled *Four times price has come here*. That was the wrong finding. The finding is **reliability** — price has ridden near this line as its ordinary lower bound across the whole record, and the four approaches are *case studies, not violations*. The hero no longer counts events; the record section is framed as "The closest approaches — and what followed", leading with continuity and the 24-month roundtrip. Anyone revisiting this page should know the rarity framing was tried and deliberately retired, not overlooked.

**The genesis-era rule, and why it hardened twice.** The single 2010 sub-floor episode (42.6% under, ≥48 days) sits in bitcoin's pre-maturity era — no mature exchange, negligible liquidity, a price in cents — the same period every careful fit of this model down-weights. Treating it as equal evidence about the modern floor is a **statistical error**, and the page says so in those terms. Round 2 dismissed it in prose; round 4 went further on JM's ruling that **a chart position is weight**, and removed it from the entry scatter entirely. It now appears exactly once, in the record section, and contributes to no measurement and no chart. Two facts worth carrying: the entry set contains **six 2010 samples and no 2011 samples at all** (the next is 2012-05-27), so the surviving era bucket is a single 2012 sample — a "2011–2012" label would name a range with no member; and with genesis excluded, every point on the scatter is modern, so "among the modern approaches alone" became a distinction with nothing on the other side.

**`MODERN_FROM` — one boundary for the whole page.** `2011-01-01`, the same boundary the tripwire's "modern era (2011–)" uses. It governs the scatter, the displayed parity medians, the below/above clause and the worked example. It replaced an inconsistent `2013` cutoff that had crept into two of those during round 3 — if a future edit needs "modern", it uses this constant, not a fresh literal.

**The parity instrument — the page's novel interaction.** Entries are every modern-era sample that closed **no more than 10% above the floor** (no lower limit — the definition is one-sided, and an earlier "within 10% of the floor, above or below it" phrasing implied a ±10% band and caused a real misreading on JM's walk). The card reports three figures as an equation — realized CAGR **−** trend-line CAGR **≈** the difference — where the third is the **median of per-entry differences**, not the difference of the two medians, which is a different and less meaningful number the layout would otherwise invite a reader to compute.

**Endpoint honesty is the interaction, not a footnote.** A synced toggle grades the same entries **to today** or **to the last time price touched the trend line**. What "did the floor pay" means depends entirely on where you stop measuring, and the toggle is what teaches that. Guard: the trend-endpoint result is always labelled the historical *bonus*; the guarantee half — that graded at its worst the entries roughly matched the trend line — leads. Every sentence naming today's position recomputes; none is static.

**The reconciliation line.** Because the page measures on the modern set and the published analysis measured on the full set, the two disagree, and the method line reconciles them explicitly: the published 2026-07-31 measurement **included the genesis samples** and found 63.8% vs 65.1%; the same dates without them, at that same endpoint, give 60.5% vs 61.9%; the difference is the six 2010 samples. Both are asserted (below), so that sentence cannot quietly go stale.

**`floorParityQA()` — three assertions, and why they are pinned differently.** (1) The **published full-set fixture**, 63.8 / 65.1 at n=26, graded to one day past the last sample on a 365.25-day year — the endpoint convention that actually reproduces the note. It passes `entrySet()` explicitly so an editorial decision about what the page *displays* can never drift a regression test. (2) The **modern-set fixture**, 60.5 / 61.9 at n=20, same endpoint — pins the numbers on screen. (3) A **leak check** failing if any pre-2011 sample reaches the displayed set. It also re-derives the four episodes from `PL_DATA` and fails if their dates, spans or depths no longer match the static cards. Deliberately **not** asserted against a live "to today" figure: the medians are endpoint-sensitive at the ±0.1pp tolerance being tested, so that test would fail on the second day for reasons unrelated to correctness.

**The tripwire is canon, and the criteria are CONJUNCTIVE.** Published 2026-08-21: a close **more than 10% below the floor**, held for **more than 30 consecutive days**. Both conditions must hold at once — depth alone is a wick, duration alone is the floor doing its job. An earlier commitment line read "if *either* criterion is met", which would have fired on a one-day 11% wick; that is fixed and the constant carries a comment saying changing these later is **a public act, not a tweak**. "Arms" (depth breached, clock starts) and "fires" (depth sustained) are different events and the copy keeps them apart. The historical record is read from the ~12-day sampled series, so its durations are lower bounds; **the tripwire itself is judged on daily closes from ship date forward** — deliberately different instruments, and the card says so.

**No inputs, no stored state.** The purest read on the site: no plan inputs, no sticky settings, no URL scenario params, no `localStorage` or `sessionStorage`. The only interactions select a view — episode cards, the endpoint toggle, the zoom reset. Nothing a reader does here needs remembering, so nothing is remembered.

**Two shared-module fixes this page produced.** Both live in `shared/power-law-data.js` and affect other pages: `plYearAxisTicks` / `plYearTickLabel` place time-axis ticks on Jan 1 so a duplicate year label ("2015 2015 2016 2016") is structurally impossible — `/the-power-law`'s channel chart had a version of this guarded to linear axes only, so its own log-time toggle fell straight back to duplicates, and it now uses the shared helper. And the page-local `flXNearest` interaction mode returns one tooltip item **per dataset** at the hovered x, because Chart.js's `nearest` returns one item across all datasets (which showed Price only) and `index` matches by array index, which is wrong when the band lines carry ~240 points against the price series' 481.
...
**The unified visit definition (2026-09-01, JM ruling 6) — this page is now its canonical home.** A *visit* is: within **1% of the 0.42× floor or below it**, with a gap of **more than ~100 days** starting a new episode. It formalises language this page already used — the strip has always labelled episodes *graze* or *break* — and adopts the site's existing 100-day independent-visit rule, so every dependent surface moved here in one commit. Authorised on the before/after table in `FLOOR_VISIT_DEFINITION_MINIREPORT.md`.
- **No published figure was recomputed.** Every depth and every 24-month outcome is a number this page already published, carried across unchanged; the only new numbers are structural (dates, sample counts, spans) read straight off `PL_DATA`. This is deliberate and worth keeping: the published outcomes come from the analysis doc and the page's interpolation does not reproduce them exactly (2010 especially), so re-deriving them would have moved figures by an amount unattributable to the ruling.
- **What moved.** The two 2015 episodes merge (24 days apart) into one seven-week approach, and the old card's line arguing *against* that merge is gone. January 2023 gains a December 2022 sample and becomes a 12-day episode, depth and outcome unchanged. A fourth, **open** episode appears — July 2026, still running. `belowPct` goes negative for it, so three helpers own the sign in one place and the copy reads "came within 0.7% of the floor". Open episodes carry no bracket days and no 24-month tiles: **omitted, not rendered empty**, so nothing invites the reader to think a measurement failed. The reversion stats exclude open episodes (median 163% → 147%, 3 of 3 → 2 of 2). The card open on arrival is now the open approach.
- **The sensitivity paragraph, and why it is asymmetric.** The method note states which half of the definition is load-bearing and which is not: the **100-day gap is insensitive from 30 to 200 days** — no episode boundary moves anywhere in that range — while the **1% band is load-bearing for exactly one episode.** Saying so is the difference between a definition and a convenient choice; a reader can see which knob would change the count and which would not.
- **`floorParityQA` asserts openness.** `computeEpisodes()` adopts the same rule and now reports whether an episode is open, and the QA harness asserts it — so a `PL_DATA` refresh that closes the July 2026 episode **fails loudly** instead of leaving a stale "no outcome yet" tile on a public page. This is the pattern to copy: when a page renders a state that the next data refresh can invalidate, the test asserts the state, not just the numbers.
...
## 52. Compare Bitcoin Retirement Plans (`/compare-retirement-plans.html`)

**Shipped 2026-08-26** (`feat/compare-retirement-plans`), built to `COMPARE_RETIREMENT_PLANS_DESIGN.md`. The **fourth spoke of the retirement family** and the one that owns *"which of my options do I take?"*. The family is a decision funnel and each page refuses the others' questions: the flagship answers *what does my plan look like*, Escape Velocity *how much is enough*, this page *which of my options*, the Stress Test *does the winner survive bad luck*.

**The fence that defines the page.** Two things it is repeatedly mistaken for, both stated in design §1: it is **not** the flagship's `rt-compare` panel (that is one plan ± canned relative nudges — variants derived from a base; this page has no base and no variant, both columns are independently configured), and it is **not** Escape Velocity's "one step in any direction" rows (those are *unit sensitivity*, automatic ±1 per input; this page's delta is a **chosen alternative** the reader configured). It is also unrelated to the Financialization cluster's *three-track scenario comparison* despite the phrase.

**Inputs: per-column, controllables only.** Three per plan — stack at retirement, retirement year, annual withdrawal. **Assumptions are shared and cannot be set per column, and this is the page's load-bearing design decision, not a simplification.** If column B could carry a different growth path or a private crash, every verdict row would silently become a model-vs-model claim, and a plan could "win" because it was handed a friendlier world rather than because it was a better choice. Attribution is the product: every visible difference in outcome must trace to a choice the reader made. The one environment control — the bear toggle — applies to **both** columns identically for the same reason, which is what keeps it from confounding anything.

**Plan B initialises as Plan A with the retirement year +1** (JM ruling, design §9). The page opens on a meaningful comparison rather than a blank column, and *"what does one more year buy?"* is the question most readers arrive holding. A carry-in link that supplies only Plan A re-derives Plan B from the arriving plan rather than falling back to the shipped default.

**The engine — one instance, and a new shared module.** Both columns run off `shared/retirement-engine.js`, which is the Escape Velocity engine (itself the flagship's, minus the DCA branch) extracted **verbatim**. The page declares no model constants of its own; design §8.1 makes any per-column re-declaration a defect. **The module is not yet adopted by the other three family pages** — repointing them is filed in `TECH_DEBT §1` as the family's most important structural debt, and JM's 2026-08-26 ruling sequences it as **the next family engineering item after preview feedback, before any other family page work** — two implementations of one model, with golden vectors as the only tripwire between them. Until then the guarantee that the module agrees with the family is carried by golden vectors, below.

**`crpParityQA()` — the tripwire, in two tiers.** (a) identical inputs in A and B produce identical outputs, asserted with the bear toggle **off and on**; (b) six vectors captured from the **deployed** Escape Velocity page on 2026-08-26 (navigating `?stack=&retire=&income=` and reading that page's own rendered verdict, horizon value and three threshold lines), plus the internal memo≡raw and multFn-identity checks; (c) both columns read the same shared constants object, asserted by identity (`E.LIMITS !== LIMITS` fails). **The two tiers exist because the engine is date-sensitive:** price at year Y runs through `dateForYear(Y)`, built from *today's* month and day, so dollar figures and solved thresholds drift daily and step at the year boundary. States, escape/turn/depletion years and whether a threshold is in range **fail** the run; values and thresholds **warn** with the measured-vs-captured pair. A tripwire that failed for the calendar would be worse than none.

**The bear toggle is single-sourced.** It injects `shared/crash-model.js` — the module the Stress Test itself runs on — at that page's shipped defaults (−60%, first year of retirement, historical recovery, ~1yr to trough), anchored to **each plan's own retirement year** so neither plan is punished merely for retiring on a different date. There is no second crash definition on this page. *Verified at build: the crash path was already in `shared/`, so the design's "promote it if page-local" contingency did not fire.*

**The hero chart** is design §5 Option 1 (JM ruling): paired balance curves, one per plan, log y, with a single marked point per curve — where it crosses, or where it runs out. No bands, no 60/40 foil, no gap-persists variant, no per-bitcoin lines: the chart has one job (*which line holds*) and earns its simplicity because the flagship already owns full model context. Hovering a column or a legend entry emphasises that plan and dims the other; a back-and-forth toggle was declined at design time because comparison requires simultaneity. **A depleted plan's curve stops at its depletion year** rather than running along a clamped floor — a log axis has no zero, and a flat line at the clamp would read as "still worth something" for every year the plan is worth nothing. **Chart typography ships at the house standard from day one** (`CHART_FONT = { tick: 12, title: 13, legend: 12 }`), ahead of the still-pending site-wide sweep.

**The delta is sentences, not arithmetic** (design §7). Two columns of numbers make the reader do the subtraction, and the subtraction is not the insight. The **delta strip** sits **above** the verdict table (JM ruling, §9) — it is the headline and the screenshot-and-share unit. Register: state what each plan buys and costs, symmetrically; **no winner-crowning** — verdicts are facts (crosses / depletes / holds) and the reader chooses. The one place the page must name a column is the carry-out link, and it does so by a stated rule (the plan that holds; Plan A when both or neither do) rather than by an editorial judgement.

**URL state, namespaced from commit one** — `a_stack` / `a_retire` / `a_income` / `b_*` / `bear=1`. Retrofitting a namespace breaks every link already shared, so it could not be deferred. The **short names are the family's own** (`stack` / `retire` / `income`, verified against the Stress Test's reader) rather than the design doc's provisional `a_year` / `a_wd`, per the §46 rule that senders speak the receiver's vocabulary; the doc explicitly delegated the exact naming to build. The page **also accepts the un-namespaced family params as Plan A**, so an existing retirement-family link opens here without translation. Write-back is `replaceState`, debounced 250ms; reset strips every param.

**Carry-out verified at build.** The Stress Test **does** expose receiver params (`stack`, `retire`, `income`, `years`, `incbasis`, plus its own crash params), so the "stress-test the winner" link carries the chosen column properly and the design's "if it has no receiver, file it as a separate item" contingency did not fire.

**Integration.** `explorations.json` (category *numbers*, group *Positioning & Strategy*, `interactive: true`, `calculator_tile` at position **245** — between Escape Velocity at 240 and The Bitcoin Floor at 250, so the tile sits with its family); `sitemap.xml` @0.9; `llms.txt` (The Numbers); homepage **Latest** card (leading the row; **The STRC Mechanism rotated off**, verified to retain its Numbers card and its carousel slide) and a **Numbers** concept card placed immediately after Escape Velocity; `updates.json` (8/26/26); SVG tile icon at `components/calc-tile-icons/compare-retirement-plans.njk`. Outbound `related:` to Escape Velocity, the Stress Test, the flagship and The Power Law.

**Deferred at ship, now resolved or still tracked.** (1) **The OG card &mdash; SHIPPED 2026-08-26.** `og-compare-retirement-plans.jpg` was generated chat-side product-forward (§6.15.2) against the live page at **default params**: the default Plan A / Plan B separation reads clearly at card size, so the explicit-params fallback recorded in `OG_ASSET_HANDOFF.md` was not needed, and the suggested waits worked on the first run. The JPEG and its `.eleventy.js` `staticAssets` entry landed in one commit (that array has no existence guard), the `CARDS` entry records the capture decision inline, and the card is now in the `MONTHLY_REFRESH_CHECKLIST §6` regeneration list because it embeds live chart data. **Superseded the same day by `-v2` — see §52.1**, after the copy review changed the page title and the legend the card had baked in. (2) **The carousel slide &mdash; SHIPPED 2026-08-26**, as the set's 43rd, at markup position 34 immediately after Bitcoin Escape Velocity. It cleared the §13 acceptance gate chat-side with room to spare (floor 50.6% against a ~25% requirement, monotonic rise, close 55.0% vs open 50.6%) and the placed file's SHA-1 was verified against the gated encode before wiring. Full record on inventory row 34 and in its iteration record. (3) **Inbound `related:` links from the three existing family pages** &mdash; still deferred; the shared family strip (§53.1) carries the linkage instead.

---


### 52.1 The OG card, and why it is `-v2` on its first day

`og-compare-retirement-plans.jpg` shipped 2026-08-26 and was superseded by **`og-compare-retirement-plans-v2.jpg`** the same day. Not a defect in v1 — the **page** changed underneath it. The copy review that afternoon retitled the H1 to *Compare Bitcoin Retirement Plans* and cut the chart legend from four entries to two, and v1 is a **product-forward** card: it embeds a live screenshot of the hero plus title chrome, so both changes are baked into the image. A card whose title and legend disagree with the page it links to is worse than a generic one.

**The filename bump is the mechanism, and `?cb=` is not.** A cache-buster creates a second card entry against the same URL instead of refreshing it; the `-v2` filename is what forces the social platforms to re-scrape. **v1 stays in the repo and stays registered in `.eleventy.js`** — cards already scraped and posted keep resolving rather than breaking retroactively. That is the BAS `-v2` precedent (`§6.15`, `OG_ASSET_HANDOFF`), applied unchanged. Orphan cleanup, if it is ever wanted, is a separate deliberate pass and not part of a bump.

**Custody, per the pattern the slide established.** The gated encode's SHA-1 was verified against the placed file **before** anything was committed: `8fa9bb62977c028adfa113e5b21b51cf4d63daea`, 61,283 bytes, 1280×720. The gate certifies an encode; the wiring PR certifies this is that encode; the post-deploy check certifies it is what production serves.

**What moved with it, all in one commit** (the `staticAssets` array has no existence guard, so the registration cannot precede the file): the JPEG and its passthrough entry; `og:image`, `twitter:image` and **both** `:alt` texts in the page's head include, the alts rewritten to the new title; the `CARDS` entry's `titleAccent` → `Bitcoin`, `titleAfter` → ` Retirement Plans` and `output_filename` → `-v2`, with the capture-URL comment standing unchanged because the capture is still default params; and the `MONTHLY_REFRESH_CHECKLIST §6` regeneration row renamed.

**The standing lesson for product-forward cards:** they have a dependency on page *copy and chrome*, not only on page *data*. The monthly-refresh discipline covers the data half. The other half has no schedule — any copy review that touches a titled, charted page invalidates its product-forward card, and the card should be recut in the same batch rather than discovered stale later.

---

## 53. Plan Your Bitcoin Retirement (`/bitcoin-retirement.html`) — the family hub

**Shipped 2026-08-26**, immediately after the fourth spoke, per `COMPARE_RETIREMENT_PLANS_DESIGN §10` and the standing cluster canon *"a family earns a hub before nav grows"* (`§49`, recorded when Escape Velocity deliberately shipped with no nav entry at three spokes). Four spokes was the threshold; it has been crossed.

**What it is.** A short landing page: the family's four questions in the order they arrive — **Build · Size · Choose · Stress** — one card per tool, plus two paragraphs on how the tools connect (inputs carry; one engine, one growth path). Nothing is computed here; there is no `page_scripts` entry at all. Container tier is **editorial 960**, not the tool pages' 1100 — four cards and two paragraphs would swim at the wider tier.

**What it is NOT: a nav-bar change — and the distinction is the whole ruling.** The desktop `.nav-links` row has no headroom for a **seventh top-level item**: it wraps the wordmark below 1150px, clips below 919px, and hardcoded sticky offsets break with it (`§47`). That constraint is about the **bar**, not about the menus hanging off it, and the two were conflated in this section's first draft.

- **At ship (2026-08-26, morning):** `category: "hub"`, which `base.njk`'s iterator — walking only foundations / arguments / numbers — never sees, so the page had no nav presence at all. Surfaced three ways: a `/calculators` tile at position **235**, a homepage card, and the family strip on the four spoke pages.
- **AMENDED 2026-08-26 (JM prominence ruling, options 1+2).** The hub's `category` moves **`hub` → `numbers`** and it joins a new **"Retirement" group** inside The Numbers dropdown, listed first and marked as the entry point. **The bar is untouched and was measured, not assumed:** 65px at 1920 and 1150, 90px at 919 — byte-identical to production at every width, with the 919px figure being the pre-existing wrap the §47 constraint describes, confirmed by measuring production as a control. Still six top-level items. **Two further discovery routes** land with it: the dropdown group, and the **family strip on the homepage** (one include above the Explore grid) — which supersedes any reliance on the rotating **Latest** card for family discovery, since that card rotates off by design.
- **Option 3 — a top-level nav item — is NOT resolved here.** It stays filed with the nav-capacity project, where it belongs: it is the one option that would cost bar geometry. The nav-*label* question ("Retirement" vs "Plan Your Retirement") remains deferred with it.

**The hub / dashboard divergence is ruled, not drift.** Both are hubs; they now sit in different categories, and a future reader will notice. The dashboard keeps `category: "hub"` and reaches the nav by a **hardcoded flat anchor in the mobile overlay only** (`§47`, amended 2026-08-18), because it is a *destination-grade read* whose site-wide doorway is the channel ribbon. This hub takes `category: "numbers"` and rides the **data-driven** dropdown, because it is the front door of a tool family that already lives in that dropdown — putting it anywhere else would separate it from the four pages it introduces. **Do not "harmonise" them:** they are answering different questions, and the divergence is the answer.

**Category is a nav-routing field, not a taxonomy.** That is the general lesson. `hub` does not mean "important landing page"; it means "no category loop renders this". Any page needing dropdown presence must carry the category of the dropdown it belongs in, whatever else it is. Two side effects of this hub's move, both checked and both improvements: the Numbers dropdown now takes its active state on the hub, and `related.njk` labels it **"The Numbers"** rather than falling through to the catch-all **"Visual index"**.

**Surfaces that render by category, and what the hub now does on each** (enumerated at the amendment so the move has no unexamined consequence):

| Surface | Mechanism | Hub's appearance | Intended? |
|---|---|---|---|
| Desktop Numbers mega dropdown | `e.category == 'numbers' and e.group == g` | First item of the new **Retirement** group, with the "start here" note | **Yes — the ruling** |
| Mobile nav accordion | same category/group loops | Same, inside the Numbers bucket | **Yes** |
| Dropdown active-bucket styling | `currentCat` from the page's own entry | The Numbers dropdown highlights while on the hub | Yes — it was highlighting nothing before |
| `related.njk` category label | `found.category` chain | "The Numbers" instead of "Visual index" | Yes — an improvement |
| `/calculators` tiles | `calculator_tile` presence, **not** category | Unchanged, position 235 | Unaffected |
| Homepage Explore grid | hand-written cards, **not** category | Unchanged | Unaffected |
| Homepage Latest row | hand-written cards | Unchanged | Unaffected |
| Sitemap / `llms.txt` | hand-maintained | Unchanged | Unaffected |

**Naming (JM ruling, design §9.4).** Slug **`/bitcoin-retirement`** — the head term, deliberately distinct from the flagship's `/the-bitcoin-retirement`; the definite article does the disambiguation and no redirect is involved. Title/H1 **"Plan Your Bitcoin Retirement"**, verb-first per the toolbox naming canon (`STYLE_GUIDE §10.4`) and carrying the head term. **The SEO claim is not defended here:** the backlog's base-rate caution stands — re-pull GSC before treating this page as the family's entry point, because the flagship already draws the site's organic retirement traffic and the hub must not cannibalise it.

**AMENDED again 2026-08-26 (JM review) — the strip became the hub's named route, and the Latest card came off.** Two changes that are one decision:

- **The strip now carries the hub's identity**, not just its links: its own eyebrow, the title *Plan Your Bitcoin Retirement* with the hub's icon beside it, and a hairline frame around the whole object. **On the four tool pages and the homepage that title is a link to the hub** and is the strip's primary route there. **On the hub itself the title is suppressed** — it would restate the H1 a few hundred pixels above — while the eyebrow, framing sentence, row and foot all remain, so the same object is recognisable on every surface.
- **The hub's homepage Latest card was removed.** With the strip sitting directly above the Explore grid, carrying the hub's name, icon and link, the card repeated all three inside a single viewport. **What Daily Conviction Bought returns to the row** — the card that rotated off when the hub went on, and correct by date (8/6 and 8/5, older than the dashboard's 8/10, so it takes the last slot). It keeps the Numbers card it gained in the meantime: **a page in both Latest and its category is the normal state**, not a duplicate. Latest stays at six.

**Net effect on the discovery-route list:** the hub is now reached by the `/calculators` tile (position 235), the **Retirement** group in The Numbers dropdown, and **the named strip on five surfaces** — which *replaces* the Latest card rather than adding to it. That is the improvement: a Latest card rotates off by design and takes the hub's discoverability with it, whereas the strip is permanent on every family page and the homepage.

**AMENDED 2026-09-21 (JM, design review — homepage placement).** On the homepage the strip moved from directly above the Explore grid to **directly beneath the hero, ahead of the Featured carousel**; component and content unchanged. Homepage order is now hero → retirement strip → Featured carousel → Get Updates (Substack) → Recent Updates → Explore. The Latest-card removal above stands: the strip still carries the hub's name, icon and link on the page. Same ruling: the hero's primary button left the solid orange fill for the site's **outlined-pill idiom** (amber border, amber text, the `.insight-cta` family) — nothing on the site uses a solid fill; it stays senior to the secondary through size, weight and position. Copy and hrefs unchanged.

**No OG recut is owed for any of this — checked, not assumed (`§52.1`).** The rule there is that a **product-forward** card bakes in whatever its `hero_selector` captures plus its own title chrome. The compare card captures `#crpChart`, and the retirement card captures `#projectionChart`; **neither selector contains the family strip, the hub, or the homepage Latest row**, and no page title changed in this pass. So the strip's new heading, the border, the icons and the Latest rotation are all invisible to every existing card. Recorded because the §52.1 rule is new enough to be worth demonstrating in the negative: it asks for a check, and the check can come back clean.

### 53.1 The shared family strip (`components/retirement-family.njk`)

**One component, five pages.** The four questions with the current page marked, the sibling links, and one line stating that inputs travel between the tools. Rendered on the flagship, Escape Velocity, Compare Retirement Plans, the Stress Test and the hub itself.

**It carries its own scoped `<style>`, and that is deliberate.** Page CSS on this site is a per-page include, so styling a shared component the usual way means editing one stylesheet per host page — four edits to add one component, and four more every time a fifth page wants it. Bundling the styles into the component makes adoption **exactly one include line** and nothing else, which is what let the strip land on the three existing family pages inside a batch that permitted precisely one change to each of them. Every selector is prefixed `rfam-`; **every colour is a `var()` with a literal fallback**, because the host pages declare their palettes independently and a bare var would fail silently on any page that happened to name a token differently.

**The current page is detected from the page's own `slug` front matter.** A page whose slug is not one of the four renders the strip with nothing marked — which is the correct behaviour on the hub, where "you are here" is the strip itself.

**The component owns its heading (2026-08-26).** It carries its own eyebrow and title, so a host page must NOT wrap it in one. The compare page did — an eyebrow/H2 pair reading "The next question" / "Four questions, in order" — which stacked two headings saying nearly the same thing; it was removed, and the component's header comment now says so, so the next page does not re-add one. Heading level is configurable via an optional `rfam_level` variable (default `h2`) so each surface's document outline stays sane.

**The icon exception, recorded because it looks like an oversight otherwise.** The strip's title takes the hub's calc-tile icon, and each hub detail card takes its own tool's — reuse only, nothing redrawn. **Three of the four tools have a calc-tile SVG; the flagship does not**, because its `/calculators` tile is a **live-chart** (`preview_kind: "live-chart"`, `mini-retirement`) and no SVG file exists for it. Rather than draw one, its **homepage concept-card art is reused inline** on the hub card, with a comment saying why. A `calc-tile-icons/the-bitcoin-retirement.njk` file was deliberately NOT created: it would imply the flagship has an SVG tile, the tile system would never read it, and the next reader would have to work out why it exists. Consequence to know: the icon box holds one 48×48 graphic beside three 80×60 ones, so the flagship's letterboxes slightly narrower within the same box (`xMidYMid meet` — nothing distorts).

**Where it sits:** at the foot of the page body on all four spokes (above the layout-level `related:` strip), and at the top of the hub, where it works as the at-a-glance funnel before the fuller cards below.

**On Compare Retirement Plans it replaced a hand-rolled funnel block**, which is the outcome the component exists for. What the generic strip cannot do is **carry state** — its hrefs are plain — so that page's two stateful handoffs ("stress-test the winner", "size one plan first") sit beneath it in a `crp-carry` block, keeping their element ids for the renderer. That division is the pattern for any future page: the strip is coherence, a page's own carry links are state.
...
**What the page is.** A briefing: where bitcoin sits in its long-run channel, the record of what a position like this one has meant for the decision the reader actually carries, each module routed to the tool that owns it. The Dashboard is the glance, the Gallery is the record, this is the read in between — a taxonomy the sister-tabs control states as UI rather than prose. The word is *map*, never *recommendation*.

**How the inputs are bounded — the v1 "zero inputs" clause is retired.** v1 took no inputs at all and that was the structural guarantee. **v2 takes three** (a stack, a retirement year, a target income) plus a question, and the guarantee is now made by what they do rather than by their absence: **they stay in the browser, they filter which record is shown and the arithmetic that reads from it, and they never turn a record into advice.** Nothing is sent to a server, there is no account, and the stack is held to a stricter rule than the rest — no persistence toggle, forgotten when the tab closes, never in the URL. This is the design the 2026-09-07 counsel pass tested the publisher exclusion against and found satisfied (impersonal, bona fide, general circulation); see the counsel-decision record filed in `OPEN_ITEMS.md`.

**Zero new engines, zero new data, zero refresh burden.** Everything computes live at load from the `shared/` modules. **No line was added to `MONTHLY_REFRESH_CHECKLIST`** and no new external dependency exists — the v1 fence held through the v2 rebuild, and the brand-forward OG card was chosen partly to keep it held.
...
- **Entry-anchored** — `visitOutcomes()`. One named historical entry followed forward by direct lookup against `PL_DATA`. **This reproduces on no source tool**, because no source tool asks the question. It is the design doc's single sanctioned exception to "every number reproducible on its source page", and all four of its conditions are met — including that the method is stated on-page in the row's sources line and re-derives from `PL_DATA` to the displayed precision.

**A case study's horizons all use the entry-anchored method, including the ones the pooled engine could have supplied.** Splicing a pooled band statistic into a narrated visit would give figures individually defensible and collectively incoherent. Each module says which basis it is on in its own words, and **a route to a pooled tool is labelled as the pooled exploration, not as verification** — it will not match a narrated case study and should not.

### Structure

Hero (static H1 → **sister-tabs control** → **live standfirst** carrying the trend multiple only; the no-JS fallback carries **no number**, never a placeholder) → context header (four live cards, one of them "Times at the floor since 2014") → **the premise ribbon**, a one-line conditionality strip with an expandable "why this matters" → **the briefing setup**: two numbers and one question, with a one-line summary chip above it that doubles as the panel’s disclosure toggle ("change ▾" / "done ▴"): the chip is visible whether the panel is open or closed, re-renders from every control that can change what it says, and its open/closed state survives a reload (tab-scoped, its own key, cleared by "Clear everything") → **the module grid** → the roadmap coda ("not here yet") → what would break this → FAQ (8) → Related (8).

**Modules are intent-filtered, not conditional on the answer.** Two are always on — **A3** (the position module, state-aware in identity: floor-approach timeline, last-episode-each-way, or reversion) and **A4** (the bar this position sets). The rest render only when their question is chosen: **D1/D2/D3** (deploy, and the DCA question D2 carries), **R1** (selling to rebuy lower), **B1** (rebalancing bands), **P1/P2** (the retirement threshold, and drawdowns from this zone). **Every module computes the same figures whichever question is selected** — choosing one says what the reader came for; it does not change a number and does not produce a recommendation.

**The premise ribbon and the coda are not modules** and the §5 six-part anatomy does not apply to them. The ribbon sits before everything deliberately, so the historical framing inherits its conditionality rather than standing as an independent claim.
...
- **Independent episode: the ~100-day gap**, lifted from `discount-or-premium.js` so the two pages cannot publish different counts for the same history. The design doc's proposed 30-day rule was struck: `PL_DATA` is a ~12-day grid whose gaps *inside* one continuous episode reach 96 days, so 30 days would have split one real episode into four.
- **Floor visit: within 1% of the 0.42× floor or below**, which is the Floor page's published "approached the line" made numeric. Sensitivity-checked: ±10% on that threshold leaves the count unchanged.
...
**Vocabulary (2026-09-07): the visible noun is "episode", not "stretch".** The data model, the thinness rule and the sources lines already counted in episodes; the surface prose lagged. Both this page and Discount-or-Premium now say "episode" wherever they mean a run. The one place the change was first held back — D-or-P's slider caption, which was reading **sample**-basis figures and printed "median ~9 months" above a row saying "6 episodes · median 8 months" — was ruled by JM the same day: the caption, and the `fastest / median / longest` markers above it, now read the **episode** basis, so `rec.min/median/max` appear in the labelled sample row and nowhere else. The tooltip that defines the noun on first use is kept (§6.13). The one surviving "stretch" in the codebase is `deployment-projection.js`'s `{ key: 'stretch', label: 'Stretch, then revert' }` on `/your-deployment-plan` — a projection path where price stretches *further* from trend before reverting. Different sense, different page, and a state key: leave it.

### The honesty mechanics — the reason the page reads as it does

- **The N<3 rule is counted in EPISODES, not samples, and the page says so.** Where the record behind a module holds fewer than three *completed* episodes, the module **names them individually** — entry, depth, duration, outcome — instead of publishing a median or a hit rate that would look far more solid than the evidence under it. Where a figure is pooled across many matched samples it is labelled as pooled, and **the two are never spliced**. Where a horizon has not arrived, the module says so rather than estimating. Discount-or-Premium carries both populations side by side with their own counts, because the counts are the whole point: a long episode contributes dozens of samples and counts once.
- **The counter-case, where the record is unanimous, is the thinness plus the unsampled scenario.** The shared engine treats any sub-floor position as sitting *on* the floor when matching entries, so **no sustained break appears in this data even in principle** — structurally absent, not merely rare. That absence is a limit of the sample, not evidence about the world.
- **The 2014 cut is disclosed, not hidden.** `channel-entries.js` excludes everything before 2014. The design doc's original "full record only" fence was contradicted by the only engine v1 could compute from, so the fence was amended to "the source engine's record, disclosed" and the sources line states it. Where a module counts from 2010 and a header card counts from 2014, **both are correct and the page says why** — two different year-counts on one screen read as an error unless explained.
- **The lede is live, not static.** Whether price is *at* the floor right now is a fact that changes, and `PL_DATA` can run weeks behind live spot — a hardcoded sentence goes false without anyone noticing. **This was caught in the build**, not in review: spot was 0.51× against a most-recent sample of 0.42×.
...
- **No card in any state duplicates the header** (JM's rule, register round two). Two duplications found under it were removed: the at-trend third card, replaced by the longest episode away from trend on record, and the floor identity's count card, now carrying only the completed/open split.

### Register

No colour on this page encodes an action or a verdict — no green below trend, no red above it, no alarm state near the floor. The one semantic colour marks a **closed** episode versus an **open** one, which is a fact about whether an outcome exists yet. Banned-word sweep run against STYLE_GUIDE §11 plus the page-specific list in `RUNDOWN_DESIGN §9` ("signal", "buy zone", "opportunity", "time to", "should" at the reader): clean. Imperative grep: clean. Second-person uses are the ratified premise-gate copy, the setup panel, and route notes that put the tool "under your own hand" — the reader as decider, which is the register working.

**Every module carries a register line** naming what it is — historical record at this position, or conditional arithmetic on the model — and what it is not. The conditional ones link to **what would break this** at the foot of the page rather than burying it in a footnote.

### Not

Not advice and not a signal service. **Not a decision matrix** — rows were cut at Phase 0 where no engine on the site conditions them on position, or where another page already owns them. The page names those absences in its roadmap coda rather than padding: *a map, growing — never a matrix, padded*. Not a borrowing surface (Bitcoin as Collateral appears in Related only). **No carousel slide** — ruled: a position read is the same case the Dashboard made for having none (§13), recorded here so nobody re-opens it.

### The review history, and what closed it

- **Register review, three rounds, complete.** Round one and a visual pass; **round two shipped 2026-09-05** in two ratified parts (date wording, module-count line, module titles, state-aware title, tooltips and two defects; then the header card recast to "Times at the floor since 2014", the tooltip clipping fix on both grids plus a measured off-screen clamp, inline tip placement per §6.13, and the 2010/2014 era clause). **Round three closed the same day with no further edits.**
- **A record-keeping failure is kept in the record.** For three days the tracker read that the review *had not happened* — an earlier "reviewed both surfaces" instruction went out with the review still to come, and a merge proceeded on it. Nothing public shipped unauthorized: the Floor-page changes were separately ruled on their own before/after table, and this page was unlisted, so no unreviewed surface was reachable by a reader who was not handed the URL.
- **Counsel pass, 2026-09-07 — formal counsel waived for public listing.** No registration attaches at any level (federal securities, federal commodities, state, BitLicense); the publisher exclusion is satisfied by the page's design rather than by disclaimer; the one live exposure is anti-fraud accuracy, which the register governs. Boundary edits were adopted and a licensee-tier versioning spec was written but deliberately **not** applied here — **that spec must never reach the public page.** Three edits are page-local (P2's floor-holds condition, the Coda's DCA wording, P1's "models bitcoin alone"), and JM extended the Coda fix to **D2's register line** the same day, since the record named the "honest read" self-label as the fix but its edit reached only one of the two places carrying it; the fourth is **site-wide and is not this page's**: the collapsible "For exploration only" disclaimer in `components/tool-framing.njk` was **extended** to carry the fee/spread/tax basis, the no-registration line and the computes-in-your-browser line, and it renders on **28 pages**. Anyone editing that strip should read the comment above it first. Formal review is engaged at the first licensing engagement, where it is needed regardless. **The full decision record is filed verbatim in `OPEN_ITEMS.md`.**
- **OG card — shipped.** Brand-forward (§6.15.1), composited by `build-og-the-rundown.py`, **never captured**: every candidate hero on this page is made of live figures and §11.9 bars live figures from the card outright, so **there is no hero selector and never was one** (the clause that once named one was struck 2026-09-05). The card carries no multiple, price, date or count. Image at the repo root, registered in `.eleventy.js` `staticAssets`, tags added in the same commit — those two never separate, because without the registration Cloudflare serves the page's HTML at the image URL with a 200 and the card breaks everywhere with no build error. Full spec: `OG_SPEC_THE_RUNDOWN.md`. **The live file is `og-the-rundown-v2.jpg` — bumped the same day, and NOT because the card was wrong** (it is byte-identical to v1). LinkedIn scraped the v1 URL before the merge, while it still served the page HTML at a 200, and cached that as the image; a social platform’s image cache does not refresh on a page re-scrape, so the URL had to change. `?cb=` cannot do it (§52.1). v1 stays on disk and stays registered. This is a **second, distinct reason** for a day-one bump, separate from §52.1’s stale-product-forward-card case: **a scrape taken too early**, whose lesson is that an unlisted page’s asset URLs can be cached wrong before the asset exists. Full note in `OPEN_ITEMS.md`.
- **The Dashboard's inbound link — shipped.** `feat-sister-tabs-dashboard` carries the reciprocal half of the sister-tabs control and was merged as part of this pass; it is the first and only inbound link, so merging it is what made the page reachable. **The [JM-3] dashboard routing chip is cancelled, not deferred** — the sister-tabs control does that job better and symmetrically, stating the two pages' relationship on both rather than adding a one-way pointer on one. `RUNDOWN_DESIGN.md` still carries the JM-3 ruling in four places as historical record; **nothing should be built from them.**

---

## 55. The shared computation modules (`_pageassets/shared/`)

**What this section is for.** Fifteen files in `src/_includes/_pageassets/shared/` are included into pages through `page_scripts` and expose one window global each. They are the site's defence against two pages quoting different numbers for the same thing. This section records the ones that carry an **editorial decision** rather than only data; the rest (`power-law-data`, `chart-copy`, `calculator-helpers`, the per-page data files) are described where their pages are.

**The extraction rule (JM, 2026-09-04) — the module owns the decision, the page owns the wording.** When the same rule appears on more than one page, extract **the decision** — which figure is permitted, what the values are — and leave **the wording** with each page. Sharing the phrasing too forces several pages into one voice for no safety gain; sharing the decision is the whole benefit. Full statement and the standing evidence: `NEW_PAGE_CHECKLIST §11`.

**A local *port* of another page's logic is the anti-pattern.** The Dashboard's copy of `scanDurations` is the case that proves it: correct when written, and it still let a samples-vs-episodes divergence go unnoticed across three pages, because two copies can be tuned separately without either looking wrong. When you find a port, retire it rather than syncing it.

### 55.1 The three Rundown v2 extractions (2026-09-01, `e96a50a`)

JM ruling 3 — **one copy in the codebase.** Each lift is pure and DOM-free, and the source page now delegates to it rather than carrying a second copy.

| Module | Lifted from | Read by | Note |
|---|---|---|---|
| `ladder-advantage.js` | `lump-sum-or-ladder-in.js` | LSLI, the Rundown | `ladderAdvantage` / `bucketAt` / `advantageCurve` / `eraStartDay`. LSLI keeps the local names as thin delegations so every call site is untouched. `bucketEntries()` was added for the Rundown's sources line — additive; nothing else calls it. |
| `reversion-durations.js` | `discount-or-premium.js` | Dashboard, D-or-P, the Rundown | `scanDurations`, reshaped to take the multiple as an **argument** instead of closing over the page's price. D-or-P now also reads `NEAR_LO`/`NEAR_HI` from it, so **the word the page prints and the band the scan hides inside cannot diverge.** |
| `retirement-engine.js` | already shared | the retirement family, the Rundown | `projectForBasis` / `lineFor` gained an optional trailing argument; existing callers are unaffected. |

`reversion-durations.js` also owns the **display-precision** ruling (JM, 2026-09-05): durations print as whole months, with a sub-month guard (`under a month`, `<1 mo`) because rounding a 0.4-month stretch to "0 months" is worse than the decimal it replaces. It lives with the scan because it is a property of the ~12-day source grid, not a per-page style choice, and because one page printing "4.3 months" while another prints "4 months" reads as two pages disagreeing when they are reporting the identical value. **Computation stays exact — only the printed string rounds.**

### 55.2 `return-window.js` — the annualisation convention (2026-09-04, `66e5d5a`)

Read by the Dashboard, Discount-or-Premium and the Rundown. Exports `MIN_MONTHS = 12`, `mayAnnualise(months)`, and `read(months, fromPrice)` returning the trend price at the window's end, the date, the total return, the annualised rate (**`null` below `MIN_MONTHS` — callers must handle it**) and a ready `rateLine`.

**It exists because the same convention had been re-implemented three times and got it wrong twice.** `STYLE_GUIDE §10.3.1` is the rule; the Dashboard had written its own inline ruling for it and still tested for the wrong thing — *"is this the fastest"* rather than *"is this under twelve months"* — because the decision lived at the call site next to the phrasing, where each site can re-derive it slightly differently. Returning `null` rather than a number is deliberate: a caller cannot accidentally print a rate it was never given.

`MIN_MONTHS` is exported so a **chart** can clip its domain to the same constant its copy reads. §10.3.1 binds plotted points, so a page that switches its readout basis and leaves its axis alone is still publishing the figure the rule forbids.

### 55.3 `channel-entries.js` — the historical channel-entry engine

Read by How Much Cash, Wait-or-Deploy-Now and the Rundown. The HMC/WODN mirror twins share it, which is why a lesson learned on one is a lift rather than a re-derivation on the other.
...
## 56. The brand mark, the icon cascade, and the footer's Elsewhere column

**Shipped 2026-08-29, PR #96 (`23aaf22` → `413d761` → `5bd5018`).** The `brand/` directory had been versioned since 2026-08-15 and deliberately unwired; this is its adoption. Five parts, and the two fences on it matter as much as the mark.

**1 · `components/head-icons.njk` — one source for the favicon cascade.** Included once from `base.njk` beside the canonical nav-CSS and ribbon-CSS blocks: site chrome belongs to the layout, not to 54 copies of itself. **Removed 267 lines from 52 per-page `-head.html` files**, and fixed three defects *by construction* rather than by hand — `/bitcoin-fixed-income` and `/bitcoin-vs-real-estate` had **no icon block at all**, and `/the-melting-ice-cube` carried only the SVG line, missing the `.ico`, both PNGs and the apple-touch-icon. Verified before merge: every diff line across the 52 files is a removal, and every removal is an icon `<link>` or its comment — so rendering is byte-identical everywhere except those three pages, which gain icons they should always have had.

**2 · The favicon set, and the trap inside it.** 16/32/48 plus apple-touch-icon-180 wired from `brand/`, with 48 newly registered in `staticAssets`. `favicon.svg` was **replaced, not kept**: it had been an 87 KB wrapper around a base64 PNG of the *old* icon, and because the SVG is declared first in the cascade, browsers preferring it would have kept showing the old mark while the PNGs showed the new one — a split-brand state no single-page check would surface. `favicon.ico` was swapped last (`5bd5018`) from this branch's own rasters, SHA-1 verified before copying and the ICO header confirmed as three images at 16/32/48. **Every entry in the cascade now renders the same mark; no browser or fallback path is left showing the previous one.**

**3 · The header mark, and the nav-capacity fence around it.** `brand/lcs-mark-5.svg`, left of the wordmark, linking home. **Sized in `em` rather than px** because ten pages override the brand font-size at mobile — an em-sized mark tracks the wordmark on every page without needing to know which.

The fence is the part to read before touching this. The wordmark **already** wraps to two lines on the deployed site below ~950px with no mark involved, growing the nav from 65px to 90px — and several pages hardcode sticky offsets against the single-line height. The mark moves that break to ~1050px. So **the mark hides between 769px and 1099px**, and the nav keeps exactly today's geometry there. **This fences a known defect; it does not fix it.** The honest consequence, stated rather than buried: the mark is absent on common laptop widths (1024–1099), not only on tablets.

**The 920px guard in the first commit was wrong, and how it was wrong is the lesson.** It was calibrated against a local nav harness carrying fewer and narrower items than the real row, so it put the wrap point ~160px too low. The correction (`413d761`) was measured **on the deploy**, hiding and re-showing the mark at the same viewport so the comparison is like-for-like. *A layout threshold measured in a harness is a guess until it is measured on the deployed page.*

**4 · The nav itself — no change, deliberately.** The briefed nav CSS consolidation was already completed and closed in `TECH_DEBT`; the remaining per-page nav overrides encode recorded design decisions (accents, the calc-tier overlay, BvRE's custom nav, Migration's fixed pairing). Removing them would regress those decisions, so nothing was touched.

**5 · The footer's Elsewhere column, and the Work With Me link.** The footer gets its **own "Elsewhere" column** for the LinkedIn pointer rather than an entry under "Site", because every other link in that column is internal and this one leaves. Work With Me had no LinkedIn link at all; one is added to its closing contact paragraph. This does not change §51's *"two entry points, and only two"* — that is about routes **into** the page; this is a route **out** of it.

**Open, and owned elsewhere.** The desktop nav-capacity item in `PAGE_IDEAS_BACKLOG` now has three customers that all turn on one wordmark decision — the pre-existing wrap defect, the mark's full-width presence, and a top-level Retirement entry — so it wants one scoped session rather than three fixes. The 2026-08-29 deploy measurements are filed there with their method.
