...
**Planning & strategy docs live in the repo — the `claude/` prefix is retired as a location convention (2026-08-08).** Every planning doc is tracked at repo root alongside `TECH_DEBT.md` and `PAGE_IDEAS_BACKLOG.md`, including the ten migrated on 2026-08-08: `OPEN_ITEMS.md`, `X_STRATEGY_PLAYBOOK.md`, `REACH_GROWTH_PLAN.md`, `FUNDING_STRATEGY.md`, `CREATOR_CREDIBILITY_KIT.md`, `PARTNERSHIPS_REFERRALS_POLICY.md`, `SEO_AUDIT.md`, `OPENSOURCE_DECISION_BRIEF.md`, `OWN_JOB_STRATEGY.md`, and `GRANTS_RESEARCH_KIT.md`. The `claude/` prefix used to denote assistant-facing, project-only docs kept *outside* the repo — but those copies drifted whenever a session couldn't read them (OPEN_ITEMS twice), so the split was dropped. **Do not recreate it:** no planning doc should live only in the project files; cite planning docs by bare `<NAME>.md`, and treat any surviving `claude/…` path as legacy citation text, not a real location. Three referenced docs are still un-migrated (no repo copy yet — fold each in the same way when a copy is available): `SEO_IMPLEMENTATION_PACK.md`, `FAQ_BLOCKS_AND_SCHEMA.md`, and `THE_BITCOIN_EXIT_PUBLISHED.md`.
...
**History.** Built Substack-first per the 2026-08-05 build prompt (branch `feat/get-updates-substack`); QA'd at 375px + desktop against a static harness (no Node locally), confirmed single-row → stacked with no horizontal overflow and zero third-party requests. Backlog "Owned audience" entry marked shipped in the same PR; `REACH_GROWTH_PLAN`'s "email capture" line is now live (project-only doc, updated out-of-repo).
