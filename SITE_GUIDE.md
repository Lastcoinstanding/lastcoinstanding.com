...
**What it is NOT: a nav change.** **The dashboard precedent governs** (`§47`): `category: "hub"`, so `base.njk`'s nav iterator — which walks only foundations / arguments / numbers — never sees it. This is a hard constraint, not a preference: the desktop `.nav-links` row has no headroom for a seventh item (it wraps the wordmark below 1150px and clips below 919px; hardcoded sticky offsets break with it). The hub is surfaced **three ways only** — a `/calculators` tile at position **235**, a homepage card, and the family strip on all four spoke pages. The nav-label question ("Retirement" vs "Plan Your Retirement") is explicitly deferred with the nav capacity work and is not coupled to it.
