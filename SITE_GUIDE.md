...
**v2 — the channel-position view (branch `feat/hurdle-position-view`, 2026-08-08).** A **view toggle** above the chart (the §6.22 `chart-range-toggle` segmented control — deliberately not a second §6.17 yin-yang) switches one chart between two readings, the band travelling with the view. **From the trend** (default, = v1): upper curve `trendCAGR`, floor band `spot→floor`. **From today's price** (opt-in): upper curve `posCAGR` = spot→trend, floor band `spot→floor` (same lower edge — only the upper swaps). Per-view y-axis (magnitudes differ ~10×). The position view plots from **H=3** — a fence that is **load-bearing for §4.2 compatibility** (it clears the §7-flag-1 overclaim that the own-axis alone does not; dropping it reopens the §4.2 rejection). The v1 "optimistic edge" stat card is reworked into the **position-adjustment card** (renders in both views, position-neutral, sign-carrying, no sixth card). Gating: when the price is not live, the position figures are explicitly labelled " (latest monthly data)", never silently live. URL state gains `?view=trend|position`; the view joins localStorage stickiness. Full record: `HURDLE_RATE_DESIGN.md §13`. **Debug param `?k=` (review-only):** forces channel position (e.g. `?k=1.5`) so the label-neutrality requirement can be checked at a simulated above-trend position without waiting for the market. It overrides spot to k×trend(t), presents as live for the review, is **not linked from the page**, and is **stripped from the URL on any interaction** so it can't leak into a shared link. Used for the mandatory pre-ship neutrality sweep (design §5).
