...
**Params.** `share` (always written) · `tax` · `pos` (**v3.1: a channel position 0–1, 3dp; absent = today.** Was a marker-day in v3 — a stale day-encoded `?pos` is out of range and falls back to today) · **`rebuy`** (**v3.3: numeric channel-position target, absent = at the cap**; legacy `first`→cap, `trend`→0.36, `floor`→0.18) · `stack`. **Sticky:** all five persist to `localStorage` (`lcs.how-much-cash.state`) with precedence URL > store > defaults (addendum A3). The ten v2 params (`exp`, `buf`, `shock`, `hz`, `yld`, `depth`, `btc`, `dep`, `cy`, `rec`) are **retired**: decoded and ignored, never rewritten. Verified.
