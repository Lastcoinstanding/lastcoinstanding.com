...
**Thesis (reframed pre-first-ship, from JM's content review).** How Much Bitcoin answers *what fraction is optimal* (Kelly); this page answers **one question and lands it**: over a long horizon, does adding bitcoin make a portfolio meaningfully better despite the drawdowns? The page leads with a computed **verdict** (the affirmative case stated WITH its cost in the same breath), and the three effects are demoted to *supporting evidence* beneath it. The earlier draft presented three co-equal "faces" and reached no conclusion; the reframe fixed that. **Risk contribution is explicitly distinguished from drawdown** (readers conflate them): drawdown = the one-time whole-portfolio fall in a crash; risk contribution = bitcoin's share of the portfolio's *ongoing* volatility, a structural share. It makes **no optimal-% recommendation** (that is Kelly / How Much Bitcoin).

**The math (self-contained, matches published figures).** Two portfolios, `w` in BTC and `1−w` in a broad-market sleeve, at stated illustrative defaults (BTC vol 45%, sleeve vol 12%, correlation **0.50**, kept on the high side per the post-ETF diversification-weakened note, register §11). `btcRiskShare(w)` = bitcoin's share of portfolio variance; `drawdownHit(w,d)=w·d`; single-hold upside `w·mBtc+(1−w)·mTrad`. **Grounded, sourced defaults replace the old invented multiples:** the reader sets a **time horizon** (default 10y, the missing dimension) and two annual rates, and the engine converts them to the multiples via `(1+rate)^years`. Defaults: bitcoin **15%/yr** (Power-Law-tempered, VanEck base, ≈4.05x over 10y), sleeve **10%/yr** (S&P long-run, ≈2.59x); at 10% the verdict reads about **+5.6% more** final wealth (2.74x vs 2.59x). QA risk-share reference points still reproduced exactly: 2%→4%, 5%→11%, **10%→24%**, **20%→48%**, 50%→85%. No new research pass; reuses the Bull & Bear register.

**Inputs.** Primary lever: bitcoin allocation % (slider 0–100, presets 1/5/10/20). Scenario knobs, in a calmer secondary block (set-and-leave, stress-test Tier-1 style): **time horizon** (years), **bitcoin rate** (%/yr → shows the multiple), **sleeve rate** (%/yr → multiple), and **crash depth** (−40/−60/−77 + slider). Optional total-portfolio $ (comma-formatted; default blank → pure %) renders the answer in dollars. URL-encoded (`alloc`/`hz`/`btcr`/`tradr`/`depth`/`port`).

**Outputs.** (a) The **verdict** (the headline answer, upside with its cost, plus the "small position, outsized weight" point: a 10% position drifts to ~15% by year 10; and the horizon-scaling clause, +14% at 20y); (b) the three **supporting effects** (extra return / drawdown / risk contribution, with a plain drawdown-vs-risk distinction line); (c) the **comparison** across allocations as grouped bars + table (upside always plotted next to its costs); (d) an **"after you choose"** rebalancing-tension note (if bitcoin wins, the position drifts up, and mechanically trimming a winner back to target is a real and open question, honest tension with Disciplined Rebalancing, no fabricated quote); (e) a **"can you hold it?"** line; (f) two-view audit table + CSV. Layout: allocation slider + verdict + effects share one viewport (Wait-or-Deploy pattern).
