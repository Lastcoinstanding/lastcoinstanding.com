...
- **No related strip / no reciprocal back-links.** The jump-back-in row serves the related-strip role; companion back-links are **deferred to the chip phase** so v1 honors the "two surfaces only" ruling. (This is why the page carries no `related:` front matter.)
...
**Open enhancements.** The channel-position chip (site-wide doorway); companion reciprocal back-links; the OG card + staticAssets registration (handback); optional macro tiles (explicitly OUT of v1 — each carries its own refresh tail).
