...
- **A green assertion is evidence about what it covers — nothing more.** The control test above tells you whether a *failure* is real. This is its opposite number: whether a *pass* means anything. When a change alters **how** code is reached rather than **what** it computes, every assertion guarding the computation keeps passing while the call sites regress.
  - **Worked example, 2026-08-26 (the retirement engine repoint).** Aliasing a page's local helpers to a shared module silently removed defaults that read page globals (`base || SCENARIO`, `basis || PRICE_BASIS`) — defaults a shared module cannot have. One call site then threw; another solved thresholds on the wrong price basis, with **wrong numbers and no error**. `evParityQA`, `stFlexQA` and `crpParityQA` all passed throughout.
  - **The sharp part:** `evParityQA` passes those arguments *explicitly*, a habit adopted after an earlier bug where it did not. **Its own hardening made it blind to the regression class the hardening was about** — the one path it exercises was the one path that could not break.
  - **So: name the boundary that moved, and check THERE.** If the change is in how a page calls an engine, the check is the page's rendered output, branch versus production, not the engine's assertion. `TECH_DEBT §1` ("a green assertion is evidence about what it covers") carries the standing recipe for this family.
