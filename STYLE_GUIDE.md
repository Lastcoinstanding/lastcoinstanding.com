...
### 6.48 One CTA vocabulary; section eyebrows (Treatment A, 2026-09-21)

**One CTA box on the site: the `.insight-cta` family** — `border-radius: 6px`, Inter 500, outlined (1px border, no solid fill), amber for the primary. The hero uses it one size up (0.95rem, 12px 26px). **999px pills are tab chips only** (`.insight-tab`, `.seg-btn`) — never a call to action. (One pre-existing exception stands as of this ruling: the Get Updates block's `.gu-btn` "Subscribe on Substack" is a 999px pill; it is a layout-level component on every page and was out of scope for the homepage ship — reconcile it under this rule when that component is next touched.) **Primary and secondary CTAs differ by color only, never by size or shape:** same height, padding, font size and radius; primary `color: var(--orange)` + `border: 1px solid rgba(247,147,26,0.5)`, secondary `color: #d8d2ca` + `border: 1px solid rgba(242,238,232,0.22)`; both on the same scrim when they sit over imagery (`rgba(10,9,8,0.35)` in the hero). Hover stays in the `.insight-cta:hover` family (faint wash, border to full strength). This is the tab-chip grammar applied to buttons: hierarchy is a color state, not a bigger box. Instance: the homepage hero, PR of 2026-09-21, which replaced a filled-pill primary (9/20) and then an outlined-pill primary that outranked its secondary by size (9/21 a.m.).
...
5. Adjacent figures must share a basis or name their bases — an unlabeled today's-dollars figure must never sit near nominal figures. (Labeled real is fine: the sweep table's "today's $" column is the pattern.)
...
**Amendment, 2026-08-26 (JM copy review): name the subject, and freeze the slug when links are already out.** The page shipped as *Compare Retirement Plans* and was retitled to **Compare Bitcoin Retirement Plans** — the H1 accent falling on *Bitcoin*, per the house convention that the `<em>` carries the subject. The original title is purpose-first and parses fine from a list, which is why it passed review at design time; what it does not do is say **what** is being retired on, and this site's whole register is bitcoin-versus-the-alternative. A visitor arriving from search on "retirement calculator" should be able to tell from the title alone that this is the bitcoin one.

**The slug did NOT change: `/compare-retirement-plans` is frozen.** Namespaced `a_`/`b_` comparison URLs were already circulating by the time the title was reviewed, and a slug change would break every one of them — a redirect preserves the path but the whole point of those links is that someone shared a *specific comparison*, and a 301 chain is a worse artifact than a title that is one word shorter than its slug. **The general rule this sets:** once a page's URLs carry user state that people share, the slug is frozen and the title is the only half still free to move. Retitle freely; rename the slug only before the first shareable link exists.

**What a retitle has to sweep, and what it does not.** The page's own `<title>`, `og:title`, `twitter:title` and H1; its carousel-slide label; and its `SITE_GUIDE` section heading, inventory row and iteration-record header. Deliberately *not* swept in the same pass: the `explorations.json` display title and the surfaces that render from it (nav dropdown, `/calculators` tile, homepage concept cards, `related:` strip, `llms.txt`). Those are **list labels in space-constrained chrome**, where the shorter form reads better and the surrounding context already supplies the subject — so the divergence is a decision, not drift. If a future pass wants them aligned, align them deliberately; do not "fix" them on sight.
