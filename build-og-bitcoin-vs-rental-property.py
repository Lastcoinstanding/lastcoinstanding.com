#!/usr/bin/env python3
"""
Brand-forward OG card generator — Bitcoin vs. Rental Property (v2, 2026-09-23 retitle).
...
Adapted from build-og-borrowing-against-your-stack.py, which reproduces its own
committed card byte-for-byte (checked 2026-09-23). Differences from that donor:
  - No accent word: "Bitcoin vs." / "Rental Property" both in bright semibold,
    as on the card this replaces. TITLE_ACCENT = None switches the donor's
    italic-amber accent line off.
  - Subtitle (JM retitle ruling 2026-09-23), two explicit lines, no em-dash
    per STYLE_GUIDE §10:
        "The yield comparison, once landlord costs"
        "and tax are surfaced."
    It replaces "The honest comparison — once landlord costs are surfaced.",
    which was printed on the old card (STYLE_GUIDE §5 / §10.1).
  - Output filename is -v2. A new path forces X and LinkedIn to re-scrape; the
    old cached card (og-bitcoin-vs-rental-property.jpg) keeps serving until
    they do. The original card predates the per-page generators and had no
    build script in the repo, so this script is its first reproducible source.
  - Title + subtitle fonts AUTO-SHRINK to fit the left column before the seam.
    If the overflow guard prints "** WIDE", lower MAX_TEXT_W a touch and rerun.

Run from repo root (where og-synthesis.jpg exists):
    python build-og-bitcoin-vs-rental-property.py
Outputs: og-bitcoin-vs-rental-property-v2.jpg (1280x720, JPEG q80, ~75-95KB).
...
TITLE_ACCENT = None   # no accent word: matches the card this replaces
TITLE_REST_LINES = ["Bitcoin vs.", "Rental Property"]
SUBTITLE_LINES = [
    "The yield comparison, once landlord costs",
    "and tax are surfaced.",
]
URL_FOOTER = "LASTCOINSTANDING.COM/BITCOIN-VS-RENTAL-PROPERTY"
...
# Title: optional italic-amber accent line (none on this card), then bright semibold.
...
while TITLE_FS > 40:
    ...
    widest = max(
        *([d.textlength(TITLE_ACCENT, font=_ital)] if TITLE_ACCENT else []),
...
for ln in ([TITLE_ACCENT] if TITLE_ACCENT else []) + TITLE_REST_LINES:
...
img.save("og-bitcoin-vs-rental-property-v2.jpg", "JPEG", quality=80, optimize=True)
print("saved og-bitcoin-vs-rental-property-v2.jpg", os.path.getsize("og-bitcoin-vs-rental-property-v2.jpg") // 1024, "KB")
