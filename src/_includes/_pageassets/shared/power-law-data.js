/* ============================================================
   Power Law data — canonical PL_DATA + constants + plPrice helper
   ============================================================
   Single source of truth for the historical price series and
   Power Law channel coefficients used across the site.

   Coefficients are Mežinskis / Porkopolis Economics canonical
   values (a = 1.6e-17, b = 5.77). Floor at 0.42× trend, upper
   band at 3× trend.

   CADENCE — PL_DATA is NOT a monthly series (corrected 2026-08-10;
   the long-standing "monthly samples" note was wrong — see DATA_AUDIT
   "PL_DATA sample cadence"). It is 481 samples on a fixed ~12-day
   grid: 476 of the 480 intervals are exactly 12 days, from day 592
   (2010-08-18) to the present. Only the trailing handful of points
   are appended one-per-month by the refresh (MONTHLY_REFRESH_CHECKLIST
   §1), so the last few gaps are ~18–36 days. Consequence for
   consumers: measure durations/streaks in ELAPSED TIME (day-spans →
   months), never by counting samples — a 72-sample below-trend run is
   ~28 months, not 72. (/what-daily-conviction-bought and
   /discount-or-premium already do this; the dashboard's streak tile
   does too.)

   Consumed by: /the-power-law (Channel + log-log charts),
   /bitcoin-vs-real-estate (BvRE projection mode),
   /the-bitcoin-retirement (forward projection),
   /disciplined-rebalancing (channel viz + Math chart).

   Promoted to /shared/ on 2026-05-08 — the 4th-page trigger
   documented in DATA_AUDIT_addendum_phase4.md (rec #1) was met
   when /disciplined-rebalancing landed.

   These declarations are at file scope; concatenated into the
   page's single <script> tag, they become script-level globals
   that consumer IIFEs read via closure. Strict mode is NOT used
   so consumer files that previously redeclared (since removed)
   would have been compatible during the migration.
============================================================ */
...
// ═══════ SHARED CHANNEL-POSITION VOCABULARY (Stage 2H) ═══════
// One graduated label for "where in the channel", consumed everywhere a position is
// described so the slider readout and the prose can never disagree. `pos` is the
...
function positionLabel(pos){
  ...
  if (pos < 0.79) return 'above trend';
  if (pos < 0.95) return 'high in the channel';
