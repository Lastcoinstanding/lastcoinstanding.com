...
// One shared CoinGecko call, consumed by every page. On ANY failure we fall
// back to the latest PL_DATA sample (always fresh after the monthly refresh)
// rather than a separately-maintained constant that can drift on its own.
// onResult(price, source) where source is 'live' | 'fallback'.
function fetchTodayPrice(onResult){
  ...
  fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd', { cache: 'no-store' })
    ...
    .catch(function(){ if (onResult) onResult(fallback, 'fallback'); });
}
