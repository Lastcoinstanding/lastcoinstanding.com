#!/usr/bin/env python3
"""
Brand-forward OG card generator — Bitcoin Allocation Sizing.
Per STYLE_GUIDE §6.15.1 (Pillow two-tier composite), cloned from
build-og-the-bitcoin-retirement-stress-test.py: right half composited from the
canonical og-synthesis.jpg template; left half procedural grain with title +
italic subtitle. No em-dashes, no self-referential text (STYLE_GUIDE §10).

Run from repo root (where og-synthesis.jpg exists):
    python build-og-bitcoin-allocation-sizing.py
Outputs: og-bitcoin-allocation-sizing.jpg (1280x720, JPEG q80).
"""
...
img.save("og-bitcoin-allocation-sizing.jpg", "JPEG", quality=80, optimize=True)
print("saved og-bitcoin-allocation-sizing.jpg", os.path.getsize("og-bitcoin-allocation-sizing.jpg") // 1024, "KB")
