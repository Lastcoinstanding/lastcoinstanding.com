...
- **Where the referenced docs live (updated 2026-08-08).** All planning/strategy docs are now **repo-tracked at root** and edited directly: SITE_GUIDE, STYLE_GUIDE, TECH_DEBT, DATA_AUDIT, MONTHLY_REFRESH_CHECKLIST, NEW_PAGE_CHECKLIST, POSITIONING_STRATEGY_GUIDE, TOOLS_FORWARD_LANGUAGE_KIT, FEEDBACK_SETUP, the design docs, and — migrated 2026-08-08 — OPEN_ITEMS, X_STRATEGY_PLAYBOOK, FUNDING_STRATEGY, REACH_GROWTH_PLAN, CREATOR_CREDIBILITY_KIT, PARTNERSHIPS_REFERRALS_POLICY, SEO_AUDIT, OPENSOURCE_DECISION_BRIEF, GRANTS_RESEARCH_KIT. The one exception is OWN_JOB_STRATEGY, which is project-side (removed from the repo 2026-09-16 per the 2026-08-10 sensitive-docs ruling; cite by name only). **The `claude/` prefix is retired as a location convention** (SITE_GUIDE §7) — do not recreate the project-only split, and cite planning docs by bare `<NAME>.md`. A few older docs still referenced with a `claude/…` path are not yet migrated (`SEO_IMPLEMENTATION_PACK`, `FAQ_BLOCKS_AND_SCHEMA`, `THE_BITCOIN_EXIT_PUBLISHED`); those citations stay legacy until the docs land at root.
...
- [ ] **Direction-aware matching — separate the entry leg from the recovery leg.** (JM capture, 2026-09-05. **MEASURED 2026-09-23 — `DIRECTION_AWARE_MATCHING_2026-09-23.md`, script `scripts/measure-direction-legs.js`; awaiting JM's build ruling.** Headline: at today's 0.50× the legs agree and nothing published would change; from about 0.7× to 2× trend they split sharply. At 0.92×, the pooled 54% is 72% on a falling leg and 37% on a rising one, and the sign holds across 36/60/96-day windows. The rising legs rest on only 2–3 halving cycles. Recommended: an additive `leg` field plus a disclosure sentence that prints its cycle count; never replace a pooled figure. NOT BUILT.) `shared/channel-entries.js` matches historical samples on **channel position alone**. A stretch below trend is traversed twice — once on the way *down* and once on the way *back up* — and both legs land in the same matched set at the same position, so a "what happened from 0.52×" figure pools an entry made into a deepening fall with one made into a recovery already under way. The engine cannot currently tell them apart, and every consumer inherits that: Wait-or-Deploy's pooled record, How Much Cash's mirror of it, the Rundown's deploy and drawdown modules, and the Dashboard's position-carried links.
  **Why it is the next honest refinement rather than a defect to fix now:** the pooling is disclosed nowhere because nobody has drawn the distinction yet, and separating the legs would roughly halve each matched set — which pushes several published figures toward the N<3 fence and may replace one honest pooled number with two numbers too thin to publish. That is a finding in itself and worth knowing before any page promises a direction-aware read.
  **Shape, if it is built:** a `leg` field on each matched entry (`falling` / `rising`, decided by the sign of the position change over some window before the sample), exposed on `bandMetrics()` so consumers can split, pool, or disclose. Do NOT split the published figures until the resulting counts are measured — the mini-report pattern applies, before/after side by side. Related: the era toggle below, which is the same shape of question about *which* samples belong in a matched set.
...
- [x] **`/bitcoin-vs-rental-property` — retire "The Honest Comparison" from the page title & social identity. DONE 2026-09-23 (`fix/rental-retitle`), JM ruling: "Bitcoin vs. Rental Property: Yield After Landlord Costs".** Applied to `<title>`, meta description, OG/Twitter titles, descriptions and image alts, and the JSON-LD name/description; `llms.txt` description too. The phrase was also **printed on the social card**, so the card was regenerated as `og-bitcoin-vs-rental-property-v2.jpg` (subtitle "The yield comparison, once landlord costs and tax are surfaced.") by the new `build-og-bitcoin-vs-rental-property.py`, adapted from the BAS generator (which reproduces its own committed card byte-for-byte); the old card stays registered for cached shares. **Second JM ruling, same PR:** the subtitle's "no policy risk" dropped on the page, in the social descriptions and on the homepage Explore card — STRC/SATA dividends are discretionary, so the claim overstated; the qualified body sentence ("no policy risk in the form a city council can produce") stays. **Owed after merge:** X/LinkedIn card re-scrape. Original entry follows. SEO decision, not a copy fix.**
...
- [ ] **Desktop nav capacity + The Dashboard entry.** Surfaced 2026-08-18, building the dashboard mobile-nav fix. The mobile overlay item shipped; **the matching desktop anchor was built, measured and reverted before merge** because the desktop row has no room for it. This entry carries the measurements so the next pass does not re-derive them.
  ...
  - **THREE CUSTOMERS NOW, AND THEY CLOSE TOGETHER (updated 2026-08-29).** This item started as "where does The Dashboard go". It now blocks three separate things, all of which turn on **one decision about the wordmark**, so it wants **one scoped session** rather than three passes: (1) the **wrap defect below ~950px**, which is live on production today and is nobody's feature; (2) the **brand mark's full-width presence** — the mark shipped guarded, absent from 769–1099px, and only the wordmark decision gives it back; (3) the **top-level Retirement entry**, which the retirement-family hub wants and which capacity currently forbids.
  - **Measured on the deployed site, 2026-08-29** (brand-adoption branch, homepage, mark hidden and re-shown at the same viewport so the comparison is like-for-like):

    | Viewport | Without the mark (= production today) | With the mark |
    |---|---|---|
    | 1100px+ | nav 65px, 1 line | nav 65px, 1 line |
    | ~1060px | nav 65px, 1 line | nav 65px, 1 line |
    | ~1040px | nav 65px, 1 line | **nav 90px, wraps** |
    | 1000px | nav 65px, 1 line | **nav 90px, wraps** |
    | 900px | **nav 90px, wraps** | (mark hidden by the guard) |
    | 820 / 780px | **nav 90px, wraps** | (mark hidden by the guard) |

    So: **production wraps below ~950px with no mark involved** — a pre-existing defect, not something the mark introduced — and the mark moves that threshold to **~1050px**. The shipped guard hides the mark from **769–1099px**, which clears the measured threshold by 40px and leaves nav geometry byte-identical to production everywhere it is hidden (verified at 1024px: nav 65px, no wrap, exactly as without the mark).
  - **DISCREPANCY TO RECONCILE, not to resolve by picking one.** The 2026-08-18 table above puts the no-7th-item nav as fitting **down to 804px**; the 2026-08-29 deploy measurement above has it wrapping at **900px**. Both are recorded rather than one overwriting the other. The 2026-08-29 figure is the more trustworthy of the two for present purposes — it was taken on the live deploy with real fonts loaded, and the nav-height jump 65px → 90px is unambiguous corroboration of the wrap. Candidate explanations for the gap: the nav gained width between the two dates, the earlier pass measured a page with a per-page brand override, or fallback-font metrics before the webfont settled. **The session should re-measure from scratch before designing to either number.**
  - **What the wordmark decision has to satisfy**, given all three customers: enough width returned to clear a 7th nav item AND the ~42px the mark costs, at every viewport down to the 768px hamburger breakpoint, without changing the 65px nav height that pages hardcode sticky offsets against.
...
- [x] **Retirement Scenario Comparison — two independent plans, side by side. SHIPPED + LIVE 2026-08-26** as **`/compare-retirement-plans`** (slug `compare-retirement-plans`), build `e3e27c1`, **merged to production via PR #82 as `9056f5a`**; carousel slide PR #86 (`6ae37f4`), OG card PR #85 recut to `-v2` PR #90 (`6d3d9a9`), copy review PR #88 (`a454828`). Post-deploy verified: page + slide + `-v2` card all serving, `crpParityQA` passing. New page. Built to `COMPARE_RETIREMENT_PLANS_DESIGN.md` (historical from ship; `SITE_GUIDE §52` is authoritative). Original entry below.
...
- [x] **Retirement family hub — the trigger has fired. SHIPPED + LIVE 2026-08-26** as **`/bitcoin-retirement`** (slug `bitcoin-retirement`, title *Plan Your Bitcoin Retirement*), build `5e20a59`, **merged to production via PR #83 as `0ab4c1f`**; prominence options 1+2 via PR #89 (`5ff5951`). Post-deploy verified: hub + family strip on all five surfaces, nav bar geometry unchanged at every measured width. Landing page + shared component; NO nav-bar change. `SITE_GUIDE §53`. Original entry below.
  ...
  - **PROMINENCE SHIPPED 2026-08-26 (JM ruling, options 1+2), option 3 explicitly NOT resolved.** The hub itself shipped earlier the same day (`SITE_GUIDE §53`); this closes the *discoverability* half. **Option 1 — a grouped "Retirement" subsection inside The Numbers dropdown:** the hub listed first and marked as the entry point, then the four tools in funnel order (Build → Size → Choose → Stress). **Option 2 — the family strip on the homepage:** one include above the Explore grid, which supersedes any reliance on the rotating **Latest** card for family discovery. **Option 3 — a top-level nav item — stays filed with the nav-capacity project and is not resolved here**, because it is the only one of the three that costs bar geometry; the bar was measured unchanged at every width (65px at 1920/1150, 90px at 919 — the last being the pre-existing wrap, confirmed against production as a control). The nav-*label* question travels with option 3.
- [x] **Family coherence pass — one deliberate read of all four pages plus the hub. DONE 2026-09-23 — findings in `RETIREMENT_FAMILY_COHERENCE_2026-09-23.md`.** Seven findings, no page edits, cross-filed: F1/F4/F6 to `TECH_DEBT` §1, F2/F3/F5 as the three entries directly below. Headline: the flagship calls a plan "escape velocity" that EV, Compare and the Stress Test all say is shrinking (1 in 7 non-depleting plans on an 80-plan grid). Original entry follows. Audit, not an edit pass.
...
- [x] **Make "your plan travels between them" true — flagship and Stress Test senders (coherence F2, 2026-09-23). SHIPPED 2026-09-23 (`fix/family-carry-and-copy`).** The flagship gained a "Take this plan to the next question" row under its verdict (Size it / Weigh it against another plan / Stress-test it); the Stress Test gained a "Carry this plan" row under its headline, and its flagship connect card now carries state. **Stack semantics handled, not assumed:** EV and Compare receive the BTC *at retirement* read off the page's own projection (its `retire` row), the Stress Test and flagship exchange raw inputs including `dca`; under "today's gap persists" EV gets the active-path count plus `basis=current` and Compare the trend-path count, since it runs trend only. A footnote says so whenever the receiving page will see something different from this one. Verified by round trip: 4 trend plans (with and without DCA) reach the same verdict on every receiver; current-basis plans agree with EV and disclose the trend-only receivers. Original entry follows. Small build. The strip, hub and homepage all promise the plan carries; measured on production, only EV and Compare send it. The flagship — the funnel's entrance and 30% of site users — carries nothing into EV, Compare or the Stress Test (its `TEASER_SELECTORS` targets only Borrowing Against Your Stack), and the Stress Test carries nothing back. **Receivers already exist and were verified** (EV, Compare-as-Plan-A and the Stress Test all adopt `stack`/`retire`/`income`/`years`/`incbasis`), so this is sender-only: extend the flagship's selector list to its family links and give the Stress Test the same sender. Detail: `RETIREMENT_FAMILY_COHERENCE_2026-09-23.md` F2.

- [x] **Compare replaces a carried `years` with 30 without saying so (coherence F3, 2026-09-23). SHIPPED 2026-09-23 (`fix/family-withdrawal-horizon`), JM ruling: adopt it.** An arriving `years` (clamped 5–60, as on EV) and `incbasis` now set the shared horizon and withdrawal basis for both plans, show on the shared card, persist in Compare's own URL when non-default, and carry onward. **`incbasis` was the larger half:** Compare had ignored it too, so a fixed-dollar plan the flagship called escape velocity read "runs out in 2049" on Compare, and since #110 that was one click away. The memo key gained both settings, so Reset cannot serve a stale result. Original entry follows. Ruling, then a small edit. Arriving with `years=27`, Compare runs its shared 30-year horizon (design §3, correct) but does not tell the reader, and forwards `years=30` to every onward link, so a reader's Stress Test run via Compare is a longer retirement than the one they set. Options: adopt an arriving `years` as the shared horizon, or keep 30, say so on carry-in, and forward the original. Detail: `RETIREMENT_FAMILY_COHERENCE_2026-09-23.md` F3.

- [x] **One name for the spending input; say what a carried stack means (coherence F5, 2026-09-23). SHIPPED 2026-09-23 (`fix/family-withdrawal-horizon`), JM ruling: "Annual withdrawal".** 75 reader-facing strings across the flagship, EV, Compare and the Stress Test: labels, tooltips, toggles ("Interpret my withdrawal as"), table headers, summaries, formulas, CSV headers and aria labels; the spending cut speaks of *spending*. Rule recorded as `STYLE_GUIDE §10.11`. Deliberately kept: essay prose on the flagship's Question and Strategies tabs that discusses retirement income as a concept, and the page name *Bitcoin and Fixed Income*. The carried-stack half shipped in #110 (the carry footnote). Original entry follows. Ruling, then a copy pass. The `income` param is labelled "Target income" (flagship), "Target annual income (USD)" (Stress Test) and "Annual withdrawal" (EV, Compare). "Stack" is today's holdings plus DCA on the flagship/Stress Test but the stack at retirement on EV/Compare; the carry is numerically safe (DCA lands at 0, verified), but the receiving page never says how it read the number. Suggestion: "Annual withdrawal" family-wide, plus one line on carry-in. Detail: `RETIREMENT_FAMILY_COHERENCE_2026-09-23.md` F5.
