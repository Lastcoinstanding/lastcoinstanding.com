#!/usr/bin/env python3
"""
Brand-forward OG card generator — Lump Sum or Ladder In?
...
Run from repo root (where og-synthesis.jpg exists):
    python build-og-lump-sum-or-ladder-in.py
Outputs: og-lump-sum-or-ladder-in.jpg (1280x720, JPEG q80, ~75-95KB).
"""
...
SEED = 5177; random.seed(SEED)
TEMPLATE = "og-synthesis.jpg"

TITLE_LINES = ["Lump Sum or", "Ladder In?"]
SUBTITLE = "Lump sum or DCA — does where you sit in the channel change the answer?"
URL_FOOTER = "LASTCOINSTANDING.COM/LUMP-SUM-OR-LADDER-IN"
...
img.save("og-lump-sum-or-ladder-in.jpg", "JPEG", quality=80, optimize=True)
print("saved og-lump-sum-or-ladder-in.jpg", os.path.getsize("og-lump-sum-or-ladder-in.jpg") // 1024, "KB")
