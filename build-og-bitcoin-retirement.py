#!/usr/bin/env python3
"""
Brand-forward OG card generator — Plan Your Bitcoin Retirement (the hub).
Per STYLE_GUIDE §6.15.1 (Pillow two-tier composite), cloned from
build-og-the-bitcoin-retirement-stress-test.py: right half composited from
the canonical og-synthesis.jpg template; left half procedural grain with
title + italic subtitle. Title treatment per OG_ASSET_HANDOFF: "Plan Your"
+ italic-amber "Bitcoin" + "Retirement", accent word on its own line.
Subtitle drawn as two explicit lines (sentence-boundary break).
No em-dashes, no self-referential text (STYLE_GUIDE §10).

Run from repo root (where og-synthesis.jpg exists):
    python build-og-bitcoin-retirement.py
Outputs: og-bitcoin-retirement.jpg (1280x720, JPEG q80).
"""
...
SEED = 6101; random.seed(SEED)
TEMPLATE = "og-synthesis.jpg"

TITLE_SIZE = 60
# (text, style) per line; "accent" renders italic-amber per the handoff.
TITLE_LINES = [("Plan Your", "normal"), ("Bitcoin", "accent"), ("Retirement", "normal")]
SUBTITLE_LINES = [
    "Four questions, in the order they actually arrive.",
    "Build a plan, size it, choose between options, then stress it.",
]
URL_FOOTER = "LASTCOINSTANDING.COM/BITCOIN-RETIREMENT"
...
cor = fetch_css_fonts("https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;1,400;1,600&display=swap", "cor")
...
cor_ital_semi = cor.get("600_italic", cor_ital)
...
tf_accent = F(cor_ital_semi, TITLE_SIZE)
y = 238
for ln, style in TITLE_LINES:
    if style == "accent":
        d.text((100, y), ln, font=tf_accent, fill=AMBER)
    else:
        d.text((100, y), ln, font=tf, fill=TEXT_BRIGHT)
    y += TITLE_SIZE + 14

sf = F(cor_ital, 28)
yy = 476
for ln in SUBTITLE_LINES:
    d.text((100, yy), ln, font=sf, fill=TEXT_DIM); yy += 38
...
for ln, style in TITLE_LINES:
    fnt = tf_accent if style == "accent" else tf
    w = d.textlength(ln, font=fnt); print(f"title '{ln}': {w:.0f}px -> ends x={100 + w:.0f}" + ("  ** WIDE" if 100 + w > 600 else ""))
for ln in SUBTITLE_LINES:
...
img.save("og-bitcoin-retirement.jpg", "JPEG", quality=80, optimize=True)
print("saved og-bitcoin-retirement.jpg", os.path.getsize("og-bitcoin-retirement.jpg") // 1024, "KB")
