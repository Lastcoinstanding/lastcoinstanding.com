...
- [ ] **Horizon `.wrap` / `.wrap-narrow` / `.wrap-wide` consolidation.** All three are now identical (960px after commit `55fe517`) and could be consolidated to a single `.wrap` rule used everywhere. Also: remove the dead `.intro .wrap-narrow` rule at L739 of `the-bitcoin-horizon.css` — orphaned reference, no markup uses that combo. Page-specific cleanup, not a site-wide concern.
