#!/usr/bin/env python3
"""
Brand-forward OG card generator — Wait, or Deploy Now?
...
Run from repo root (where og-synthesis.jpg exists):
    python build-og-wait-or-deploy-now.py
Outputs: og-wait-or-deploy-now.jpg (1280x720, JPEG q80, ~75-95KB).
"""
...
SEED = 2629; random.seed(SEED)
TEMPLATE = "og-synthesis.jpg"

TITLE_LINES = ["Wait, or", "Deploy Now?"]
SUBTITLE = "Sometimes waiting for a better entry pays off — and the channel tells you when."
URL_FOOTER = "LASTCOINSTANDING.COM/WAIT-OR-DEPLOY-NOW"
...
img.save("og-wait-or-deploy-now.jpg", "JPEG", quality=80, optimize=True)
print("saved og-wait-or-deploy-now.jpg", os.path.getsize("og-wait-or-deploy-now.jpg") // 1024, "KB")
